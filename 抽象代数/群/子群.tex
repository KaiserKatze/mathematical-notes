...
运算\(\times\)的限制\(\otimes\)在新的定义域\(H^2\)上仍然具有和原本的运算\(\times\)一样的性质.
如果\(\times\)在\(G^2\)上服从结合律，那么\(\otimes\)在\(H^2\)上也服从结合律.
如果\(\times\)在\(G^2\)上服从交换律，那么\(\otimes\)在\(H^2\)上也服从交换律.
...
设\(\opair{G,\times}\)是群，
\(H\)是\(G\)的一个非空子集.
又设映射\(\otimes\)是\(\times\)在直积\(H^2\)上的限制，
即\(\otimes = (\times \SetRestrict H^2)\).
...
则\begin{equation*}
	e' \otimes e' = e'.
\end{equation*}
根据子群的定义，\(\otimes = (\times \SetRestrict H^2)\)；
...
除此以外的子群，但凡存在，
都统称为“\(\opair{G,\times}\)的\DefineConcept{非平凡子群}（nontrivial subgroup）”.
\end{definition}
...
记作\(\opair{H,\times}\AlgebraProperSubstructure\opair{G,\times}\).
\end{definition}

\begin{proposition}
子群中任一元素的逆元都在子群中，且该元素在子群中的逆元也是它在群中的逆元.
\begin{proof}
设\(\opair{H,\times}\)是群\(\opair{G,\times}\)的一个子群.
由群、子群、二元代数运算的定义可知，
\(H\)对\(\times\)封闭，
即\begin{equation*}
...
\begin{theorem}
%@see: 《近世代数》（熊全淹） P25 定理1
设\(\opair{G,\times}\)是群，
\(H\)是\(G\)的一个非空子集，
那么“\(\opair{H,\times}\)是\(\opair{G,\times}\)的子群”的充分必要条件是：
\(a,b \in H\)蕴含\(a \times b \in H\)
且\(a \in H\)蕴含\(a^{-1} \in H\).
\end{theorem}

\begin{theorem}
%@see: 《近世代数》（熊全淹） P25 定理2
设\(\opair{G,\times}\)是群，
\(H\)是\(G\)的一个非空子集，
那么“\(\opair{H,\times}\)是\(\opair{G,\times}\)的子群”的充分必要条件是：\begin{equation*}
...
考虑到\((b^{-1})^{-1} = b\)，
于是\begin{equation*}
	a \times b = a \times b \in H,
\end{equation*}
这就是说\(H\)对\(\times\)封闭，
或者说\(\times\)可以看成是定义在\(H\)上的一个二元代数运算.
由于运算\(\times\)服从结合律，
那么作为\(\times\)的限制，
...
就有\begin{equation*}
	(\forall a \in H)[
		a \times e = a \times e = a = e \times a = e \times a
	],
\end{equation*}
这就说明\(e\)是\(\opair{H,\times}\)的单位元.
...
%@see: 《近世代数》（熊全淹） P25
数域\(K\)上的行列式为\(1\)的全体\(n\)阶矩阵是
数域\(K\)上的\(n\)阶一般线性群\(\GeneralLinearGroup_n(K)\)的一个子群，
称为“数域\(K\)上的\(n\)阶\DefineConcept{特殊线性群}”，
记作\(\SpecialLinearGroup_n(K)\).
...
\(a,b \in H\)蕴含\(a \times b \in H\).
% TODO proof
% 因为这是可以用消去律代替群定义中所要求的单位元、逆元的存在性
% 而且消去律在群G中成立，它保证消去律在H中也同样成立
\end{corollary}
