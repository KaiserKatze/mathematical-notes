...
%@see: 《近世代数》（熊全淹） P16
全体正有理数对乘法成群，单位元是\(1\).
有理数集\(\mathbb{Q}\)对加法成群，单位元是\(0\)；
但是它对乘法只成半群，而不成群，因为\(0\)没有乘法逆元.

%@see: 《近世代数》（熊全淹） P16
仅由一个自然数\(0\)组成的集合对加法成群，但对乘法不成群.
仅由一个自然数\(1\)组成的集合对乘法成群，但对加法不成群.
仅由两个整数\(1,-1\)组成的集合对乘法成群.

%@see: 《近世代数》（熊全淹） P16
数域\(K\)上的全体\(n\)阶满秩矩阵对乘法成群，
称为“数域\(K\)上的\(n\)阶\DefineConcept{一般线性群}”，
记作\(\GeneralLinearGroup_n(K)\).
一般线性群的单位元是单位矩阵.
...
\begin{definition}
%@see: 《近世代数》（熊全淹） P16
设只有一个元素的集合\(G \defeq \{e\}\)对于\(\times\)运算成群，
则称“\(\opair{G,\times}\)是\DefineConcept{单位元群}”.
\end{definition}
...
设\(G\)是有限集，
\(G\)对于\(\times\)运算成群，
则称“\(\opair{G,\times}\)是\DefineConcept{有限群}”.
\end{definition}
...
设\(G\)是无限集，
\(G\)对于\(\times\)运算成群，
则称“\(\opair{G,\times}\)是\DefineConcept{无限群}”.
\end{definition}
...
\end{enumerate}
%@see: https://unimath.github.io/agda-unimath/group-theory.abelian-groups.html
\end{definition}
