...
三条坐标轴中任意两条可以确定一个平面，这样定出的三个平面统称为\textbf{坐标面}.\(x\)轴及\(y\)轴所确定的坐标面叫做\textbf{ \(xOy\)面}，\(y\)轴及\(z\)轴所确定的坐标面叫做\(yOz\)面，\(z\)轴及\(x\)轴所确定的坐标面叫做\textbf{ \(zOx\)面}.
...
\draw(N)node[below]{\(N\)};
\draw(M)node[right]{\(M\)};
\draw(R)node[above right]{\(R\)};
\draw(K)node[right]{\(K\)};
\draw(Q)node[below right]{\(Q\)};
\end{tikzpicture}
...
\]上式称为向量\(\mat{r}\)的\textbf{坐标分解式}；%
\(x\mat{i}\)、\(y\mat{j}\)、\(z\mat{k}\)称为向量\(r\)沿三个坐标轴方向的\textbf{分向量}.
...
向量\(\mat{r}=\vec{OM}\)称为点\(M\)关于原点\(O\)的\textbf{向径}（或\DefineConcept{位矢}）.
...
\begin{example}
% Manipulate[ParametricPlot[{a Cos[t],b Sin[t]},{t,0,2 \[Pi]}],{a,1,100},{b,1,100}]
...
\frac{x^2}{a^2} + \frac{y^2}{b^2} = 1.
\]这是椭圆的标准方程，所以点\(P\)的轨迹是椭圆（见\cref{figure:解析几何.椭圆的参数方程} ）.
...
如\cref{figure:解析几何.椭圆的焦点三角形} 所示，设\(P\)是椭圆\(C: \frac{x^2}{a^2} + \frac{y^2}{b^2} = 1\)（\(a>b>0\)）上任一点，\(F_1\)、\(F_2\)是\(C\)的焦点，那么\[
...
\begin{theorem}
设直线\(l: Ax+By+C=0\)与椭圆\(C: \frac{x^2}{a^2} + \frac{y^2}{b^2} = 1\)（\(a>b>0\)）相交于两点\(M\)和\(N\)，那么\[
\abs{MN} = \frac{2ab\sqrt{A^2+B^2}\sqrt{a^2 A^2 + b^2 B^2 - C^2}}{a^2 A^2 + b^2 B^2}.
\]

特别地，若\(l\)过\(C\)的焦点，\(l\)与\(x\)轴的夹角为\(\theta\)，那么\[
...
% RevolutionPlot3D[{Sec[t], Tan[t]}, {t, -Pi, Pi}, PlotRange -> {-5, 5}]
\item \textbf{双曲抛物面}（又称\textbf{马鞍面}） \(\frac{x^2}{a^2}-\frac{y^2}{b^2}=z\).
% RevolutionPlot3D[{Tan[t], Sec[t]}, {t, -Pi, Pi}, PlotRange -> {-5, 5}]
...
由于过空间一点可作而且只能作一条直线平行于一已知直线，所以当给定直线\(L\)上一点\(M_0\opair{x_0,y_0,z_0}\)和它的一个方向向量\(\mat{s}=\opair{m,n,p}\)时，直线\(L\)的位置就完全确定了.设点\(M\opair{x,y,z}\)是直线\(L\)上的任一点，那么向量\(\vec{M_0M}\)与\(L\)的方向向量\(\mat{s}\)平行，所以两向量的对应坐标成比例，由\(\vec{M_0M}=\opair{x-x_0,y-y_0,z-z_0}\)，从而有\[
\left\{ \begin{array}{c}
x-x_0=\lambda m, \\
y-y_0=\lambda n, \\
z-z_0=\lambda p
