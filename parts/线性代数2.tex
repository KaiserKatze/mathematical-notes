%TODO
%@see: https://mathworld.wolfram.com/PolynomialNorm.html

\chapter{多项式环}
\input{线性代数/多项式/多项式}
\input{线性代数/多项式/整除性}
\input{线性代数/多项式/最大公因式}
\input{线性代数/多项式/最小公倍式}
\input{线性代数/多项式/不可约多项式}
\input{线性代数/多项式/重因式}
\input{线性代数/多项式/多项式的根}
\input{线性代数/多项式/实数域上的不可约多项式}
\input{线性代数/多项式/有理数域上的不可约多项式}
\input{线性代数/多项式/多元多项式环}
\input{线性代数/多项式/对称多项式}
\input{线性代数/多项式/剩余类环}

\chapter{线性空间}
本章我们将建立一个数学模型 --- 线性空间.
我们将研究线性空间的结构.
它是研究客观世界中线性问题的一个重要理论.
即使对于非线性问题，
经过局部化后，
就可以运用线性空间的理论，
或者用线性空间的理论研究非线性问题的某一侧面.

\input{线性代数/线性空间/线性空间的结构}
\input{线性代数/线性空间/子空间}
\input{线性代数/线性空间/线性空间的同构}
\input{线性代数/线性空间/商空间}

\chapter{线性映射}
我们在上一章研究了域\(F\)上线性空间的结构.
在许多数学分支和实际问题中都会遇到线性空间之间的映射，
并且这种映射保持加法和纯量乘法两个运算，
称其为\emph{线性映射}.
线性代数就是研究线性空间和线性映射的理论.
这一章我们来研究线性映射的理论.

\input{线性代数/线性映射/线性映射}
\input{线性代数/线性映射/线性映射的核与像}
\input{线性代数/线性映射/线性映射的矩阵表示}
\input{线性代数/线性映射/线性函数与对偶空间}

\chapter{\texorpdfstring{$\lambda$}{\textlambda} - 矩阵}
\input{线性代数/lambda矩阵/lambda矩阵}

\chapter{线性变换的特征值与特征向量}
\begingroup
\NewDocumentCommand\LambdaExp{mo}{(\lambda-\lambda_{#1})\IfValueTF{#2}{^{#2}}{}}%
\NewDocumentCommand\EigenPoly{mmo}{(#1-\lambda_{#2}\vb{I})\IfValueTF{#3}{^{#3}}{}}%
\NewDocumentCommand\RestrictedEigenPoly{mmo}{\EigenPoly{#1 \SetRestrict W_{#2}}{#2}[#3]}%
\NewDocumentCommand\LambdaExpL{m}{\LambdaExp{#1}[l_{#1}]}%
\NewDocumentCommand\EpAj{o}{\EigenPoly{\vb{A}}{j}[#1]}%
\NewDocumentCommand\RepAj{o}{\RestrictedEigenPoly{\vb{A}}{j}[#1]}%
\input{线性代数/线性映射/线性变换的特征值与特征向量}
\input{线性代数/线性映射/线性变换的不变子空间}
\input{线性代数/线性映射/哈密顿凯莱定理}
\input{线性代数/线性映射/线性变换的最小多项式}
\input{线性代数/线性映射/幂零变换的结构}
\input{线性代数/线性映射/线性变换的若尔当标准型}
\endgroup

\chapter{内积空间}
迄今为止，我们对于线性空间和线性映射的研究
主要是围绕线性空间的加法、纯量乘法两种运算来进行的.
我们曾经对实数域上的\(n\)维向量空间\(\mathbb{R}^n\)定义了一个内积，
从而在\(\mathbb{R}^n\)中引进了长度、正交等度量概念.
受此启发，我们在本章讨论如何在实数域、复数域以至于任意域上的线性空间中引进度量概念，
然后研究具有度量的线性空间的结构，
并且研究与度量有关的线性变换的性质.

\input{线性代数/内积空间/双线性函数}
\input{线性代数/内积空间/欧几里得空间}
\input{线性代数/内积空间/正交补}
\input{线性代数/内积空间/正交变换}
\input{线性代数/内积空间/对称变换}
\input{线性代数/内积空间/酉空间}
\input{线性代数/内积空间/辛空间}

\chapter{赋范线性空间}
\input{线性代数/赋范线性空间/范数}

\chapter{仿射几何与射影几何}
\input{线性代数/仿射几何与射影几何/仿射空间}
\input{线性代数/仿射几何与射影几何/仿射几何}
\input{线性代数/仿射几何与射影几何/射影几何}
