...
\subsection{集合的概念}
\begin{definition}
我们将一些事物的\DefineConcept{收集}（collection）看成一个整体，%
将这个整体称为\DefineConcept{集合}（set），简称为\DefineConcept{集}.
组成这个集合的事物被称为该集合的\DefineConcept{元素}（member，或element），简称\DefineConcept{元}.
集合常用大写字母（如\(A,B,C,X,Y\)等）表示，元素常用小写字母（如\(a,b,c\)等）表示.

如果元素\(a\)是集合\(A\)的元素，%
就说“\(a\)是\(A\)的元素（\(a\) is a member of \(A\)）”，%
或“\(a\)\DefineConcept{属于} \(A\)（\(a\) belongs to \(A\)）”，%
记作\(a \in A\)\footnote{%
...
表记集合的方法大致有两类：
我们可以把集合的全体元素一一列举出来，用以表示集合，这种方法被称为\DefineConcept{列举法}.
例如由\(3\)个元素\(x,y,z\)组成的集合可以表记为\[
\{x,y,z\};
\]而由\(n\)个元素\(a_i\ (i=1,2,\dotsc,n)\)组成的集合可以表记为\[
\Set{ \v{a}{n} }.
\]
我们也可以通过说明集合中的元素都具有的性质，这种方法称为\DefineConcept{描述法}.
...
\begin{definition}
由不是集合的元素组成的集合，叫做\DefineConcept{第一层集}（first-order set）.
把第一层集当作元素组成的集合，叫做\DefineConcept{第二层集}（second-order set）或\DefineConcept{系}（system）.
\end{definition}
...
它们统称为\DefineConcept{集合存在公理}（Set Existence Axioms）.

\subsection{外延公理}
\begin{axiom}[外延公理]
%@see: 《Elements of Set Theory》 P17. Extensionality Axiom
如果集合\(A\)与集合\(B\)具有相同的元素，则称“集合\(A\)与集合\(B\)\DefineConcept{相等}”，记作\(A=B\)，即\[
...
\begin{definition}
称集合\(\Set{ A, B }\)为\(A\)和\(B\)的\DefineConcept{对集}（pair set）.
\end{definition}
...
\begin{definition}
设系\(A = \Set{\v{a}{n}}\).
称集合\(\v{a}{n}\)的并为\(A\)的\DefineConcept{并}，%
...
\begin{definition}
设系\(A = \Set{\v{a}{n}}\).
称集合\(\v{a}{n}\)的并为\(A\)的\DefineConcept{交}\footnote{%
...
\subsection{元组的概念}
我们可以把有序对的概念进一步扩展，形成一系列称为\(n\)\DefineConcept{元组}（n-tuple）的概念.
例如定义\[
\opair{x_1,x_2,x_3}
\defeq
...
\]称作“\(X\)与\(Y\)之间的\DefineConcept{二元关系}”；
当\(X = Y\)时，就称之为“\(X\)上的\DefineConcept{二元关系}”.
...
\]称作“关系\(\rel{R}\)的\DefineConcept{定义域}（domain）”，记作\(\dom \rel{R}\).

把集合\[
	\Set{ y \given \opair{x,y} \in \rel{R} }
\]称作“关系\(\rel{R}\)的\DefineConcept{值域}（range）”，记作\(\ran \rel{R}\).

把集合\[
	\dom \rel{R} \cup \ran \rel{R}
\]称作“关系\(\rel{R}\)的\DefineConcept{域}（field）”，记作\(\fld \rel{R}\).
\end{definition}
...
记作\(D_f\)，即\(D_f = X\)；
而\(X\)中所有元素的像所组成的集合称为“映射\(f\)的\DefineConcept{值域}”，%
