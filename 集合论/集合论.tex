...
再利用\hyperref[theorem:集合论.自然数的消去律]{消去律}便得\[
	m+s=n+r;
\]
于是\(\opair{m,n}\sim\opair{r,s}\)，\(\sim\)具有传递性.
综上所述，\(\sim\)是一个等价关系.
\end{proof}
\end{theorem}

我们把“‘差’的等价类”叫做\DefineConcept{整数}（integer）.
于是我们可以构造出“整数集”.
\begin{definition}
%@see: 《Elements of Set Theory》 P92. Definition
称“\(\omega\times\omega\)对\(\sim\)的商集”为\DefineConcept{整数集}，
记作\(\mathbb{Z}\)，即\[
	\mathbb{Z} \defeq (\omega\times\omega)/\kern-2pt\sim.
\]
\end{definition}
