...
\begin{enumerate}
	...
	\item 像的交是交的像的子集：\begin{gather}
		...
		\quad(A \neq \emptyset)
	\end{gather}
	当且仅当\(F\)是单根的，有：\begin{gather}
		F\ImageOfSetUnderRelation{A \cap B}
		= F\ImageOfSetUnderRelation{A} \cap F\ImageOfSetUnderRelation{B}, \\
		F\ImageOfSetUnderRelation{\bigcap A}
		= \bigcap\Set{ F\ImageOfSetUnderRelation{a} \given a \in A }.
		\quad(A \neq \emptyset)
	\end{gather}
\end{enumerate}
\end{theorem}
