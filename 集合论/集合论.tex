...
\subsection{等价关系，偏序关系}
\begin{definition}
设\(\rel{R}\)是集合\(A\)上的二元关系，即\(\rel{R} \subseteq A^2\).
\begin{itemize}
	\item 如果\(\rel{R}\)同时具有自反性、对称性、传递性，
	则称“\(\rel{R}\)是\(A\)上的\DefineConcept{等价关系}（equivalence relation）”.

	\item 如果\(\rel{R}\)同时具有自反性、反对称性、传递性，
...
如果\begin{enumerate}
	\item \(\rel{R}\)具有传递性，
	\item \(\rel{R}\)在\(A\)上满足\DefineConcept{三一律}（trichotomy），
