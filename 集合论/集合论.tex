...
我们可以用描述法把这个集合表示为\[
	a \defeq \Set{ x \given \varphi(x) },
\]其中\(\varphi(x)\)表示那个任意给定的性质，它是关于这个集合的每一个元素\(x\)的命题公式.
...
策梅洛与弗伦克尔提供了以下解决方案：
对于每个合式公式\(\varphi(a,a_1,\dotsc,a_n)\)，
我们可以引入符号\footnote{%
满足合式公式\(\varphi(x,a_1,\dotsc,a_n)\)的全部\(x\)的类.
}\[
	\Set{ x \given \varphi(x,a_1,\dotsc,a_n) }
\]
表示\DefineConcept{类}（class）.
