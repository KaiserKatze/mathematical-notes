...
\begin{align*}
	\dom A \times \ran A
	&= \Set{ \opair{x,y} \given x \in \dom A, y \in \ran A } \\
	&= \Set{ \opair{x,y} \given \exists u, \exists v: \opair{x,u},\opair{v,y} \in A }
\end{align*}
\end{example}
...
\begin{definition}
	...
	\begin{enumerate}
		...
		或\DefineConcept{一一映射}（one-to-one）”.
	\end{enumerate}
\end{definition}
...
如果\[
	...
	\opair{x,y} \in \rel{R},
\]
则称“关系\(\rel{R}\)是\DefineConcept{单根的}（single-rooted）”.
\end{definition}
...
\begin{enumerate}
	...
	\]为“关系\(F\)的\DefineConcept{逆}（inverse）”，
	记作\(F^{-1}\).
	特别地，如果\(F^{-1}\)是映射，
