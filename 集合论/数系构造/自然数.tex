...
设\(A\)是集合\(X\)的一个子集，
且\(A\)是归纳的，
则称“\(A\)是\(X\)的\DefineConcept{归纳子集}（inductive subset）”.
\end{definition}
...
如果集合\(x\)属于每一个归纳集，那么称其为\DefineConcept{自然数}（natural number），即\begin{equation*}
	\text{\(x\)是自然数}
	\defiff
...
%@see: 《Elements of Set Theory》 P69 Induction Principle for \(\omega\)
%@see: 《高等代数与解析几何（第三版 上册）》（孟道骥） P6 完全归纳原理
...
%@see: 《高等代数与解析几何（第三版 上册）》（孟道骥） P7 双重数学归纳法
进一步，我们还可以引出“双重数学归纳法”：
如果我们要证明形如“对于每个自然数\(m\)和\(n\)，涉及\(m\)和\(n\)的命题公式\(\lambda(m,n)\)成立”的命题，
我们可以构造一个集合\begin{equation*}
	T \defeq \Set{ \opair{m,n} \in \omega \times \omega \given \lambda(m,n) }.
\end{equation*}
我们只要首先证明集合\(
	T' \defeq \Set{ \opair{m,0} \given \lambda(m,0) }
\)是归纳的，
再证明对于任意给定自然数\(m\)，集合\(T\)也是归纳的，
就能证明“对于每个自然数\(m\)和\(n\)，涉及\(m\)和\(n\)的命题公式\(\lambda(m,n)\)成立”是真命题.
类似地，我们还可以有更多重的数学归纳法.
...
\begin{example}
%@see: 《Elements of Set Theory》 P72 Example
\def\a{\Set{\emptyset}}%
\def\b{\Set{\a}}%
...
\end{equation*}称为\DefineConcept{通项公式}，
把\begin{equation*}
	h(n^+) = F(h(n)) \quad(n\in\omega)
\end{equation*}称为\DefineConcept{递推公式}.
...
\begin{theorem}[三一律]\label{theorem:集合论.自然数集的三一律}
...
称为“自然数集\(\omega\)上的\DefineConcept{三一律}”.
...
%@see: 《高等代数与解析几何（第三版 上册）》（孟道骥） P6 定理0.2.1
设\(A\)是\(\omega\)的一个非空子集.
...
%@see: 《高等代数与解析几何（第三版 上册）》（孟道骥） P6
这就引出“第二数学归纳法”的方法论，即：
为了证明一个与自然数有关的性质\(\lambda(n)\)对所有自然数成立，
我们只要
	首先证明\(\lambda(0)\)成立，
	然后在归纳假设（对于每个小于\(n\)的自然数\(k\)，总有\(\lambda(k)\)）成立的情况下
		证明\(\lambda(n)\)成立，
就可以证明\(\lambda(n)\)对所有自然数都成立.

%@see: 《高等代数与解析几何（第三版 上册）》（孟道骥） P11 习题 6.
% \DefineConcept{大跨度数学归纳法}

%@see: 《高等代数与解析几何（第三版 上册）》（孟道骥） P11 习题 7.
% \DefineConcept{反向数学归纳法}

%@see: 《高等代数与解析几何（第三版 上册）》（孟道骥） P11 习题 10.
% \DefineConcept{跷跷板数学归纳法}

%@see: 《高等代数与解析几何（第三版 上册）》（孟道骥） P12 习题 12.
% \DefineConcept{螺旋式数学归纳法}
