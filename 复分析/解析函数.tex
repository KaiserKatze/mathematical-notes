...
\]存在，且极限值为有限复数\(v_0\ (\abs{v_0}<+\infty)\)，则称函数\(w=f(z)\)在点\(z_0\)\DefineConcept{可导}（或\DefineConcept{可微}），极限值称为\(f(z)\)在点\(z_0\)的\DefineConcept{导数}（或\DefineConcept{微商}），记作\(f'(z_0)\)，或\(\eval{\dv{f}{z}}_{z=z_0}\)，或\(\eval{\dv{w}{z}}_{z=z_0}\)，即\begin{equation}\label{equation:解析函数.导数的定义式}
