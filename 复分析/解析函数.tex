...
又假设，经变换\(w = f(z)\)之后，\(C\)的象曲线\begin{equation*}
	\Gamma = f\ImageOfSetUnderRelation{C}: w = f[z(t)]
	\quad(t_0 \leq t \leq t_1)
\end{equation*}在点\(z_0\)的象点\(w_0 = f(z_0) = f[z(t_0)]\)的邻域内是光滑的；
象曲线\(\Gamma\)在\(w_0\)点的切向量\(w'(t_0)\)存在且不为零
（即\(w'(t_0) = f'(z_0) z'(t_0) \neq 0\)）.
象曲线的切向量\(w'(t_0)\)与\(w\)平面实轴间的夹角为\begin{equation*}
...
事实上，设\(C_1\)与\(C_2\)在点\(z_0\)处的正向切线倾角分别为\(\psi_1\)与\(\psi_2\)，\(C_1\)和\(C_2\)在变换\(w = f(z)\)下的象曲线\(\Gamma_1\)和\(\Gamma_2\)在点\(w_0 = f(z_0)\)的切线倾角分别为\(\Psi_1\)与\(\Psi_2\).则由\cref{equation:解析函数.导函数的辐角的几何意义} 有\begin{equation*}
\Psi_1 - \psi_1 = \arg f'(z_0) = \Psi_2 - \psi_2,
\end{equation*}整理得\begin{equation*}
\Psi_2 - \Psi_1 = \psi_2 - \psi_1.
\end{equation*}上式左端是象曲线\(\Gamma_1\)与\(\Gamma_2\)之间的夹角，右端是原象曲线\(C_1\)和\(C_2\)之间的夹角，它们相等表示夹角大小经变换后保持不变.
...
设原象曲线是射线\(C: \arg z = \theta_0\)，那么\(z\)平面上的动点\(z = r e^{\iu\theta_0}\)在\(w\)平面上对应的是双曲线\begin{equation*}
\Gamma: \left\{ \def\arraystretch{1.5} \begin{array}{l}
