...
\begin{definition}
设函数\(f\)在点\(z_0\)的某个邻域\(N(z_0)\)内有定义，且\(z=z_0+\increment z \in N(z_0)\)，若极限\[
...
\]也就是说，函数\(f\)只在点\((0,0)\)和\(\opair*{\frac{3}{4},\frac{3}{4}}\)满足C-R条件，也只在这两点可微.
...
\]显然有\(u_y' = 0 = -v_x'\).但要使\(u_x' = 2x = -1 = v_y'\)，必有\(x = -1/2\)，即仅在直线\(x=-1/2\)上满足C-R条件，从而\(f\)仅在直线\(x=-1/2\)上可微，在\(z\)平面上处处不解析.
\end{solution}
\end{example}
