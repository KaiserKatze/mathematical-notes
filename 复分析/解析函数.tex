...
\begin{definition}
设函数\(f\)在点\(z_0\)的某个邻域\(N(z_0)\)内有定义，且\(z=z_0+\increment z \in N(z_0)\)，若极限\begin{equation*}
\lim_{z \to z_0} \frac{f(z)-f(z_0)}{z-z_0}
\quad\text{或}\quad
\lim_{\increment z\to0} \frac{f(z_0+\increment z)-f(z_0)}{\increment z}
\end{equation*}存在，且极限值为有限复数\(v_0\ (\abs{v_0}<+\infty)\)，
则称函数\(f\)在点\(z_0\) \DefineConcept{可导}（或\DefineConcept{可微}），
极限值称为\(f\)在点\(z_0\)的\DefineConcept{导数}（或\DefineConcept{微商}），
记作\(f'(z_0)\)，
...
若\(f\)在区域\(D\)内任一点均可微，即\begin{equation*}
...
\right],
\end{equation*}则称\(f\)在区域\(D\)内可微.
\end{definition}

由极限的唯一性，可知：
\begin{theorem}
若\(f\)在区域\(D\)内可微，则\(f\)在\(D\)内的导数\(f'(z)\)仍是在\(D\)内有定义的一个单值函数.
\end{theorem}

\begin{theorem}[可微的必要条件]
若\(f\)在点\(z\)可微，则\(f\)在点\(z\)必定连续.
\begin{proof}
固定点\(z\)，令\begin{equation*}
\alpha = \frac{f(z+\increment z)-f(z)}{\increment z} - f'(z).
\end{equation*}显然\(\alpha\)是\(\increment z \to 0\)时的无穷小.因为\(\abs{\alpha \increment z}\)是\(\increment z \to 0\)时较\(\abs{\increment z}\)高阶的无穷小，且\(f'(z)\)在定点\(z\)处是一个有限常数，以及\(f(z+\increment z) - f(z) = f'(z) \increment z + \alpha \increment z\)，所以\begin{equation*}
...
如果\(u=g(z)\)在点\(z\)可导，而\(w=f(u)\)在点\(u=g(z)\)可导，则复合函数\(w=f[g(z)]\)在点\(z\)可导，且其导数为\begin{equation*}
...
\end{gather}
\begin{proof}
首先证明\((C)' = 0\).记\(f(z) = C\)，则根据\cref{equation:解析函数.导数的定义式} 有\begin{equation*}
f'(z_0) = \lim_{z \to z_0} \frac{f(z)-f(z_0)}{z-z_0}
= \lim_{z \to z_0} \frac{C-C}{z-z_0} = 0.
\end{equation*}

再证明\((z^n)' = n z^{n-1}\ (n\in\mathbb{N})\).利用牛顿二项式可以得到\begin{equation*}
(z_0 + \increment z)^n = z_0^n + n z_0^{n-1} \increment z + \dotsb + (\increment z)^n,
\end{equation*}再根据\cref{equation:解析函数.导数的定义式} 和\cref{theorem:解析函数.函数的和差积商的导数} 有\begin{align*}
...
\begin{solution}
记\(u = 1\)，\(v = z\)，则由\cref{theorem:解析函数.函数的和差积商的导数} 和\cref{theorem:解析函数.基本导数} 有\begin{equation*}
...
\begin{proof}
对\(z\)平面上任意取定一点\(z=x+\iu y\)，\begin{equation*}
...
= 0,
\end{equation*}所以，\(f\)在点\(z\)连续.

但当\(\increment z\)沿实轴方向（即\(\increment z = \increment x\)，\(\increment y = 0\)）趋于零时，\begin{equation*}
...
\end{equation*}当\(\increment z\)沿虚轴方向（即\(\increment z = \iu \increment y\)，\(\increment x = 0\)）趋于零时，\begin{equation*}
...
\end{equation*}故\(f\)在点\(z\)处不可微.

综上所述，\(f\)在\(z\)平面上处处连续但处处不可微.
\end{proof}
\end{example}
...
= 0,
\end{equation*}故\(f\)在点\(z = 0\)可导，\(f'(0) = 0\).

再任取\(z\)平面上一点\(z_0 = x_0 + \iu y_0 \neq 0\).
当\(z = x + \iu y\)沿路径\(y = y_0\)趋于\(z_0\)时，\begin{equation*}
\lim_{z \to z_0} \frac{f(z) - f(z_0)}{z - z_0}
= \lim_{x \to x_0} \eval{\frac{(x^3 - \iu y^3) - (x_0^3 - \iu y_0^3)}{(x + \iu y) - (x_0 + \iu y_0)}}_{y = y_0}
= \lim_{x \to x_0} \frac{x^3 - x_0^3}{x - x_0}
= 3 x_0^3;
\end{equation*}而当\(z = x + \iu y\)沿路径\(x = x_0\)趋于\(z_0\)时，\begin{equation*}
\lim_{z \to z_0} \frac{f(z) - f(z_0)}{z - z_0}
= \lim_{y \to y_0} \frac{-\iu y^3 + \iu y_0^3}{\iu (y - y_0)}
= -3 y_0^3.
\end{equation*}因此\(f\)在非零点\(z\)处不可导.
\end{proof}
\end{example}

\subsection{复函数可微与实部、虚部函数可微的联系\ 柯西--黎曼条件}
我们知道，复函数\(f(z)=u(x,y)+\iu v(x,y)\)在点\(z=x+\iu y\)连续，等价于二元实函数\(u(x,y)\)和\(v(x,y)\)在点\((x,y)\)连续.但是，\(f\)在点\(z\)可微却不等价于\(u(x,y)\)和\(v(x,y)\)在点\((x,y)\)可微.

\begin{theorem}\label{theorem:解析函数.柯西--黎曼条件}
函数\(f(z)=u(x,y)+\iu v(x,y)\)在点\(z=x+\iu y\)可微的充分必要条件为：
\begin{enumerate}
\item 二元实函数\(u(x,y)\)和\(v(x,y)\)在点\((x,y)\)可微；
\item \(u(x,y)\)和\(v(x,y)\)在点\((x,y)\)满足\DefineConcept{柯西--黎曼条件}（简称C-R条件），即\begin{equation*}
...
若函数\(f\)在点\(z\)处可微，则函数\(f\)的导数为\begin{equation}\label{equation:解析函数.导数的计算式}
f'(z)
...
先证必要性.设\(f\)在点\(z\)可微，即\begin{equation*}
f(z+\increment z) - f(z) = f'(z) \increment z + \odz,
\end{equation*}其中\(\odz\)是当\(\increment z\to0\)时的无穷小.
...
\end{align*}当\(\increment z\to0\)时，\(\beta_1\)和\(\beta_2\)是\(\abs{\increment z} = \sqrt{\increment x^2 + \increment y^2}\)的高阶无穷小.比较可得\begin{equation*}
\increment u = a \increment x - b \increment y + \beta_1,
\qquad
\increment v = b \increment x + a \increment y + \beta_2,
\end{equation*}这也意味着\(u(x,y)\)和\(v(x,y)\)在点\((x,y)\)可微，且有\begin{equation*}
a = \pdv{u}{x} = \pdv{v}{y},
\qquad
-b = \pdv{u}{y} = -\pdv{v}{x}.
\end{equation*}

再证充分性.设\(u(x,y)\)和\(v(x,y)\)在点\((x,y)\)可微且满足C-R条件，则\begin{equation*}
\increment u = \pdv{u}{x} \increment x + \pdv{u}{y} \increment y + \beta_1,
\qquad
\increment v = \pdv{v}{x} \increment x + \pdv{v}{y} \increment y + \beta_2,
\end{equation*}其中当\(\increment z\to0\)时，\(\beta_1\)和\(\beta_2\)是\(\abs{\increment z} = \sqrt{\increment x^2 + \increment y^2}\)的高阶无穷小.又令\begin{equation*}
\pdv{u}{x} = \pdv{v}{y} = a,
\qquad
\pdv{u}{y} = -\pdv{v}{x} = -b,
\end{equation*}则有\begin{align*}
\increment f(z) &= \increment u + \iu \increment v \\
&= a \increment x - b \increment y + \beta_1 + \iu (b \increment x + a \increment y + \beta_2) \\
&= (a+\iu b)(\increment x + \iu \increment y) + \beta_1 + \iu \beta_2,
\end{align*}或\begin{equation*}
\frac{\increment f}{\increment z} = a + \iu b + \beta,
\end{equation*}其中\(\beta = \frac{\beta_1 + \iu \beta_2}{\increment z}\).
因为\begin{equation*}
\abs{\beta} \leq
...
\begin{solution}
记\(f(z) = u(x,y) + \iu v(x,y)\)，显然\(u(x,y) = \sqrt{\abs{xy}}\)，\(v(x,y) = 0\)，\begin{equation*}
u'_x(0,0) = u'_y(0,0) = v'_x(0,0) = v'_y(0,0) = 0,
\end{equation*}也就是说\(u'_x,u'_y,v'_x,v'_y\)在点\((x,y)\)处存在，且满足C-R条件.
但差商\begin{equation*}
\frac{f(\increment z) - f(0)}{\increment z}
= \frac{ \sqrt{ \abs{ \increment x \increment y } } }{ \increment x + \iu \increment y }
\end{equation*}在\(\increment z \to 0\)时的极限不存在.
...
显然实部、虚部函数的一阶偏导数\begin{equation*}
u'_x = 3x^2,
\qquad
u'_y = -3y^2,
\qquad
...
\qquad
v'_y = 4x^2y
\end{equation*}都在\(z\)平面上连续.依据C-R条件联立方程组\begin{equation*}
\left\{ \begin{array}{*7r}
...
\end{equation*}也就是说，函数\(f\)只在点\((0,0)\)和\(\opair*{\frac{3}{4},\frac{3}{4}}\)满足C-R条件，也只在这两点可微.

接下来，根据\cref{equation:解析函数.导数的计算式} 可以计算得到\begin{equation*}
...
\begin{solution}
由二元初等实函数的可微性，\(u = e^x \cos y\)和\(v = e^x \sin y\)都是全平面上的可微函数，且C-R条件\begin{equation*}
\left\{ \begin{array}{*5r}
u'_x &=& e^x \cos y &=& v'_y, \\
u'_y &=& - e^x \sin y &=& - v'_x
\end{array} \right.
\end{equation*}在全平面上处处成立，故\(f\)在全平面可微，且\begin{equation*}
...
若函数\(f\)在点\(z_0\)的一个邻域\(N(z_0)\)内处处可微，
则称“\(f\)在点\(z_0\) \DefineConcept{解析}（analytic）”，
或称“点\(z_0\)是\(f\)的\DefineConcept{解析点}”.

若函数\(f\)在区域\(D\)内处处可微，
则称“\(f\)在区域\(D\)内\DefineConcept{解析}”，
或称“\(f\)是区域\(D\)内的\DefineConcept{解析函数}（analytic function）”.
%@see: https://mathworld.wolfram.com/AnalyticFunction.html

若函数\(f\)在点\(z_0\)不解析，
但在点\(z_0\)的任意邻域内存在\(f\)的解析点，
则称“点\(z_0\)为\(f\)的\DefineConcept{奇点}”.
\end{definition}
以后当我们说函数\(f\)在闭区域\(\overline{D}\)上解析时，
其意义是指\(f\)在某个包含\(\overline{D}\)的开区域内解析.

从\cref{definition:解析函数.解析函数的定义} 可以看出，
“函数\(f\)在区域\(D\)内解析”与“函数\(f\)在区域\(D\)内处处可微”等价.
但“函数\(f\)在点\(z_0\)解析”却只是“函数\(f\)在点\(z_0\)可微”的充分不必要条件.
...
现在就可以依据\cref{theorem:解析函数.函数在区域内处处解析的充分必要条件,theorem:解析函数.函数的和差积商的解析性,theorem:解析函数.复合函数的解析性} 判定函数\(f\)是否在区域\(D\)内解析.

\begin{example}
多项式函数\begin{equation*}
p(z) = a_0 z^n + a_1 z^{n-1} + \dotsb + a_n \quad (a_0 \neq 0)
\end{equation*}在\(z\)平面解析.
\end{example}

\begin{example}
有理分式函数\begin{equation*}
\frac{p(z)}{q(z)} = \frac{a_0 z^n + a_1 z^{n-1} + \dotsb + a_n}{b_0 z^m + b_1 z^{m-1} + \dotsb + b_m} \quad (a_0 \neq 0,\ b_0 \neq 0)
\end{equation*}在\(z\)平面上除使分母\(q(z)=0\)的各点外解析.
\end{example}
...
\end{equation*}显然有\(u_y' = 0 = -v_x'\).但要使\(u_x' = 2x = -1 = v_y'\)，必有\(x = -1/2\)，即仅在直线\(x=-1/2\)上满足C-R条件，从而\(f\)仅在直线\(x=-1/2\)上可微，在\(z\)平面上处处不解析.
\end{solution}
\end{example}
...
\end{equation*}要满足C-R条件，必有\(u_x' = 2x = 0 = v_y' \land u_y' = 2y = 0 = -v_y'\)，即\(x=y=0\).
也就是说仅在原点0处满足C-R条件，从而\(f\)仅在原点可微，在\(z\)平面上处处不解析.
\end{solution}
\end{example}

\subsection{解析函数的性质}
如果函数\(f\)在点\(z_0\)处解析，则\(f\)在点\(z_0\)处无穷次可微，其各阶导函数\(f^{(n)}(z)\ (n=1,2,\dotsc)\)也在点\(z_0\)处解析，\(f\)可以在点\(z_0\)的邻域内展开成幂级数.
...
过点\(z_0\)任意作一条有向光滑曲线\begin{equation*}
	C: z = z(t)
	\quad(t_0 \leq t \leq t_1),
\end{equation*}
并假设\(C\)在点\(z_0 = z(t_0)\)的切向量\(z'(t_0)\)存在且不为零，
显然原像曲线的切向量与\(z\)平面实轴间的夹角（也称为切线倾角）为\(\arg z'(t_0)\).
又假设，经变换\(w = f(z)\)之后，\(C\)的像曲线\begin{equation*}
	\Gamma = f\ImageOfSetUnderRelation{C}: w = f[z(t)]
	\quad(t_0 \leq t \leq t_1)
\end{equation*}在点\(z_0\)的像点\(w_0 = f(z_0) = f[z(t_0)]\)的邻域内是光滑的；
像曲线\(\Gamma\)在\(w_0\)点的切向量\(w'(t_0)\)存在且不为零
（即\(w'(t_0) = f'(z_0) z'(t_0) \neq 0\)）.
像曲线的切向量\(w'(t_0)\)与\(w\)平面实轴间的夹角为\begin{equation*}
...
无论过点\(z_0\)作怎样的光滑曲线，它在点\(z_0\)的正向切线与像曲线在点\(w_0\)的正向切线之间的夹角总是\(\arg f'(z_0)\)；
或者说，像曲线过点\(w_0\)的切线相比于原像曲线过点\(z_0\)的切线，总是绕点\(z_0\)旋转了一个角度\(\arg f'(z_0)\).
变换\(w = f(z)\)取定之后，角\(\arg f'(z_0)\)仅与点\(z_0\)有关，而与原像曲线\(C\)无关，因此称\(\arg f'(z_0)\)为变换\(w = f(z)\)在点\(z_0\)的\DefineConcept{旋转角}.
这就是导数辐角的几何意义.
“变换\(w = f(z)\)取定之后，旋转角\(\arg f'(z_0)\)仅与点\(z_0\)有关，而与过点\(z_0\)的曲线\(C\)的无关”这个特性，通常也称为“旋转角的不变性”.

下面再看导数的模的几何意义.根据\hyperref[equation:解析函数.导数的定义式]{导数的定义}有\begin{equation*}
f'(z_0) = \lim_{z \to z_0} \frac{f(z) - f(z_0)}{z - z_0},
\end{equation*}又因为\(f'(z)\)是连续的，根据\cref{theorem:复变函数.连续函数的性质}  2，函数\(\abs{f'(z)}\)也是连续的，所以\begin{equation*}
\abs{f'(z_0)}
= \abs{ \lim_{z \to z_0} \frac{f(z) - f(z_0)}{z - z_0} }
= \lim_{z \to z_0} \abs{ \frac{f(z) - f(z_0)}{z - z_0} }.
\end{equation*}因此\begin{equation}\label{equation:解析函数.导函数的模的几何意义}
\abs{f'(z_0)}
= \lim_{z \to z_0} \frac{\abs{f(z) - f(z_0)}}{\abs{z - z_0}}.
\end{equation}
由于曲线\(C\)是过点\(z_0\)任意作的一条光滑曲线，所以\cref{equation:解析函数.导函数的模的几何意义} 表明：
像点之间的距离\(\abs{w - w_0}\)与原像之间的距离\(\abs{z - z_0}\)之比的极限为\(\abs{f'(z_0)}\)，且此极限值仅与点\(z_0\)的选择有关，而与曲线\(C\)的选择无关，也与点\(z\)沿曲线\(C\)从什么方向逼近点\(z_0\)无关.
...
事实上，设\(C_1\)与\(C_2\)在点\(z_0\)处的正向切线倾角分别为\(\psi_1\)与\(\psi_2\)，\(C_1\)和\(C_2\)在变换\(w = f(z)\)下的像曲线\(\Gamma_1\)和\(\Gamma_2\)在点\(w_0 = f(z_0)\)的切线倾角分别为\(\Psi_1\)与\(\Psi_2\).则由\cref{equation:解析函数.导函数的辐角的几何意义} 有\begin{equation*}
\Psi_1 - \psi_1 = \arg f'(z_0) = \Psi_2 - \psi_2,
\end{equation*}整理得\begin{equation*}
\Psi_2 - \Psi_1 = \psi_2 - \psi_1.
\end{equation*}上式左端是像曲线\(\Gamma_1\)与\(\Gamma_2\)之间的夹角，右端是原像曲线\(C_1\)和\(C_2\)之间的夹角，它们相等表示夹角大小经变换后保持不变.
...
因此，若\(w = f(z)\)在点\(z_0\)处解析，则\(f\)在点\(z_0\)的邻域内连续.
...
\item 反函数\(z = f^{-1}(w)\)在区域\(G\)内单叶解析，且\begin{equation*}
...
这时，\(D\)内任一点\(z_0\)邻域内的任一小三角形，通常会变换为\(G\)内的像邻域内的一个小曲边三角形，这两个三角形的对应角相等，对应边长近似成比例，因此，它们是近似的相似形.
这就是保形变换的名称的由来.
...
\item \(e^z\)是以\(2\pi\iu\)为基本周期的周期函数，即\begin{equation*}
...
\end{equation*}解得\(z_1 = z_2\)或\(z_1 z_2 = 1\).
...
\end{equation*}因为\(e^{\iu\theta} = \cos\theta + \iu \sin\theta\)，所以\begin{align*}
...
\end{align*}故\begin{equation*}
u = \frac{1}{2} \cos\theta \left( r + \frac{1}{r} \right),
\qquad
v = \frac{1}{2} \sin\theta \left( r - \frac{1}{r} \right).
\end{equation*}由此可见，\(z\)平面上每个圆周\(C: \abs{z} = r_0 > 0\)都会变成\(w\)平面上的一个椭圆\begin{equation*}
...
\end{array} \right.
\end{equation*}椭圆\(\Gamma\)的半长轴、半短轴分别是\begin{equation*}
...
设原像曲线是射线\(C: \arg z = \theta_0\)，那么\(z\)平面上的动点\(z = r e^{\iu\theta_0}\)在\(w\)平面上对应的是双曲线\begin{equation*}
\Gamma: \left\{ \def\arraystretch{1.5} \begin{array}{l}
u = \frac{1}{2} \left( r + \frac{1}{r} \right) \cos\theta_0, \\
v = \frac{1}{2} \left( r - \frac{1}{r} \right) \sin\theta_0
\end{array} \right.
\end{equation*}上的点.
当\(\theta_0=0,2\pi\)时，像曲线是射线\(\Set{ w = u + \iu v \given u\geq1, v=0 }\)（分别称为射线的“上岸”和“下岸”）；
当\(\theta_0=\pi\)时，像曲线是射线\(\Set{ w = u + \iu v \given u\leq-1, v=0 }\)；
当\(\theta_0=\frac{1}{2}\pi,\frac{3}{2}\pi\)时，像曲线是\(w\)平面上的虚轴.
除这些特殊的\(\theta_0\)值外，对于不同的\(\theta_0\)，它们所对应的双曲线是共焦的，焦点都是\(\pm1\).
总之，茹科夫斯基函数将\(z\)平面上去掉原点的单位圆内部区域和单位圆外部区域都分别地保形变换为去掉线段\([-1,1]\)后的\(w\)平面.

如果把上半\(z\)平面或下半\(z\)平面作为茹科夫斯基函数的单叶性区域，与上面的讨论相仿，茹科夫斯基函数把上半\(z\)平面\(\Im z > 0\)和下半\(z\)平面\(\Im z < 0\)都分别保形地变换成去掉实轴上射线\begin{equation*}
\Set{ \mathbb{C} \ni w = u + \iu v \given -\infty < u \leq -1 }
\quad\text{和}\quad
\Set{ \mathbb{C} \ni w = u + \iu v \given 1 \leq u < +\infty }
\end{equation*}后的整个\(w\)平面.

\begin{definition}
定义机翼剖面函数\begin{equation*}
...
复三角函数具有以下性质：
\begin{enumerate}
\item \(\sin z\)和\(\cos z\)都以\(2\pi\)为基本周期，即\begin{equation*}
\sin(z+2\pi) = \sin z, \qquad
\cos(z+2\pi) = \cos z;
\end{equation*}
\item \(\sin z\)是奇函数，\(\cos z\)是偶函数，即\begin{equation*}
\sin(-z) = -\sin z, \qquad
\cos(-z) = \cos z;
\end{equation*}
...
余弦函数是由\begin{equation*}
\xi = \iu z,
\qquad
\zeta = e^{\xi},
\qquad
w = \frac{1}{2} \left( \zeta + \frac{1}{\zeta} \right)
\end{equation*}三个函数复合而成.
...
\begin{property}
正切函数\(\tan z\)和余切函数\(\cot z\)都以\(\pi\)为基本周期，即\begin{equation*}
\tan(z+\pi) = \tan z,
\qquad
\cot(z+\pi) = \cot z.
\end{equation*}

正割函数\(\sec z\)和余割函数\(\csc z\)都以\(2\pi\)为基本周期，即\begin{equation*}
\sec(z+2\pi) = \sec z,
\qquad
\csc(z+2\pi) = \csc z.
\end{equation*}
...
\subsection{反三角函数}
\begin{definition}
定义复反三角函数为复三角函数的反函数.对任意复变量\(z\)，称满足方程\begin{equation*}
z = \sin w
\end{equation*}的复数\(w\)为复数\(z\)的反正弦函数，记作\(w = \Arcsin z\)；称满足方程\begin{equation*}
z = \cos w
\end{equation*}的复数\(w\)为复数\(z\)的反余弦函数，记作\(w = \Arccos z\).
\end{definition}

\begin{property}
复反正弦函数满足\begin{equation*}
w = \Arcsin z = -\iu \Ln(\iu z + \sqrt{1-z^2}).
\end{equation*}这里，根式理解为双值函数，故不必像实根式一样在前面添加“\(\pm\)”号.
\end{property}
...
\subsection{对数函数}
对数函数是作为指数函数的反函数来定义的，
即对任意非零有限复数\(z\ (z \neq 0 \land z \neq \infty)\)，称满足方程\begin{equation*}
	e^w = z
\end{equation*}的复数\(w\)为复数\(z\)的对数，记作\(w = \Ln z\).

显然，由于指数函数是周期函数，周期函数是多叶函数，多叶函数的反函数是多值函数，故\(\Ln z\)是（无穷）多值函数.
若设\(z = r e^{\iu\theta}\)，\(w = u + \iu v\)，则由\(e^w = z\)可得\begin{equation*}
e^{u + \iu v} = r e^{\iu\theta},
\end{equation*}因而\begin{equation*}
\left\{ \begin{array}{l}
u = \ln r, \\
v = \theta+2k\pi
\end{array} \right.
...
\quad(k\in\mathbb{Z}),
\end{equation*}即\begin{equation}\label{equation:解析函数.对数函数与辐角的关系式}
...
\end{equation}则对数函数的一般值是\begin{equation*}
\Ln z = \ln z + 2k\pi\iu
\quad(k\in\mathbb{Z}).
\end{equation*}
...
\begin{example}
设\(a > 0\)，则\begin{equation*}
\Ln a = \ln a + 2k\pi\iu \quad(k\in\mathbb{Z}).
\end{equation*}同时\begin{equation*}
\Ln(-a) = \ln a + (2k+1)\pi\iu \quad(k\in\mathbb{Z}),
\end{equation*}\begin{equation*}
\ln(-a) = \ln a + \pi\iu.
\end{equation*}特别地，\begin{equation*}
\ln(-1) = \pi\iu,
\end{equation*}\begin{equation*}
...
下面讨论\(w = \Ln z\)的单叶性区域.
根据前面对指数函数的讨论，指数函数\(z = e^w\)在宽为\(2\pi\)的水平带状区域\begin{equation*}
A_k = \Set{ \mathbb{C} \ni w = u + \iu v \given (2k-1)\pi < v < (2k+1)\pi }
\quad(k\in\mathbb{Z})
\end{equation*}内单叶解析，因而是保形的.
它把\(w\)平面上所有水平带状区域的集合\(\{A_1,A_2,\dotsc\}\)中的每一个都保形地变成\(z\)平面去掉原点和负实轴后的区域\begin{equation*}
...
\subsection{幂函数}
\begin{definition}
设\(\alpha\)是任意给定的复数，对复变量\(z \neq 0\)，定义\(z\)的\(\alpha\)次幂为\begin{equation*}
w = z^\alpha = e^{\alpha \Ln z}.
\end{equation*}

记\(w_0 = e^{\alpha \ln z}\).
若\(\ln z\)为\(\Ln z\)的主值（支），即\(\ln z = \ln\abs{z} + \iu \arg z\)，则称\(w_0\)为\(z^\alpha\)的主值（支），继而有\begin{equation*}
...
\item 当\(\alpha=\frac{p}{q}\in\mathbb{Q}-\mathbb{Z}\)时，由于\(\frac{p}{q} \cdot 2 k \pi\)仅当\(k=0,1,\dots,q-1\)时的\(q\)个值关于\(2\pi\)是不同余的；\(k\)取其他值时，其相应的值\(\frac{p}{q} \cdot 2 k \pi\)均与上述\(q\)个值中的一个关于\(2\pi\)是同余的.所以这时\(z^\alpha\)是有限多值函数.特别地，当\(\alpha=\frac{1}{n}\ (n\in\mathbb{N}^+)\)时，有\begin{equation*}
z^{1/n} = \abs{z}^{1/n} \exp(\iu\frac{\arg z + 2 k \pi}{n}), \quad k=0,1,\dotsc,n-1,
\end{equation*}由复数的开方公式可知，它是整幂函数\(z^n\)的反函数---根式函数\(\sqrt[n]{z}\)，这是一个\(n\)值函数；
...
\end{equation*}称为\DefineConcept{分式线性函数}.

当\(c \neq 0\)时，定义\begin{equation*}
L\left(-\frac{d}{c}\right) = \infty,
\end{equation*}\begin{equation*}
L(\infty) = \frac{a}{c}.
\end{equation*}

当\(c = 0\)时，定义\begin{equation*}
L(\infty) = \infty.
\end{equation*}
\end{definition}
