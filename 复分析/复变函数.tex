...
\subsubsection{简单曲线}
\begin{definition}
设\(x(t)\)和\(y(t)\)是实变数\(t\)的两个实函数，且在闭区间\([\alpha,\beta]\)上连续，则由方程组\begin{equation*}
\left\{ \begin{array}{l}
x = x(t) \\
y = y(t)
\end{array} \right., \quad t \in [\alpha,\beta]
\end{equation*}所确定的点集\(C = \{ z \mid z = x + \iu y \}\)称为复平面上的一条连续曲线.上式称为曲线\(C\)的参数方程.
...
\begin{definition}
设简单（闭）曲线\(C\)的参数方程为\begin{equation*}
z = x(t) + \iu y(t), \quad t \in [\alpha,\beta].
\end{equation*}若在\([\alpha,\beta]\)上，\(x'(t)\)与\(y'(t)\)存在、连续且不全为零，则称曲线\(C\)为\DefineConcept{光滑曲线}.由有限条光滑曲线衔接而成的连续曲线称为\DefineConcept{逐段光滑曲线}.
\end{definition}
...
\item 三个点集彼此不交，即\begin{equation*}
C \cap I(C) = \emptyset,
\qquad
C \cap E(C) = \emptyset,
\qquad
...
\subsubsection{序列的极限}
\begin{definition}
若\(\lim_{n\to\infty} \abs{z_n - z_0} = 0\)，即\begin{equation*}
\forall \epsilon > 0, \exists N \in \mathbb{N}^+ \bigl(
n > N \implies \abs{z_n - z_0} < \epsilon
\bigr),
\end{equation*}则称\(\{z_n\}\)为收敛序列，\(z_0\)为序列\(\{z_n\}\)的极限，也称序列\(\{z_n\}\)收敛于\(z_0\)，又记作\begin{equation*}
\lim_{n\to\infty} z_n = z_0.
\end{equation*}
\end{definition}

\begin{definition}
若序列\(\{z_n\}\)满足\begin{equation*}
\forall \epsilon > 0, \exists N \in \mathbb{N}^+, \forall p\in\mathbb{N}^+ \bigl(
n > N  \implies  \abs{z_{n+p} - z_n} < \epsilon
\bigr),
\end{equation*}则称\(\{z_n\}\)为\DefineConcept{基本序列}或\DefineConcept{Cauchy序列}.
\end{definition}
...
\item \(\lim_{n\to\infty} x_n = x_0 \land \lim_{n\to\infty} y_n = y_0\)，其中\begin{equation*}
...
\begin{definition}
在包含无穷远点\(\infty\)的闭平面\(C_\infty\)上，任一圆周的外部（包含点\(\infty\)）皆可称为无穷远点的邻域.将有限点\(a\in\mathbb{C}\)和有限数\(r\in(0,+\infty)\)确定的点集\begin{equation*}
\Set*{ z\in\mathbb{C} \given \abs{z-a}>r }
\end{equation*}可称为\DefineConcept{以\(a\)为中心的无穷远点的邻域}.

特别地，以原点为中心的无穷远点邻域\begin{equation*}
\Set*{ z \given \abs{z} > \frac{1}{\epsilon} }
\end{equation*}常称为\(\infty\)的\(\epsilon\)邻域，记作\(N_{\epsilon}(\infty)\).
\end{definition}
...
\end{theorem}
这样一来，不论序列\(\{z_n\}\)是否有界，也不论\(z_0\)是有限点或无穷远点，在闭平面上\begin{equation*}
\lim_{n\to\infty} z_n = z_0
\end{equation*}的充分必要条件是：\(z_0\)是\(\{z_n\}\)的唯一聚点.
...
对于\(\forall z_1,z_2 \in E\)总有\begin{equation*}
	z_1 \neq z_2
	\implies
	f(z_1) \neq f(z_2),
\end{equation*}
...
\begin{property}
已知复变函数\(w=f(z)\)的定义域、值域分别为\(E\)、\(M\).如果\begin{equation*}
\forall w \in M: w=f[f^{-1}(w)]
\end{equation*}且当反函数\(z=f^{-1}(w)\)也是单值函数时，还有\begin{equation*}
...
\begin{definition}
如果函数\(w = F(\zeta)\ (\zeta \in M)\)和\(\zeta = f(z)\ (z \in E)\)，且\(\ran f \subseteq M\)，则函数\begin{equation*}
w = F[f(z)] \quad (z \in E)
\end{equation*}称为由函数\(w = F(\zeta)\)与函数\(\zeta = f(z)\)构成的\DefineConcept{复合函数}.
\end{definition}

一个复变函数等价于两个二元实函数的有序组合.
当复变量\(z\)用代数形式\(z = x + \iu y\)表示时，函数\(w = f(z)\)可表为\begin{equation*}
w = u(x,y) + \iu v(x,y);
\end{equation*}其中\(u(x,y), v(x,y)\)是二元实函数；
当复变量\(z\)用指数形式\(z = r e^{\iu \theta}\)表示时，函数\(w = f(z)\)可表为\begin{equation*}
w = p(r,\theta) + \iu q(r,\theta),
\end{equation*}其中\(p(r,\theta), q(r,\theta)\)也是二元实函数.
...
在变换（或映射）\(w=f(z)\)下，点\(w_0=f(z_0)\)和点集\(M=f\ImageOfSetUnderRelation{E}\)分别称作点\(z_0\)和点集\(E\)的\DefineConcept{像}，而点\(z_0\)和点集\(E\)则分别称作点\(w_0\)和点集\(M=f\ImageOfSetUnderRelation{E}\)的\DefineConcept{原像}.
...
\end{equation*}则称“\(w=f(z)\)把\(E\)变（映）入\(F\)”，或称“\(w=f(z)\)是\(E\)到\(F\)的\DefineConcept{入变换}”，记为\(f\ImageOfSetUnderRelation{E} \subseteq F\).

若\(f\ImageOfSetUnderRelation{E} \subseteq F\)，且\begin{equation*}
\forall w \in F, \exists z \in E : w = f(z),
\end{equation*}则称“\(w = f(z)\)把\(E\)变换成\(F\)”或称“\(w = f(z)\)是\(E\)到\(F\)的\DefineConcept{满变换}”，记作\(f\ImageOfSetUnderRelation{E} = F\).
...
\begin{example}
求直线\(x=1\)在变换\(w=z^2\)下的像.
\begin{solution}
令\(z=x+\iu y\)，\(w=u+\iu v\)，则由\(w=z^2\)可得\(u=x^2-y^2\)和\(v=2xy\)，代入\(x=1\)可得\begin{equation*}
u=1-y^2, \qquad v=2y,
\end{equation*}消去\(y\)得\begin{equation*}
u=1-\frac{1}{4} v^2.
\end{equation*}这是\(w\)平面上开口向左的一条抛物线.
\end{solution}
\end{example}

\subsection{复变函数的极限}
\begin{definition}
设复函数\(w=f(z)\)在点集\(E\)上有定义，\(z_0\)为\(E\)的聚点.若\(\exists w_0 \in \mathbb{C}\)，使对\(\forall \epsilon > 0\)，总\(\exists \delta > 0\)，只要\(0<\abs{z-z_0}<\delta, z \in E\)，就有\(\abs{f(z)-w_0} < \epsilon\)，则称\(f(z)\)在\(z\)沿\(E\)趋于\(z_0\)时以\(w_0\)为\DefineConcept{极限}，记为\begin{equation*}
\lim_{\substack{z \to z_0 \\ (z \in E)}} f(z) = w.
\end{equation*}

不特别强调变量\(z\)所在点集时，符号\(\lim_{\substack{z \to z_0 \\ (z \in E)}}\)可以简化为\(\lim_{z \to z_0}\).
\end{definition}
从几何上看，\(z\)趋近于\(z_0\)时\(f(z)\)以\(w_0\)为极限的意义是：只要自变量点\(z\)进入\(z_0\)的充分小去心邻域，对应的像点\(f(z)\)就一定落入\(w_0\)的任意给定的\(\epsilon\)邻域.
...
\begin{proof}
当\(z\)沿正实轴\(E: x > 0\)趋于\(0\)时，\begin{equation*}
\lim_{\substack{z\to0 \\ (z \in E)}} \frac{z}{\abs{z}}
= \lim_{x\to0^+} \frac{x}{x} = 1.
\end{equation*}

当\(z\)沿负实轴\(E: x < 0\)趋于\(0\)时，\begin{equation*}
\lim_{\substack{z\to0 \\ (z \in E)}} \frac{z}{\abs{z}}
= \lim_{x\to0^-} \frac{x}{-x} = -1.
\end{equation*}
...
\begin{proof}
令\(z = x + \iu y\).当\(z\)沿直线\(E: y = kx\)趋于零时，有\begin{equation*}
\lim_{\substack{z\to0 \\ (z \in E)}} \frac{\Re z}{z}
= \lim_{x\to0} \frac{x}{x + \iu kx}
= \lim_{x\to0} \frac{1}{1+\iu k}.
\end{equation*}显然上式随\(k\)变化而变化，所以\(\lim_{z\to0} \frac{\Re z}{z}\)极限不存在.
\end{proof}
\end{example}

可见，从某种意义上说，复变函数的极限类似于二元实变函数的极限.
\begin{theorem}\label{theorem:复变函数.复变函数极限与二元实变函数的关系}
设\(f(z)=u(x,y)+\iu v(x,y)\)在点集\(E\)上有定义，\(z_0=x_0+\iu y_0\)为\(E\)的聚点，\(w_0=u_0+\iu v_0\)为已知复数，则\begin{equation*}
...
\begin{example}
设\(z_0\neq0,\infty\).证明：\begin{equation*}
\lim_{n\to\infty} z_n = z_0
\iff
\lim_{n\to\infty} \abs{z_n} = \abs{z_0},
\lim_{n\to\infty} \Arg z_n = \Arg z_0.
\end{equation*}其中\(\lim_{n\to\infty} \Arg z_n = \Arg z_0\)应这样理解：对\(\Arg z_0\)的任意一个确定值\(\arg z_0\)，总可以选取一个\(z_n\)的辐角取确定值的（不必取主值）数列\(\{ \arg z_n \}\)，使得\(n\to\infty\)时，有\(\arg z_n \to \arg z_0\).
\end{example}

\subsection{复变函数的连续性}
\begin{definition}\label{definition:复变函数.复变函数的连续性}
设函数\(w=f(z)\)在点集\(E\)上有定义，\(z_0\)为\(E\)的聚点，且\(z_0 \in E\)，令\(\increment z = z - z_0\).如果\begin{equation*}
...
\item\label{theorem:复变函数.连续函数的性质.3a} \(f(z)\)在\(E\)上有界，即\begin{equation*}
\exists M>0 \bigl[
z \in E \implies \abs{f(z)} \leq M
\bigr];
\end{equation*}

\item\label{theorem:复变函数.连续函数的性质.3b} \(\abs{f(z)}\)在\(E\)上有最大值\(M\)与最小值\(m\)，即\begin{equation*}
\exists z_1 \in E, \forall z \in E : \abs{f(z)} \leq \abs{f(z_1)} = M,
\end{equation*}\begin{equation*}
\exists z_2 \in E, \forall z \in E : \abs{f(z)} \geq \abs{f(z_2)} = m;
\end{equation*}

\item\label{theorem:复变函数.连续函数的性质.3c} \(f(z)\)在\(E\)上一致连续，即\begin{equation*}
...
当\(z\)从上、下半平面趋于负实轴上的点时，\begin{equation*}
\lim_{\substack{\Im z \to 0^+ \\ (\Re z < 0)}} \arg z = \pi,
\qquad
\lim_{\substack{\Im z \to 0^- \\ (\Re z < 0)}} \arg z = -\pi,
\end{equation*}故\(\arg z\)在负实轴上不连续.
...
\(N_{\delta}(z_0)\)内任一点\(z\)都满足\(\arg z < \arg z_0 + \epsilon\)，于是，当\(\abs{z - z_0} < \delta\)时，总有\begin{equation*}
