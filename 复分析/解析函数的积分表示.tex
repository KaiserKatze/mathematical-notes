...
\begin{theorem}[莫雷拉（Morera）定理]\label{theorem:解析函数的积分表示.莫雷拉定理}
若函数\(f(z)\)在单连通区域\(D\)内连续，且对\(D\)内的任一围线\(C\)有\[
\int_C f(z) \dd{z} = 0,
\]则\(f(z)\)在\(D\)内解析.
\end{theorem}
\hyperref[theorem:解析函数的积分表示.莫雷拉定理]{莫雷拉定理}又称作\DefineConcept{摩累拉定理}.对\cemph{单连通区域内连续复变函数}而言，它是柯西积分定理的逆定理.
...
\begin{theorem}[刘维尔（Liouville）定理]\label{theorem:解析函数的积分表示.刘维尔定理}
