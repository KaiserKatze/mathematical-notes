...
称级数\begin{equation*}
	\sum_{n=0}^\infty c_n (z-a)^n
\end{equation*}为“\(f(z)\)在点\(a\)的\DefineConcept{解析部分}或\DefineConcept{正则部分}”.
称级数\begin{equation*}
	\sum_{n=1}^\infty c_{-n} (z-a)^{-n}
\end{equation*}为“\(f(z)\)在点\(a\)的\DefineConcept{主要部分}（principal part）
...
当\(r \leq R\)时，
罗朗级数 \labelcref{equation:解析函数的级数表示.罗朗级数} 的和函数\(f(z)\)满足\begin{equation*}
	f(z) = \phi(z) + \psi(z),
\end{equation*}
