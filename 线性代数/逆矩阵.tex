...
\A\B=\B\A=\E,
\]
则称“\(\A\)是\DefineConcept{可逆矩阵}（invertible matrix）”，
或称“\(\A\) \DefineConcept{可逆}”；
并称\(\B\)为\(\A\)的\DefineConcept{逆矩阵}（inverse matrix），
记作\(\A^{-1}\)，即\(\A^{-1} = \B\).
\end{definition}

\begin{definition}
对于\(n\)阶方阵\(\A\)，%
若\(\abs{\A}=0\)，称“\(\A\)是\DefineConcept{奇异矩阵}（singular matrix）”；%
若\(\abs{\A} \neq 0\)，称“\(\A\)是\DefineConcept{非奇异矩阵}（non-singular matrix）”.
\end{definition}
...
则称矩阵\(\A\)与\(\B\) \DefineConcept{等价}%
或\DefineConcept{相抵}（equivalent），
记作\(\A\cong\B\).
\end{definition}
...
则有\begin{gather}
	...
	\end{bmatrix},
\end{gather}
其中\(\D - \C \A^{-1} \B\)称为矩阵\(\begin{bmatrix} \A & \B \\ \C & \D \end{bmatrix}\)关于\(\A\)的\DefineConcept{舒尔补}.
\end{theorem}
