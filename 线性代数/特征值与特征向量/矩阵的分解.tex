...
接下来，对于给定矩阵\(A \in M_n(\mathbb{R})\)，
如果我们能找到\(n\)个豪斯霍尔德矩阵\(H_1,\dotsc,H_n\)，
使得\(R \defeq Q^T A\)成为一个上三角矩阵，
其中\(Q \defeq H_1^T H_2^T \dotsm H_{n-1}^T H_n^T\)，
那么就有\(A = Q R\).

设\(
	\epsilon_1 \defeq (1,\dotsc,0)^T,
	\dotsc,
	\epsilon_n \defeq (0,\dotsc,1)^T
\)是\(\mathbb{R}^n\)的标准正交基.
若取\begin{equation*}
	\beta_1 \defeq \norm{\alpha_1} \epsilon_1 - \alpha_1,
	\qquad
	\gamma_1 \defeq \beta_1 / \norm{\beta_1},
	\qquad
	H_1 \defeq E_n - 2 \gamma_1 \gamma_1^T.
\end{equation*}
可以证明\(H_1 \alpha_1 = \norm{\alpha_1} \epsilon_1\).
记\(\alpha \defeq (a_{11},\dotsc,a_{n1})^T\)，
注意到\begin{align*}
	\norm{\beta_1}^2
	&= \norm{\norm{\alpha_1} \epsilon_1 - \alpha_1}^2 \\
	&= (\norm{\alpha_1} - a_{11})^2 + \sum_{j=2}^n a_{1j}^2 \\
	&= \norm{\alpha_1}^2 - 2 \norm{\alpha_1} a_{11} + a_{11}^2 + \sum_{j=2}^n a_{1j}^2 \\
	&= 2 (\norm{\alpha_1}^2 - \norm{\alpha_1} y_{11}),
\end{align*}
且\begin{align*}
	...
	&= 2 \gamma_1
	\frac{
		\norm{\alpha_1} \epsilon_1^T \alpha_1 - \alpha_1^T \alpha_1
	}{
		\norm{
			\norm{\alpha_1} \epsilon_1 - \alpha_1
		}
