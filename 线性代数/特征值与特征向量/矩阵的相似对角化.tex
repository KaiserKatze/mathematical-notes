...
\end{proof}
\end{example}
\begin{example}\label{example:幂零矩阵.幂等矩阵的相似标准型}
...
\end{example}
\begin{remark}
\cref{example:幂零矩阵.幂等矩阵的相似标准型} 说明：
任意一个幂等矩阵\(\vb{A}\)
一定与分块矩阵\(
	\begin{bmatrix}
		\vb{E}_r & \vb0 \\
...
而\(r = \rank\vb{A}\).
\end{remark}
\begin{example}\label{example:幂等矩阵.幂等矩阵的秩的性质2}
%@see: 《高等代数（第三版 上册）》（丘维声） P183 习题5.6 8.
证明：幂等矩阵的秩等于它的迹.
\begin{proof}
由\cref{example:幂零矩阵.幂等矩阵的相似标准型} 可知
秩为\(r\)的幂等矩阵\(\vb{A}\)的相似标准型是分块对角阵\(\diag(\vb{E}_r,\vb0)\).
因为迹是相似不变量，所以\begin{equation*}
	\tr\vb{A} = \tr\diag(\vb{E}_r,\vb0) = r = \rank\vb{A}.
	\qedhere
\end{equation*}
\end{proof}
\end{example}
