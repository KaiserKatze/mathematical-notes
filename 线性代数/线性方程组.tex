...
\end{array} \right.
\end{equation}
为\(n\)元\DefineConcept{线性方程组}（system of linear equations）.
这里，\(s\)为方程的个数\footnote{%
在方程组\labelcref{equation:线性方程组.线性方程组的代数形式} 中，%
方程的数目\(s\)可以等于未知数的数目\(n\)，%
也可以不相等（即\(s<n\)或\(s>n\)）.
}；%
我们把数\(a_{ij}\ (i=1,2,\dotsc,s; \allowbreak
j=1,2,\dotsc,n)\)称为“第\(i\)个方程中\(x_j\)的\DefineConcept{系数}”，简称为“系数”；
把数\(b_i\ (i=1,2,\dotsc,s)\)叫做“第\(i\)个方程的\DefineConcept{常数项}”.
\end{definition}
...
则线性方程组 \labelcref{equation:线性方程组.线性方程组的代数形式} 有以下两种等价表示：
\begin{enumerate}
\item 矩阵形式
\(\A \x = \b\).
\item 向量形式
...
\begin{definition}
如果存在\(n\)个数\(\v{c}{n}\)满足线性方程组 \labelcref{equation:线性方程组.线性方程组的代数形式}，即\[
a_{i1} c_1 + a_{12} c_2 + \dotsb + a_{in} c_n \equiv b_i
\quad(i=1,2,\dotsc,s),
\]
则称“线性方程组 \labelcref{equation:线性方程组.线性方程组的代数形式} 有解”，%
或“线性方程组 \labelcref{equation:线性方程组.线性方程组的代数形式} 是相容的”；
否则，称“线性方程组 \labelcref{equation:线性方程组.线性方程组的代数形式} 无解”，%
或“线性方程组 \labelcref{equation:线性方程组.线性方程组的代数形式} 是不相容的”.

称这\(n\)个数\(\v{c}{n}\)构成的列向量\((\v{c}{n})^T\)为%
线性方程组 \labelcref{equation:线性方程组.线性方程组的代数形式} 的一个\DefineConcept{解}（solution）%
或\DefineConcept{解向量}（solution vector）.
线性方程组 \labelcref{equation:线性方程组.线性方程组的代数形式} 的解的全体构成的集合，%
称为“线性方程组 \labelcref{equation:线性方程组.线性方程组的代数形式} 的\DefineConcept{解集}”.
\end{definition}
...
\]线性表出，则称\(\alpha\)可由\(\beta\)\DefineConcept{线性表出}.

如果\(\alpha\)与\(\beta\)可以相互线性表出，则称\(\alpha\)与\(\beta\)\DefineConcept{等价}，记作\(\alpha \cong \beta\).
\end{definition}
