...
称这\(n\)个数构成的列向量\((\v{c}{n})^T\)为它的一个\textbf{解}（solution）.
线性方程组\labelcref{equation:线性方程组.线性方程组的代数形式} 的解的全体构成的集合\(S\)，%
...
\begin{definition}
称常数项中有非零数的线性方程组为\textbf{非齐次线性方程组}.
称常数项全为零的线性方程组为\textbf{齐次线性方程组}.
\end{definition}
...
\begin{theorem}
\(\alpha=\{\v{\mat{\alpha}}{s}\}\ (s>1)\)%
线性相关的充要条件是：
\(\alpha\)可由某个部分组%
\[\mat{\alpha}_1,\dotsc,\mat{\alpha}_{i-1},\mat{\alpha}_{i+1},\dotsc,\mat{\alpha}_s\]
线性表出.
\end{theorem}
...
假设部分组\(\a1,\a2,\dotsc,\a{r+1}\)线性无关，%
那么\[
\rank\{\a1,\a2,\dotsc,\a{r+1}\} = r+1.
\]
