...
j=1,2,\dotsc,n)\)称为“第\(i\)个方程中\(x_j\)的\textbf{系数}”，简称为“系数”；
把数\(b_i\ (i=1,2,\dotsc,s)\)叫做“第\(i\)个方程的\textbf{常数项}”.
\end{definition}
...
线性方程组 \labelcref{equation:线性方程组.线性方程组的代数形式} 的一个\textbf{解}（solution）%
或\textbf{解向量}（solution vector）.
线性方程组 \labelcref{equation:线性方程组.线性方程组的代数形式} 的解的全体构成的集合，%
称为“线性方程组 \labelcref{equation:线性方程组.线性方程组的代数形式} 的\textbf{解集}”.
\end{definition}
...
称数\(a_i\ (i=1,2,\dotsc,n)\)为“\(\a\)的第\(i\)个\textbf{分量}”.
\end{definition}
...
\]
称为“向量组\(\AutoTuple{\a}{s}\)的一个\textbf{线性组合}（linear combination）”，
称\(\AutoTuple{k}{s}\)为这个线性组合的\textbf{系数}；

给定\(\b \in K^n\).
如果\(\exists k_i \in K\ (i=1,2,\dotsc,s)\)，使得\[
	\b = \sum\limits_{i=1}^s k_i \a_i,
\]
则称“向量\(\b\)可以由向量组\(\AutoTuple{\a}{s}\)\textbf{线性表出}”.
\end{definition}
