...
由于\(\vb{A}\alpha_i\in V'\)，
因此\(\vb{A}\alpha_i\)可以
由\(V'\)的基\(\AutoTuple{\beta}{s}\)唯一地线性表出：\begin{equation}\label{equation:用矩阵表示两个有限维线性空间之间的线性映射.形式1}
%@see: 《高等代数（第三版 下册）》（丘维声） P118 (4)
	...
	\end{array} \right.
\end{equation}
我们可以在形式上把上式写成\begin{equation}\label{equation:用矩阵表示两个有限维线性空间之间的线性映射.形式2}
...
下的\DefineConcept{矩阵}”，
记作\(\LinearMapMatrix(\vb{A},(\AutoTuple{\alpha}{n}),(\AutoTuple{\beta}{s}))\)，
或在不致混淆的情况下简记为\(\LinearMapMatrix(\vb{A})\).
...
那么\cref{equation:用矩阵表示两个有限维线性空间之间的线性映射.形式1,equation:用矩阵表示两个有限维线性空间之间的线性映射.形式2}
可以化为\begin{equation*}
...
设\(V,V',V''\)都是域\(F\)上的有限维线性空间，
\(\vb{A}\)是\(V\)到\(V'\)的一个线性映射，
\(\vb{B}\)是\(V'\)到\(V''\)的一个线性映射，
\(A\)是\(\vb{A}\)在\(V\)的基\(\AutoTuple{\alpha}{n}\)和\(V'\)的基\(\AutoTuple{\beta}{m}\)下的矩阵，
\(B\)是\(\vb{B}\)在\(V'\)的基\(\AutoTuple{\beta}{m}\)和\(V''\)的基\(\AutoTuple{\gamma}{t}\)下的矩阵，
则\(B A\)是\(\vb{B} \vb{A}\)在\(V\)的基\(\AutoTuple{\alpha}{n}\)和\(V''\)的基\(\AutoTuple{\gamma}{t}\)下的矩阵.
\end{proposition}
...
\(\AutoTuple{\alpha}{n}\)与\(\AutoTuple{\beta}{n}\)下的矩阵分别为\(A,B\).
如果\(S\)是从基\(\AutoTuple{\alpha}{n}\)到基\(\AutoTuple{\beta}{n}\)的过渡矩阵，
...
\end{theorem}
\begin{remark}
\cref{theorem:线性映射的矩阵表示.线性变换在不同基下的矩阵相似} 说明：
同一个线性变换\(\vb{A}\)在\(V\)的不同基下的矩阵是相似的.
\end{remark}
