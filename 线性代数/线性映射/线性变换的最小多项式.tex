...
%@see: 《高等代数（第三版 下册）》（丘维声） P142 (1)
	...
	\end{bmatrix}.
\end{equation}
称为一个\(k\)阶\DefineConcept{若尔当块}.
\end{definition}

\begin{example}
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P204
设\(J_k(a)\)是一个\(k\)阶若尔当块，
则\begin{equation*}
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P204 (6.1.4)
	[J_k(a)]^n
	=
	\begin{bmatrix}
		a^n & C_n^1 a^{n-1} & C_n^2 a^{n-2} & \dots & C_n^{k-2} a^{n-k+2} & C_n^{k-1} a^{n-k+1} \\
		0 & a^n & C_n^1 a^{n-1} & \dots & C_n^{k-3} a^{n-k+3} & C_n^{k-2} a^{n-k+2} \\
		0 & 0 & a^n & \dots & C_n^{k-4} a^{n-k+4} & C_n^{k-3} a^{n-k+3} \\
		\vdots & \vdots & \vdots & & \vdots & \vdots \\
		0 & 0 & 0 & \dots & a^n & C_n^1 a^{n-1} \\
		0 & 0 & 0 & \dots & 0 & a^n
	\end{bmatrix},
\end{equation*}
其中\begin{equation*}
	C_n^k = \begin{cases}[cl]
		\frac{n!}{k!(n-k)!},
		& k \leq n, \\
		0,
		& k > n.
...
\begin{example}
设\(f(J_k) = \lambda_0 E + \lambda_1 J_k + \lambda_2 J_k^2 + \dotsb + \lambda_k J_k^k\)
是\(k\)阶若尔当块\(J_k(a)\)的多项式，
则\begin{align*}
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P205 (6.1.5)
	f(J_k)
	= \begin{bmatrix}
		f(a) & f'(a) & \frac{1}{2!} f''(a) & \dots & \frac{1}{(k-1)!} f^{(k-1)}(a) \\
		& f(a) & f'(a) & \dots & \frac{1}{(k-2)!} f^{(k-2)}(a) \\
		&& f(a) & \dots & \frac{1}{(k-3)!} f^{(k-3)}(a) \\
		&&& \ddots & \vdots \\
		&&&& f(a) \\
	\end{bmatrix}.
\end{align*}
...
\begin{example}
设分块对角矩阵\(J = \diag(J_1,J_2,\dotsc,J_r)\)是矩阵\(A \in M_n(F)\)的若尔当标准型，
则\(J\)的多项式为\begin{gather*}
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P205 (6.1.6)
	f(J) = \diag(f(J_1),f(J_2),\dotsc,f(J_r));
\end{gather*}
存在可逆矩阵\(P\)，使得\begin{equation}\label{equation:线性映射.矩阵多项式的若尔当表示}
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P205 (6.1.7)
	f(A) = P ~ f(J) ~ P^{-1}.
\end{equation}
\end{example}
我们把\cref{equation:线性映射.矩阵多项式的若尔当表示}
称为“矩阵多项式\(f(A)\)的\DefineConcept{若尔当表示}”.
