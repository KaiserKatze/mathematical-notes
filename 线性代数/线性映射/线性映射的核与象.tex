...
%@see: 《Linear Algebra Done Right (Fourth Eidition)》（Sheldon Axler） P98 3.93
设\(V\)是域\(F\)上的线性空间，
...
%@see: 《Linear Algebra Done Right (Fourth Eidition)》（Sheldon Axler） P65
给定正整数\(m,n\)，
矩阵\(A \in M_{m \times n}(F)\)，
...
\begin{example}
%@see: 《高等代数（第三版 下册）》（丘维声） P115
%@see: 《Linear Algebra Done Right (Fourth Eidition)》（Sheldon Axler） P90 3.78
...
\begin{theorem}
%@see: 《Linear Algebra Done Right (Fourth Eidition)》（Sheldon Axler） P102 3.107
\def\T{\vb{T}}%
\def\wT{\widetilde{\T}}
设\(V,W\)都是域\(F\)上的线性空间，
\(\T\)是从\(V\)到\(W\)的一个线性映射，
