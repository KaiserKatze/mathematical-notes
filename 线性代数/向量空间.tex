...
于是，\begin{align*}
	...
	&\iff \text{\(K\)中存在一组数\(c_1,c_2,\dotsc,c_n\)，使得\(c_1 \a_1 + c_2 \a_2 + \dotsb + c_n \a_n = \b\)成立} \\
	&\iff \text{\(\b\)可以由\(\a_1,\a_2,\dotsc,\a_n\)线性表出}.
\end{align*}
这样我们把线性方程组有没有解的问题归结为：
常数项列向量\(\b\)能不能由系数矩阵的列向量组线性表出.
这个结论有两方面的意义：
一方面，为了从理论上研究线性方程组有没有解，
就需要去研究\(\b\)能否由\(\a_1,\a_2,\dotsc,\a_n\)线性表出；
另一方面，对于\(K^n\)中给定的向量组\(\a_1,\a_2,\dotsc,\a_n\)，
以及给定的\(\b\)，
为了判断\(\b\)能否由\(\a_1,\a_2,\dotsc,\a_n\)线性表出，
