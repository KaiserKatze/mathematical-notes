...
称为非齐次线性方程组\(\vb{A}\vb{x}=\vb\beta\)的\DefineConcept{导出组}.

\begin{proposition}
%@see: 《高等代数（第三版 上册）》（丘维声） P97 性质1
非齐次线性方程组\(\vb{A}\vb{x}=\vb\beta\)的两个解的差是它的导出组的一个解.
\begin{proof}
设\(\vb{x}_1,\vb{x}_2\)是非齐次线性方程组\(\vb{A}\vb{x}=\vb\beta\)的任意两个解，即\[
...
\def\ma{\vb{\xi}}
\def\mb{\vb{\zeta}}
设\(\ma\)是\(\vb{A}\vb{x}=\vb\beta\)的一个解，\(\mb\)是\(\vb{A}\vb{x}=\vb0\)的一个解，即\[
