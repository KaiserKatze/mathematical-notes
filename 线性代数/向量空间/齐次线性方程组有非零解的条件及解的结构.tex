...
\begin{example}
设\(\A\)为\(s \times n\)矩阵，\(\B\)为\(n \times m\)矩阵，\(\A\B=\z\).证明：\(\rank\A + \rank\B \leqslant n\).
\begin{proof}
...
则\(\beta\)可由\(X\)线性表出，\(\rank\beta \leqslant \rank X\).
而\(\rank\beta=\rank\B\)，\(\rank X=n-\rank\A\).

综上所述，\(\rank\A+\rank\B \leqslant n\)成立.
\end{proof}
\end{example}
