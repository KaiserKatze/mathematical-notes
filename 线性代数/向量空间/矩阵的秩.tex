...
\begin{itemize}
	\item \(\vb{A}\)的列向量组生成的子空间\begin{equation*}
		\Set{
			\vb{y} \in K^s \given \vb{y} = \vb{A} \vb{x}, \vb{x} \in K^n
		}
	\end{equation*}
	...
	\item \(\vb{A}\)的行向量组生成的子空间\begin{equation*}
		\Set{
			\vb{y} \in K^n \given \vb{y}^H = \vb{x}^H \vb{A}, \vb{x} \in K^s
		}
	\end{equation*}
...
矩阵\(\vb{A}\)的行空间就是它的共轭转置的列空间，
它的列空间就是它的共轭转置的行空间，
即\begin{equation*}
	\SpanR\vb{A} = \SpanC\vb{A}^H,
	\qquad
	\SpanC\vb{A} = \SpanR\vb{A}^H.
\end{equation*}
...
\begin{figure}[hbt]% 实数域上向量空间的四个特殊子空间
...
\begin{figure}[hbt]% 复数域上向量空间的四个特殊子空间
%@see: https://www.cs.utexas.edu/~flame/laff/alaff-beta/chapter04-four-fundamental-spaces.html
	\centering
	\begin{tikzpicture}
		\begin{scope}[rotate=40]
		...
		\begin{scope}[-{Stealth[length=4.5mm,width=0pt 8]}]
			...
			\draw (xn)--(o)node[sloped,pos=.55,below]{$\vb{A} \vb{x}_0 = \vb0$};
		\end{scope}
		\draw[white] (rsnw)--(rsne)node[sloped,midway,below,black]{$\Im\vb{A}^H$};
		\draw[white] (nsnw)--(nssw)node[sloped,midway,above,black]{$\Ker\vb{A}$};
		\draw[white] (csnw)--(csne)node[sloped,midway,below,black]{$\Im\vb{A}$};
		\draw[white] (lsne)--(lsse)node[sloped,midway,above,black]{$\Ker\vb{A}^H$};
		\draw[dashed] (xr)--(x)--(xn);
		\draw (-2,0)node{\Huge$\mathbb{C}^n$};
		\draw (7.5,0)node{\Huge$\mathbb{C}^s$};
	\end{tikzpicture}
	% 在图中，我们把\(\Ker\vb{A}\)和\(\Im\vb{A}^H\)都画在左侧，靠近符号\(K^n\)，就是表示它们都是向量空间\(K^n\)的子空间；
	% 我们把\(\Ker\vb{A}^H\)和\(\Im\vb{A}\)都画在右侧，靠近符号\(K^s\)，就是表示它们都是向量空间\(K^s\)的子空间.
	% 根据\cref{example:齐次线性方程组的解集的结构.矩阵的像空间与它的左核空间互为正交补}，
	% 我们把代表\(\Ker\vb{A}\)和\(\Im\vb{A}^H\)的两个矩形的长边画成互相垂直，就是表示这两个子空间中的向量互相正交；
	% 同样地，我们把代表\(\Ker\vb{A}^H\)和\(\Im\vb{A}\)的两个矩形的长边画成互相垂直，就是表示这两个子空间中的向量互相正交.
	\caption{}
\end{figure}
