...
设\(\A\)是\(s \times n\)矩阵，则\(0 \leqslant \rank\A \leqslant \min\{s,n\}\).
\end{property}
...
矩阵的乘积的秩\(\rank(\A\B) \leqslant \min\{\rank\A,\rank\B\}\).
\begin{proof}
...
\rank\A = \rank(\P^{-1}(\P\A)) \leqslant \rank(\P\A) \leqslant \rank\A,
...
当\(n>2\)时，\(\rank(\A^T\A) \leqslant \rank\A \leqslant2\)，\(\D = \A^T \A\)不满秩，故由\cref{theorem:向量空间.满秩方阵的行列式非零} 有\(\abs{\D}=0\).
\end{solution}
\end{example}
