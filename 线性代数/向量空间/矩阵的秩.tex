...
\begin{itemize}
	...
	“\(\A\)的\DefineConcept{行空间}（row space）”，记为\(\SpanR\A\).
	\item \(\A\)的列向量组生成的子空间称为
	“\(\A\)的\DefineConcept{列空间}（column space）”，记为\(\SpanC\A\).
	\item \(\A\)的行向量组的秩称为
	“\(\A\)的\DefineConcept{行秩}（row rank）”，记为\(\RankR\A\).
	\item \(\A\)的列向量组的秩称为
	“\(\A\)的\DefineConcept{列秩}（column rank）”，记为\(\RankC\A\).
\end{itemize}
...
因此\[
	...
	= \rank\A+\rank\B.
\]
当且仅当存在矩阵\(\vb{X} \in M_{n \times m}(K)\)，存在矩阵\(\vb{Y} \in M_{s \times l}(K)\)，
使得\(\C=\A\vb{X}+\vb{Y}\B\)成立时，
上式取“\(=\)”号.
