...
矩阵的乘积的秩\(\rank(\A\B) \leq \min\{\rank\A,\rank\B\}\).
\begin{proof}
...
当\(m>n\)时，
根据\cref{theorem:线性方程组.矩阵的秩的性质2}，有\[\rank\A,\rank\B \leq \min\{m,n\} = n;\]
再根据\cref{theorem:线性方程组.矩阵乘积的秩}，有\[\rank(\A\B) \leq \min\{\rank\A,\rank\B\} = n < m,\]
也就是说，矩阵\(\A\B\)不满秩；
...
\end{proof}
\end{theorem}
\cref{equation:线性方程组.柯西-比内公式} 称为\DefineConcept{柯西-比内公式}.
...
当\(n>2\)时，\(\rank(\A^T\A) \leq \rank\A \leq2\)，\(\D = \A^T \A\)不满秩，故由\cref{theorem:向量空间.满秩方阵的行列式非零} 有\(\abs{\D}=0\).
\end{solution}
\end{example}
