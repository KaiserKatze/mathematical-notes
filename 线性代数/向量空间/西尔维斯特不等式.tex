...
\(\rank(\A\B) = \rank\A\).
由\cref{theorem:向量空间.零矩阵的秩为零,theorem:向量空间.秩为零的矩阵必为零矩阵} 可知\[
	\A\B=\vb0
	\iff
	\rank\A=\rank(\A\B)=0
	\implies
...
%@credit: {c5f76621-34f0-4114-845f-e36475300576}
充分性.
假设\(\abs{\A}=0\)，
则\(\rank\A = r < n\)，
取\(\Ker\A\)的一个基\(\AutoTuple{\vb\beta}{n-r}\)
构成一个矩阵\(\B=(\AutoTuple{\vb\beta}{n-r})\)，
那么\(\A\B=\vb0\).
\end{proof}
\end{example}
