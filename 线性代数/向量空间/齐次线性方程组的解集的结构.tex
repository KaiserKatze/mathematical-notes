...
\]是\(K^n\)的一个子空间.
我们把它称为“方程组\(\A\x=\vb0\)的\DefineConcept{解空间}（space of solution）”.
有时也把它称为“矩阵\(\A\)的\DefineConcept{零空间}（null space）”，
并记为\(\NullSpace\A\).
...
%%DELETE: 前面已经有\cref{equation:线性方程组.西尔维斯特不等式} 了，这里可以删除了.
% \cref{theorem:线性方程组.齐次线性方程组的解向量个数}
% 揭示矩阵\(\A\)的秩与方程\(\A\x=\vb0\)的基础解系所含向量个数的关系.
% 它不仅对\(\A\x=\vb0\)的求解有重要意义，而且可以用来解决矩阵的秩的一些问题.
% \begin{example}
% 设\(\A\)为\(s \times n\)矩阵，\(\B\)为\(n \times m\)矩阵，\(\A\B=\vb0\).
% 证明：\(\rank\A + \rank\B \leq n\).
% \begin{proof}
% 设矩阵\(\B\)与\(\A\B=\vb0\)右端的零矩阵列分块矩阵分别为\[
% 	\B=(\AutoTuple{\b}{m}),
% 	\quad
% 	\vb0=(\vb0,\dotsc,\vb0)
% \]
...
% 	= (\vb0,\vb0,\dotsc,\vb0)
% \]或\[
% 	\A\b_j = \vb0 \quad(j=1,2,\dotsc,m)
% \]
% 即\(\beta=\{\AutoTuple{\b}{m}\}\)是齐次线性方程组\(\A\x=\vb0\)解向量组.

% 若\(\rank\A=n\)，则\(\A\x=\vb0\)只有零解，\(\B=\vb0\)，\(\rank\B=0=n-\rank\A\).

% 若\(\rank\A<n\)，则\(X=\{\AutoTuple{\x}{n-r}\}\)是\(\A\x=\vb0\)的一个基础解系，
