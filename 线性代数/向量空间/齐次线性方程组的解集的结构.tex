...
\begin{enumerate}
	...
	\item 当\(k=1,2,\dotsc,n-r\)时，
	让解向量\(\vb{x} = (\AutoTuple{x}{n})^T\)
	第\(j_k\)个分量\(x_{j_k}\)取\(1\)（或一个合适的非零数），
	而\(x_{j_1},\dotsc,x_{j_{k-1}},x_{j_{k+1}},\dotsc,x_{j_{n-1}}\)全都取零，
	代入\(\vb{J} \vb{x} = \vb0\)，
	求出\(x_{i_1},\dotsc,x_{i_r}\)，
	于是得到\(n-r\)个解向量\(\AutoTuple{\vb{x}}{n-r}\)，
	这就是齐次线性方程组\(\vb{A}\vb{x}=\vb0\)的一个基础解系.
\end{enumerate}
\end{algorithm}
