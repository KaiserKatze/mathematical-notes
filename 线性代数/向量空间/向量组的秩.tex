...
%\begin{example}
%在数域\(K\)上，满足\[
%\abs{a_{ii}} > \sum\limits_{\substack{1 \leq j \leq n \\ j \neq i}} \abs{a_{ij}}
%\quad (i=1,2,\dotsc,n)
...
%\]\[
%	\abs{a_{ll}} \leq \sum\limits_{\substack{1 \leq j \leq n \\ j \neq l}} \frac{\abs{k_j}}{\abs{k_l}} \abs{a_{lj}}
%	\leq \sum\limits_{\substack{1 \leq j \leq n \\ j \neq l}} \abs{a_{lj}}.
%\]
%这与已知条件矛盾！
...
\begin{enumerate}
	\item 如果\(\rank{A}=s\)，则向量组\(A\)线性无关.
	\item 如果\(\rank{A}<s\)，则向量组\(A\)线性相关.
\end{enumerate}
\begin{proof}
\(\text{\(A\)线性无关}
...
\begin{proof}
假设矩阵\(\widetilde{\A}=(\a_{j1},\a_{j2},\dotsc,\a_{jk},\a_l)\)经相同的初等行变换化为\[
	\widetilde{\B}=(\b_{j1},\b_{j2},\dotsc,\b_{jk},\b_l) \quad(l=1,2,\dotsc,m).
\]
考虑以下四个向量形式的线性方程组
\begin{gather}
