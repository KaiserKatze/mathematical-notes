...
\begin{definition}
设\(U\)是\(K^n\)的一个子空间.
如果\(\AutoTuple{\a}{r} \in U\)，并且满足\begin{enumerate}
	\item \(\AutoTuple{\a}{r}\)线性无关，%
	\item \(U\)中的每一个向量都可以由\(\AutoTuple{\a}{r}\)线性表出，%
\end{enumerate}
那么称\(\AutoTuple{\a}{r}\)是\(U\)的一个\DefineConcept{基}.
\end{definition}
由于基\(\AutoTuple{\a}{r}\)线性无关，因此如果\(\a\)可以由\(\AutoTuple{\a}{r}\)线性表出，那么表法唯一.

\begin{example}
试证：任意\(n\)维向量\(\a = (\AutoTuple{k}{n})\)是向量组\(\e_1 = (1,0,\dotsc,0)\)，\(\e_2 = (0,1,\dotsc,0)\)，\(\e_n = (0,0,\dotsc,1)\)的一个线性组合.
\begin{proof}
由向量的线性运算规律易知，\[
(\AutoTuple{k}{n})
= k_1 (1,0,\dotsc,0)
...
\begin{property}
基本向量组\(\AutoTuple{\e}{n}\)是\(K^n\)的一个基，称之为\(K^n\)的\DefineConcept{标准基}.
\end{property}
...
\begin{theorem}
向量组\(\AutoTuple{\a}{s}\)的一个极大无关组是这个向量组生成的子空间\(\opair{\AutoTuple{\a}{s}}\)的一个基，从而\begin{equation}\label{equation:线性方程组.子空间的维数与向量组的秩的联系}
