...
也形成\(F\)上的线性空间，
则称“\(U\)是\(V\)的一个\DefineConcept{（线性）子空间}（\(U\) is a \emph{subspace} of \(V\)）”，
...
设\(U\)是域\(F\)上线性空间\(V\)的一个子空间.
如果\(U\)不是\(V\)的零子空间，
则称“\(U\)是\(V\)的一个\DefineConcept{非零子空间}”.

\begin{proposition}
空集\(\emptyset\)不是线性空间\(V\)的子空间.
\end{proposition}
...
\begin{example}%\label{example:幂等矩阵.幂等矩阵的核空间与像空间}
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P107 定理3.4.2
设\(\vb{A} \in M_n(K)\)是一个幂等矩阵.
证明：\begin{itemize}
	\item \(\Ker\vb{A} = \Img(\vb{E} - \vb{A})\)，
	\item \(\Img\vb{A} = \Ker(\vb{E} - \vb{A})\)，
	\item \(\Ker\vb{A} \DirectSum \Img\vb{A} = K^n\).
\end{itemize}
% 由\cref{example:幂等矩阵.幂等矩阵的零因子} 立即可得
\end{example}
