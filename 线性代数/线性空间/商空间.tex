...
记作\(\alpha+W\)，
称之为“（以\(\alpha\)为代表的）\(W\)的一个\DefineConcept{陪集}%
...
设\(S_1\)和\(S_2\)是\(V\)的两个陪集.
把包含\(\mathscr{S} \defeq S_1 \cup S_2\)的所有陪集之交\(
	\bigcap\Set{
		X
		\given
...
设\(\{S_i\}_{i \in I}\)是\(V\)的一个陪集族.
把包含\(\mathscr{S} \defeq \bigcup_{i \in I} S_i\)的所有陪集之交\(
	\bigcap\Set{
		X
		\given
		X \supseteq \mathscr{S}
