...
\begin{definition}
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P144 定义3.11.1
设\(\vb{A} \in M_n(\mathbb{C})\)是厄米矩阵.
对于任意一个\(n\)维非零向量\(\vb{X}\)，
把实数\begin{equation}
	\RayleighQuotient_{\vb{A}}(\vb{X})
	\defeq
	\frac{
		\vb{X}^H \vb{A} \vb{X}
	}{
...
\end{equation}
称为“厄米矩阵\(\vb{A}\)的\DefineConcept{瑞利商}（Rayleigh quotient）”.
%@see: https://en.wikipedia.org/wiki/Rayleigh_quotient
\end{definition}

由\cref{theorem:正规矩阵.厄米矩阵的特征值} 可知，
厄米矩阵的特征值都是实数，
实际上我们可以利用瑞利商估计厄米矩阵的特征值的取值范围.
\begin{theorem}
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P144 定理3.11.1(1)
设\(\vb{A}\)是一个\(n\)阶厄米矩阵，
则对于任意实数\(k\)，
有\begin{equation*}
	\RayleighQuotient_{\vb{A}}(k \vb{X})
	= \RayleighQuotient_{\vb{A}}(\vb{X}).
\end{equation*}
\end{theorem}
\begin{theorem}\label{example:二次型.瑞利商的取值范围}
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P144 定理3.11.1(2)
%@see: 《数值分析（第5版）》（李庆扬、王能超、易大义） P242 定理4(1)
%@see: 《高等代数（第三版 上册）》（丘维声） P202 习题6.1 11.
设\(\lambda_1 \geq \dotsb \geq \lambda_n\)都是\(n\)阶厄米矩阵\(\vb{A}\)的特征值，
则对于\(\mathbb{C}^n\)中任一非零向量\(\vb\alpha\)，
总有\begin{equation*}
	\lambda_n \leq \RayleighQuotient_{\vb{A}}(\vb\alpha) \leq \lambda_1.
\end{equation*}
\begin{proof}
假设酉矩阵\(\vb{U}\)满足\begin{equation*}
	\vb{U}^T \vb{A} \vb{U} = \vb\Lambda,
\end{equation*}
其中\(\vb\Lambda = \diag(\AutoTuple{\lambda}{n})\)，
那么\begin{equation*}
	\vb\alpha^T \vb{A} \vb\alpha
	= \vb\alpha^T (\vb{U} \vb\Lambda \vb{U}^T) \vb\alpha
	= (\vb{U}^T \vb\alpha)^T \vb\Lambda (\vb{U}^T \vb\alpha)
	= \lambda_1 b_1^2 + \dotsb + \lambda_n b_n^2,
\end{equation*}
其中\(\vb{U}^T \vb\alpha = (\AutoTuple{b}{n})\).
...
又由于\begin{equation*}
	b_1^2 + \dotsb + b_n^2
	= (\vb{U}^T \vb\alpha)^T (\vb{U}^T \vb\alpha)
	= \vb\alpha^T (\vb{U} \vb{U}^T) \vb\alpha
	= \vb\alpha^T \vb\alpha
	\neq 0,
\end{equation*}
...
即\(\lambda_n \leq \RayleighQuotient_{\vb{A}}(\vb\alpha) \leq \lambda_1\).
\end{proof}
\end{theorem}

\begin{theorem}
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P144 定理3.11.1(3)
%@see: 《数值分析（第5版）》（李庆扬、王能超、易大义） P242 定理4(2)
设\(\lambda_1 \geq \dotsb \geq \lambda_n\)都是\(n\)阶厄米矩阵\(\vb{A}\)的特征值，
则\begin{equation*}
	\min_{\vb{X} \neq \vb0} \RayleighQuotient_{\vb{A}}(\vb{X}) = \lambda_n,
	\qquad
	\max_{\vb{X} \neq \vb0} \RayleighQuotient_{\vb{A}}(\vb{X}) = \lambda_1.
\end{equation*}
\end{theorem}
