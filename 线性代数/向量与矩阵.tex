...
位于这些行与列交叉处的\(kl\)个元素，按原顺序排成的\(k \times l\)矩阵\[
	...
	\end{array}
\]称为“\(\vb{A}\)的一个\(k\)阶\DefineConcept{子矩阵}（submatrix）”.
...
\begin{definition}
对于\(n\)维向量\(\vb\alpha = (\AutoTuple{a}{n})\)和\(\vb\beta = (\AutoTuple{b}{n})\).
称向量\(\vb\alpha\)与向量\(\vb\beta\)的负向量\(-\vb\beta\)的和为向量\(\vb\alpha\)与向量\(\vb\beta\)的\DefineConcept{差}，即\[
...
\begin{enumerate}
	\item 称矩阵\[
		...
		\end{bmatrix}
	\]为“\(\vb{A}\)与\(\vb{B}\)的\DefineConcept{和}（sum）”，
	记作\(\vb{A}+\vb{B}\).
	...
	\item 任取\(k\in K\)，称矩阵\[
		...
		\end{bmatrix}
	\]为“\(k\)与矩阵\(\vb{A}\)的\DefineConcept{数乘}”，
	记作\(k\vb{A}\).
	...
	\item 称矩阵\[
		(-a_{ij})_{s \times n}=\begin{bmatrix}
			...
			-a_{s1} & -a_{s2} & \dots & -a_{sn}
		\end{bmatrix}
	\]为\(\vb{A}\)的\DefineConcept{负矩阵}，记作\(-\vb{A}\).
\end{enumerate}
\end{definition}
