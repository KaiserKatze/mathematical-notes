...
\end{equation*}
称为“\(\vb{A}\)的\DefineConcept{子块}（block）”.
以子块为元素的矩阵称为\DefineConcept{分块矩阵}（block matrix）.
%@see: https://mathworld.wolfram.com/BlockMatrix.html
\end{definition}
...
\begin{definition}
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P96 定义3.1.3
设矩阵\(\vb{A} = (a_{ij})_{s \times n} \in M_{s \times n}(\mathbb{C})\).
将\(\vb{A}\)转置后，再对各元素取共轭，
把这样的矩阵\((\ComplexConjugate{a_{ji}})_{n \times s}\)
...
\begin{property}\label{theorem:矩阵的共轭转置.性质1}
...
\begin{property}\label{theorem:矩阵的共轭转置.性质2}
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P96
设\(\vb{A},\vb{B} \in M_n(K)\)，
则\begin{equation}
	(\vb{A} + \vb{B})^H = \vb{A}^H + \vb{B}^H.
\end{equation}
\end{property}

\begin{property}\label{theorem:矩阵的共轭转置.性质3}
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P97
设\(\vb{A} \in M_n(K)\)，\(k \in K\)，
则\begin{equation}
	(k \vb{A})^H = \ComplexConjugate{k} \vb{A}^H.
\end{equation}
\end{property}

\begin{property}
如果矩阵\(\vb{A} \in M_{s \times n}(\mathbb{C})\)的每个元素都是实数，
则\(\ComplexConjugate{\vb{A}} = \vb{A}\)且\(\vb{A}^H = \vb{A}^T\).
\end{property}
...
\begin{itemize}
	...
	即\begin{equation*}
		...
		\quad(i=1,2,\dotsc,s;j=1,2,\dotsc,n),
	\end{equation*}
	则称“\(\vb{A}\)是\DefineConcept{正矩阵}（elementwise positive matrix）”.
	...
	即\begin{equation*}
		...
		\quad(i=1,2,\dotsc,s;j=1,2,\dotsc,n),
	\end{equation*}
	则称“\(\vb{A}\)是\DefineConcept{非负矩阵}（elementwise non-negative matrix）”.
\end{itemize}
\end{definition}
...
\begin{itemize}
	...
	即\begin{equation*}
		...
		\quad(i=1,2,\dotsc,n),
	\end{equation*}
	则称“\(\vb\beta\)是\DefineConcept{正向量}（elementwise positive matrix）”.
	...
	即\begin{equation*}
		...
		\quad(i=1,2,\dotsc,n),
	\end{equation*}
	则称“\(\vb\beta\)是\DefineConcept{非负向量}（elementwise non-negative matrix）”.
\end{itemize}
\end{definition}
