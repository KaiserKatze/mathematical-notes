...
%@see: https://mathworld.wolfram.com/AffineSpace.html
\end{definition}
...
\begin{theorem}
%@see: 《基础代数（第二卷）》（席南华） P167 定理4.14
设\(\AutoTuple{p}[0]{k}\)是仿射空间\(A\)中\(k+1\)个点.
\begin{itemize}
...
\begin{definition}
%@see: 《基础代数（第二卷）》（席南华） P168
设\(\AutoTuple{p}[0]{k}\)是仿射空间\(A\)中\(k+1\)个点.
如果\(\AutoTuple{p}[0]{k}\)都属于某个\(k-1\)维仿射子空间，
则称“点\(\AutoTuple{p}[0]{k}\) \DefineConcept{仿射相关}”；
否则称“点\(\AutoTuple{p}[0]{k}\) \DefineConcept{仿射无关}”
或“点\(\AutoTuple{p}[0]{k}\) \DefineConcept{处于一般位置}”.
\end{definition}

\begin{proposition}
%@see: 《基础代数（第二卷）》（席南华） P168
设\(\AutoTuple{p}[0]{k}\)是仿射空间\(A\)中\(k+1\)个点，
则\(\AutoTuple{p}[0]{k}\)仿射无关，
当且仅当\(k\)个向量\(\vec{p_0p_1},\dotsc,\vec{p_0p_k}\)线性无关.
%TODO proof 依据如下：
%@see: 《基础代数（第二卷）》（席南华） P167 定理4.14
\end{proposition}
