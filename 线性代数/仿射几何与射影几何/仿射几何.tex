...
%@see: https://mathworld.wolfram.com/AffineGeometry.html
\end{definition}
\begin{remark}
...
“仿射几何\(A\)与\(A'\)同构（\(A\) is \emph{isomorphic} to \(A'\)）”，
记作\(A \Isomorphism A'\).
\end{definition}
%TODO 仿射同构的定义难道不要求两个仿射几何的域相同吗？下面的定理\cref{theorem:仿射几何.仿射同构.仿射几何同构的充分必要条件} 就有要求！
...
\begin{theorem}%\label{theorem:仿射几何.仿射同构.仿射几何同构的充分必要条件}
...
到仿射几何\(\mathcal{A}(\alpha' + W')\)的一个\DefineConcept{仿射映射}”.
\end{definition}
...
\end{equation*}
则称“\([O;\AutoTuple{P}{n}]\)是仿射几何\(A\)的（以\(O\)为原点的）\DefineConcept{标架}”.
\end{definition}
...
\begin{definition}
%@see: 《高等代数与解析几何（第三版 下册）》（孟道骥） P458 定义10.6.1
设\(P,A,B,C,A',B',C'\)是7个不同点，
\(A,B,C\)三点不共线，
\(A',B',C'\)三点不共线，
记\(
...
\).
如果\(L,M,N\)三点互不相同且共线，
则称“\(\triangle ABC\)与\(\triangle A'B'C'\)是从直线\(LMN\) \DefineConcept{透射的}（\(\triangle ABC\) and \(\triangle A'B'C'\) are \emph{perspective} from \(LMN\)）”；
把直线\(LMN\)称为\DefineConcept{透视轴}（perspective axis，perspectrix）
...
\begin{example}
%@see: 《高等代数与解析几何（第三版 下册）》（孟道骥） P451 习题 4.
