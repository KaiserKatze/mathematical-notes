...
则称\cref{equation:二次型.二次型的矩阵表示} 为二次型\(f(\v{x}{n})\)的\DefineConcept{矩阵表示}，对称矩阵\(\A\)为\(f\)的\DefineConcept{矩阵}，\(\A\)的秩\(r_{\A}\)为\(f\)的\DefineConcept{秩}.
\end{definition}
...
\subsection{矩阵合同的概念}
\begin{definition}
设\(\A\)和\(\B\)是两个\(n\)阶矩阵，若存在可逆矩阵\(\C\)，使得\(\B=\C^T\A\C\)，则称“\(\A\)与\(\B\)\DefineConcept{合同}（congruent）”或“\(\B\)是\(\A\)的\DefineConcept{合同矩阵}”，记为\(\A\simeq\B\).
\end{definition}
...
证明：矩阵\(\A,\B\)合同且相似.
\begin{proof}
显然\(\diag(d_{i_1},d_{i_2},\dotsc,d_{i_n})\)可由\(\diag(\v{d}{n})\)同时左乘和右乘若干个初等矩阵\[
...
\begin{theorem}
对于任一\(n\)元实二次型\(f(\v{x}{n})=\x^T\A\x\)（\(\A=\A^T\)），都存在正交矩阵\(\Q\)，由\(\Q\)构成的线性替换\(\x=\Q\mat{Y}\)（称为\DefineConcept{正交变换}，{\rm orthogonal operator}）将\(f\)化为标准型\[
f(\v{x}{n})
...
\C=\P_1\P_2\dotsm\P_m,
\]那么\[
\C^T\A\C = (\P_m^T\P_{m-1}^T\dotsm\P_1^T)\A(\P_1\P_2\dotsm\P_m)=\diag(\v{d}{n}).
\]
...
\begin{enumerate}
\item \(f(\v{c}{n}) > 0\)，%
则称\(f\)为\DefineConcept{正定的}（positive definite），%
\(\A\)为\DefineConcept{正定矩阵}；
\item \(f(\v{c}{n}) \geqslant 0\)，%
则称\(f\)为\DefineConcept{半正定的}（positive semi-definite）；
\item \(f(\v{c}{n}) < 0\)，%
则称\(f\)为\DefineConcept{负定的}（negative definite），%
\(\A\)为\DefineConcept{负定矩阵}；
\item \(f(\v{c}{n}) \leqslant 0\)，%
则称\(f\)为\DefineConcept{半负定的}（negative semi-definite）；
\item 否则（\(f\)既可取正值，又可取负值），%
称\(f\)为\DefineConcept{不定的}（indefinite）.
\end{enumerate}
\end{definition}
...
\]称为\(f(\v{x}{n})\)的\DefineConcept{规范型}（{\rm normal form}），且规范型是唯一的.
\begin{proof}
...
\begin{definition}
在秩为\(r\)的实二次型\(f(\v{x}{n})\)所化成的标准型（或规范型）中，正平方项的项数\(p\)称为\(f\)的\DefineConcept{正惯性指数}（positive index of inertia），负平方项的项数\(q=r-p\)称为\(f\)的\DefineConcept{负惯性指数}（minus index of inertia），正、负惯性指数之差\(d=p-q=2p-r\)称为\(f\)的\DefineConcept{符号差}（signature）.
\end{definition}
