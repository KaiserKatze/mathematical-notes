...
\begin{example}
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P94 例3.1.2
在\(V = \mathbb{R}^2\)中，
令\(f(\alpha,\beta) \defeq 2 a_1 b_1 + a_1 b_2 + a_2 b_1 + a_2 b_2\)，
其中\(\alpha=(\AutoTuple{a}{2})^T,
\beta=(\AutoTuple{b}{2})^T\).
证明：\(f\)是\(V\)上的一个内积.
%TODO
\end{example}
...
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P94 例3.1.3
在\(V = M_n(\mathbb{R})\)中，
...
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P95 例3.1.4
在\(V = C[a,b]\)中，
...
\begin{example}
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P95 例3.1.5
设\(A\)是\(n\)阶正定矩阵.
在\(V = \mathbb{R}^n\)中，
令\(f(\alpha,\beta) \defeq \alpha^T A \beta\)，
其中\(\alpha=(\AutoTuple{a}{n})^T,
\beta=(\AutoTuple{b}{n})^T\).
证明：\(f\)是\(V\)上的一个内积.
\end{example}

\begin{definition}%\label{definition:欧几里得空间.权函数}
...
\begin{definition}
%@see: 《高等代数（第三版 下册）》（丘维声） P174 定义2
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P93 定义3.1.1
设\(V\)是一个实内积空间.
如果\(V\)是有限维的，
则称“\(V\)是一个\DefineConcept{欧几里得空间}（Euclidean space）”，
简称为\DefineConcept{欧氏空间}；
...
%@see: 《数值分析（第5版）》（李庆扬、王能超、易大义） P53 定理2（柯西-施瓦茨不等式）
在实内积空间\((V,\rho)\)中，
...
对于任意实数\(t\)，
有\begin{equation*}
	% 根据内积的正定性，内积为零当且仅当向量为零，
	% 但是\(t \alpha - \beta \neq 0\)，
	% 所以\(t \alpha - \beta \neq 0\)与它自己的内积\(\rho(t \alpha - \beta, t \alpha - \beta)\)一定大于零
	0 < \rho(t \alpha - \beta, t \alpha - \beta)
	% % 利用内积的线性性，展开并整理得
	% = t \rho(\alpha,t \alpha - \beta) - \rho(\beta, t \alpha - \beta)
	% = t (t \rho(\alpha,\alpha) - \rho(\alpha,\beta)) - (t \rho(\beta,\alpha) - \rho(\beta,\beta))
	% = t^2 \rho(\alpha,\alpha) - t \rho(\alpha,\beta) - t \rho(\beta,\alpha) + \rho(\beta,\beta)
	% % 利用的内积的对称性，即\(\rho(\alpha,\beta) = \rho(\beta,\alpha)\)
	% = t^2 \rho(\alpha,\alpha) - 2t \rho(\alpha,\beta) + \rho(\beta,\beta)
	% % 代入\(\VectorLengthA{\alpha}^2 = \rho(\alpha,\alpha)\)和\(\VectorLengthA{\beta}^2 = \rho(\beta,\beta)\)
...
\)，
% 在实数域上对两个非负数\(\rho^2(\alpha,\beta)\)
% 和\(\VectorLengthA{\alpha}^2 \VectorLengthA{\beta}^2\)开平方，
% 仍旧保持它们的相对大小顺序
即\(
	\abs{\rho(\alpha,\beta)}
	< \VectorLengthA{\alpha} \VectorLengthA{\beta}
...
\begin{property}
在欧几里得空间中，零向量与任意一个向量正交.
%TODO proof
\end{property}

\begin{property}
在欧几里得空间中，与每一个向量都正交的向量一定是零向量.
%TODO proof
\end{property}

\begin{property}
在欧几里得空间中，只有零向量与它本身正交.
%TODO proof
\end{property}
...
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P99 定理3.2.1
在欧几里得空间\((V,\rho)\)中，
任意一个正交向量组一定线性无关.
\begin{proof}
设\(\AutoTuple{\alpha}{s}\)是一个正交向量组.
假设\(\AutoTuple{k}{s} \in \mathbb{R}\)
满足\(k_1 \alpha_1 + \dotsb + k_s \alpha_s = 0\)，
那么对于向量\(\alpha_j\ (j=1,2,\dotsc,s)\)总有\begin{equation*}
	\rho( k_1 \alpha_1 + \dotsb + k_s \alpha_s, \alpha_j )
	= \sum_{i=1}^s k_i \rho(\alpha_i,\alpha_j)
	% 因为当\(i \neq j\)时有\(\rho(\alpha_i,\alpha_j) = 0\)
	= k_j \rho(\alpha_j,\alpha_j)
	= 0,
\end{equation*}
% 因为正交向量组中的每一个向量都是非零向量，
% 所以\(\rho(\alpha_j,\alpha_j) \neq 0\).
% 又因为\(k \vb\alpha = \vb0 \implies k = 0 \lor \vb\alpha = \vb0\)，
从而有\(k_j = 0\ (j=1,2,\dotsc,s)\)，
说明\(\AutoTuple{\alpha}{s}\)是一个线性无关向量组.
\end{proof}
\end{proposition}

\begin{proposition}
设\((V,\rho)\)是一个\(n\)维内积空间，
\(\AutoTuple{\alpha}{s}\)是一个正交向量组，
则\(s \leq n\).
\end{proposition}
\begin{remark}
\(n\)维内积空间中，两两正交的非零向量组最多有\(n\)个向量.
\end{remark}

\begin{definition}
%@see: 《高等代数（第三版 下册）》（丘维声） P176
%@see: 《Linear Algebra Done Right (Fourth Edition)》（Sheldon Axler） P199 6.27
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P100 定义3.2.2
设\((V,\rho)\)是\(n\)维欧几里得空间，
\(A\)是\(V\)的一个基.
如果\(A\)是正交向量组，
...
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P101 定理3.2.2
设\((V,\rho)\)是欧几里得空间，
...
把\begin{equation}\label{equation:欧几里得空间.格拉姆矩阵}
...
则\((\alpha,\beta) = X^T G Y\).
%TODO proof
\end{theorem}

\begin{theorem}
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P97
欧氏空间中任意一个基的度量矩阵是实对称矩阵.
%TODO proof
\end{theorem}

对于欧几里得空间中两个不同的基，它们的度量矩阵是不同的.
\begin{theorem}
...
\begin{definition}
设\(A\)和\(B\)是实数域上的两个\(n\)阶矩阵.
若存在实数域上的一个可逆矩阵\(P\)，
使得\begin{equation*}
	B = P^T A P,
\end{equation*}
则称“\(A\)与\(B\)~\DefineConcept{实相合}”.
\end{definition}

\begin{example}
%@see: 《高等代数（第三版 下册）》（丘维声） P180 习题10.2 10.
%@see: 《数值分析（第5版）》（李庆扬、王能超、易大义） P54 定理3
设\(\AutoTuple{\alpha}{m}\)是\(n\)维欧几里得空间\((V,\rho)\)中一个向量组.
证明：\begin{equation*}
