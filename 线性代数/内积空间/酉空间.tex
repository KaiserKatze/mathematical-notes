...
有\begin{equation*}
	...
	= -f(\alpha,\alpha).
\end{equation*}
若要求\(V\)中任意一个向量\(\alpha\)都满足\(f(\alpha,\alpha) \in \mathbb{R}\)，
...
\begin{equation}\label{equation:酉空间.厄米性}
	...
	[
		...
		~\footnote{
			%@credit: 经过 {c791c701-7186-45bd-86fa-47bac11b3e12},{57e837df-6d92-457b-a5f0-e915ab36f532},{306c719d-5361-46a2-99e4-c3ef565a997c} 的核实，这里的运算顺序是先求内积、再求共轭
			\(\ComplexConjugate{\rho(\beta,\alpha)}\)的运算顺序是先求内积、再取共轭.
		}
	].
\end{equation}
...
如果映射\(\rho\colon V \times V \to \mathbb{C}\)满足以下性质\begin{itemize}
...
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P95 定义3.1.2
...
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P95 例3.1.6
在\(V = \mathbb{C}^n\)中，
...
\begin{definition}
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P99 定义3.2.1
设\(\alpha,\beta\)是复内积空间\((V,\rho)\)中的两个非零向量.
如果\(\rho(\alpha,\beta) = 0\)，
则称“\(\alpha\)与\(\beta\) \DefineConcept{正交}（orthogonal）”，
记为\(\alpha \perp \beta\).
\end{definition}
...
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P99 定义3.2.1
设\((V,\rho)\)是一个有限维酉空间.
如果\(A\)是\(V\)中的一个非零向量组，且\(A\)中的向量两两正交，
即\begin{equation*}
	(\forall \alpha \in A)
	[\alpha\neq0],
	\qquad
...
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P99 定义3.2.1
设\((V,\rho)\)是一个有限维酉空间.
如果\(A\)是\(V\)中的一个单位向量组，且\(A\)中的向量两两正交，
即\begin{equation*}
	(\forall \alpha \in A)
...
或“\(A\)是\((V,\rho)\)中的一个\DefineConcept{标准正交向量组}”.
\end{definition}
...
% \begin{example}
% 设\begin{equation*}
% 	\alpha_1 \defeq (1,-1,\iu,\iu)^T,
% 	\qquad
% 	\alpha_2 \defeq (-1,1,\iu,\iu)^T,
% 	\qquad
% 	\alpha_3 \defeq (1,1,\iu,\iu)^T.
% \end{equation*}
% 求\(\Span\{\alpha_1,\alpha_2,\alpha_3\}\)的一个标准正交基.
% % 解法：先求向量组\(\{\alpha_1,\alpha_2,\alpha_3\}\)的秩，找出一个极大线性无关组，然后对它做施密特正交规范化
% \end{example}
...
\begin{proof}
设\((V,\rho)\)是一个\(n\)维酉空间，
\(\AutoTuple{\eta}{n}\)是\(V\)的一个标准正交基，
向量组\(\AutoTuple{\beta}{n}\)满足\begin{equation*}
%@see: 《高等代数（第三版 下册）》（丘维声） P196 (11)
	(\AutoTuple{\beta}{n})
	= (\AutoTuple{\eta}{n}) P,
\end{equation*}
则\(\beta_i\)在标准正交基\(\AutoTuple{\beta}{n}\)下的坐标
是矩阵\(P\)的第\(i\)列\(X_i\)，
于是\begin{align*}
	&\text{$\AutoTuple{\beta}{n}$是$V$的一个标准正交基} \\
	&\iff \rho(\beta_i,\beta_j) = \delta_{ij}\ (i,j=1,2,\dotsc,n) \\
	&\iff X_j^H X_i = \delta_{ij}\ (i,j=1,2,\dotsc,n) \\
	&\iff
	\begin{bmatrix}
		X_1^H X_1 & X_1^H X_2 & \dots & X_1^H X_n \\
		X_2^H X_1 & X_2^H X_2 & \dots & X_2^H X_n \\
		\vdots & \vdots & & \vdots \\
		X_n^H X_1 & X_n^H X_2 & \dots & X_n^H X_n \\
	\end{bmatrix}
	= \begin{bmatrix}
		1 & 0 & \dots & 0 \\
		0 & 1 & \dots & 0 \\
...
\begin{definition}% 格拉姆矩阵
设\(\AutoTuple{\alpha}{m}\)是\(n\)维酉空间\((V,\rho)\)中一个向量组，
把\begin{equation}%\label{equation:酉空间.格拉姆矩阵}
...
\begin{definition}
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P96
设\(V\)是一个\(n\)维酉空间，
\(\AutoTuple{\alpha}{n}\)是一个基，
把\(\AutoTuple{\alpha}{n}\)的格拉姆矩阵
称为“基\(\AutoTuple{\alpha}{n}\)的\DefineConcept{度量矩阵}”.
\end{definition}
...
设\(V\)是一个\(n\)维酉空间，
\(\AutoTuple{\alpha}{n}\)是一个基，
\(G\)是\(\AutoTuple{\alpha}{n}\)的度量矩阵，
向量\(\alpha,\beta \in V\)在基\(\AutoTuple{\alpha}{n}\)下的坐标分别为\(X,Y\)，
% 即\(X = \VectorMatrix(\alpha), Y = \VectorMatrix(\beta)\)
则\((\alpha,\beta) = X^T G \ComplexConjugate{Y}\).
%TODO proof
\end{theorem}

\begin{theorem}
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P97
酉空间中任意一个基的度量矩阵是厄米矩阵.
%TODO proof
\end{theorem}

对于酉空间中两个不同的基，它们的度量矩阵是不同的.
\begin{theorem}
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P97 定理3.1.1
设\(V\)是一个\(n\)维酉空间，
\(\AutoTuple{\alpha}{n}\)和\(\AutoTuple{\beta}{n}\)是\(V\)的两个基，
\(A,B\)分别是它们的度量矩阵，
\(P\)是基\(\AutoTuple{\alpha}{n}\)到基\(\AutoTuple{\beta}{n}\)的过渡矩阵，
即\begin{equation*}
	(\AutoTuple{\beta}{n})
	= (\AutoTuple{\alpha}{n}) P,
\end{equation*}
...
\begin{definition}
设\(A\)和\(B\)是复数域上的两个\(n\)阶矩阵.
若存在复数域上的一个可逆矩阵\(P\)，
使得\begin{equation*}
	B = P^H A P,
\end{equation*}
则称“\(A\)与\(B\)~\DefineConcept{复相合}”.
\end{definition}
...
%\cref{theorem:矩阵分解.复方阵酉相似于上三角阵}

\begin{theorem}% 舒尔不等式
%@see: 《矩阵分析（第3版）》（史荣昌、魏丰） P115 定理3.6.2（Schur不等式）
设\(\lambda_1,\dotsc,\lambda_n\)是矩阵\(A \in M_n(\mathbb{C})\)的特征值，
则\begin{equation*}
	\sum_{i=1}^n \abs{ \lambda_1 }^2
	\leq \sum_{i=1}^n \sum_{j=1}^n \abs{ a_{ij} }^2,
\end{equation*}
当且仅当\(A\)可以酉相似对角化时，
上式取“\(=\)”号.
\end{theorem}
...
\begin{example}
设\(\alpha \in C^n\)满足\(\alpha^H \alpha = 1\)，
令\(A \defeq E_n - 2 \alpha \alpha^H\).
证明：\(A\)是酉矩阵.
\begin{proof}
因为\(
	(\alpha \alpha^H)^H
	= \alpha \alpha^H,
	(E_n - 2 \alpha \alpha^H)^H
	= (E_n - 2 \alpha \alpha^H)
\)，
所以\begin{align*}
	A^H A
	&= (E_n - 2 \alpha \alpha^H)^H (E_n - 2 \alpha \alpha^H) \\
	&= (E_n - 2 \alpha \alpha^H) (E_n - 2 \alpha \alpha^H) \\
	&= E_n - 4 \alpha \alpha^H + 4 \alpha \alpha^H
	= E_n.
	\qedhere
\end{align*}
\end{proof}
...
\subsection{伴随变换}
\begin{definition}
%@see: 《高等代数学习指导书（第二版 下册）》（丘维声） P642 定义11
设\(\vb{A}\)是酉空间\((V,\rho)\)上的一个线性变换.
如果存在酉空间\((V,\rho)\)上的一个线性变换\(\vb{B}\)
使得\begin{equation*}
	(\forall \alpha,\beta \in V)
	[
		\rho(\vb{A}\alpha,\beta)
...
\end{equation*}
则称“\(\vb{B}\)是\(\vb{A}\)在\(V\)上的一个\DefineConcept{伴随变换}”.
\end{definition}

\subsection{酉相似类}
\begin{definition}
把矩阵\(A \in M_n(K)\)在酉相似关系下的等价类\begin{equation*}
	\Set{ B \in M_n(K) \given \text{$A$与$B$酉相似} }
\end{equation*}
称为“矩阵\(A\)的\DefineConcept{酉相似类}”.
\end{definition}

\begin{example}
设\(A \in M_n(\mathbb{C})\)是厄米矩阵，
则每一个与\(A\)酉相似的矩阵都是一个厄米矩阵.
\begin{proof}
%@credit: {gemini}
设\(A\)是一个厄米矩阵，
则\(A^H = A\).
设矩阵\(B\)与\(A\)酉相似，
则存在酉矩阵\(U \in M_n(\mathbb{C})\)，
使得\(B = U^H A U\).
因为\begin{equation*}
	B^H = (U^H A U)^H
	% (X Y)^H = Y^H X^H
...
\begin{example}
设\(A \in M_n(\mathbb{C})\)是反厄米矩阵，
则每一个与\(A\)酉相似的矩阵都是一个反厄米矩阵.
\begin{proof}
%@credit: {gemini}
设\(A\)是反厄米矩阵，
则\(A^H = -A\).
...
\begin{example}
设\(A \in M_n(\mathbb{C})\)是酉矩阵，
则每一个与\(A\)酉相似的矩阵都是一个酉矩阵.
\begin{proof}
%@credit: {gemini}
设\(A\)是酉矩阵，
则\(A^H A = I\).
设矩阵\(B\)与\(A\)酉相似，
则存在一个酉矩阵\(U \in M_n(\mathbb{C})\)，
使得\(B = U^H A U\).
因为\begin{equation*}
	B^H = (U^H A U)^H
	% (X Y Z)^H = Z^H Y^H X^H
