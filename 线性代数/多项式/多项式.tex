...
如果表达式\begin{equation}\label{equation:多项式.多项式}
...
%@see: 《Linear Algebra Done Right (Fourth Edition)》（Sheldon Axler） P30 2.10
\end{definition}

系数全为零的多项式称为\DefineConcept{零多项式}.
把系数不全为零的多项式称为\DefineConcept{非零多项式}.
在多项式 \labelcref{equation:多项式.多项式} 中，
把\(a_i x^i\)称为“\(i\)次\DefineConcept{项}”，
把\(a_0\)称为\DefineConcept{零次项}或\DefineConcept{常数项}.
只有常数项不为零（即\(a_0 \neq 0\)），
其余项系数全为零（即\(a_i = 0\ (i=1,2,\dotsc)\)）的
多项式称为\DefineConcept{零次多项式}.

从\cref{definition:多项式.多项式的定义} 知道，
数域\(K\)上两个一元多项式相等当且仅当它们的同次项的系数都相等.

%@see: 《Linear Algebra Done Right (Fourth Edition)》（Sheldon Axler） P31 2.11
设\(f(x)\)表示多项式 \labelcref{equation:多项式.多项式}.
如果\(a_n\neq0\)，
...
%@see: 《Linear Algebra Done Right (Fourth Edition)》（Sheldon Axler） P31 2.12
