...
由\(n\)元线性方程组\[
	\vb{A} \vb{x} = \vb\beta
\]的系数和常数项按原位置构成的\(s \times (n+1)\)矩阵\((\vb{A},\vb\beta)\)，
...
\begin{proof}
设\(\vb{A}_{s \times n} \neq \vb0\)，则\(\vb{A}\)经0次或1次交换两行的变换化为\(\vb{B}\)，即\[
...
\]
其中\(b_{1 j_1} \neq 0\).
分别将\(\vb{B}\)的第一行的\(-b_{i j_1}/b_{1 j_1}\)倍加到第\(i\ (i=2,3,\dotsc,s)\)行，则\[
...
若\(b \neq 0\)，则\(\abs{\vb{C}} = 0\)，由归纳假设，齐次线性方程组\[
