...
\begin{definition}
如果存在\(n\)个数\(\AutoTuple{c}{n}\)满足方程组 \labelcref{equation:线性方程组.线性方程组的代数形式}，即\begin{equation*}
	a_{i1} c_1 + a_{12} c_2 + \dotsb + a_{in} c_n \equiv b_i
	\quad(i=1,2,\dotsc,s),
\end{equation*}
则称“方程组 \labelcref{equation:线性方程组.线性方程组的代数形式} \DefineConcept{有解}”，
或“方程组 \labelcref{equation:线性方程组.线性方程组的代数形式} 是\DefineConcept{相容的}”；
否则，称“方程组 \labelcref{equation:线性方程组.线性方程组的代数形式} \DefineConcept{无解}”，
或“方程组 \labelcref{equation:线性方程组.线性方程组的代数形式} 是\DefineConcept{不相容的}”.

称这\(n\)个数\(\AutoTuple{c}{n}\)构成的列向量\((\AutoTuple{c}{n})^T\)为%
方程组 \labelcref{equation:线性方程组.线性方程组的代数形式} 的一个\DefineConcept{解}（solution）%
或\DefineConcept{解向量}（solution vector）.
方程组 \labelcref{equation:线性方程组.线性方程组的代数形式} 的解的全体构成的集合，
称为“方程组 \labelcref{equation:线性方程组.线性方程组的代数形式} 的\DefineConcept{解集}”.
\end{definition}
...
\begin{definition}
%@see: 《矩阵分析与应用（第2版）》（张贤达） P52
方程个数等于未知量个数、具有非奇异系数矩阵的线性方程组，
称为\DefineConcept{适定方程组}（well-determined）.
\end{definition}

\begin{definition}
%@see: 《矩阵分析与应用（第2版）》（张贤达） P52
独立的方程个数少于独立的未知量个数的线性方程组，
称为\DefineConcept{欠定方程组}（under-determined）.
\end{definition}

\begin{definition}
%@see: 《矩阵分析与应用（第2版）》（张贤达） P52
独立的方程个数多于独立的未知量个数的线性方程组，
称为\DefineConcept{超定方程组}（over-determined）.
\end{definition}
...
称为“方程组 \labelcref{equation:线性方程组.线性方程组的代数形式} 的
