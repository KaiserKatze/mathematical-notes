...
记作\(\ExactlyDividedBy{x}{y}\)，
即\begin{equation*}
%@see: 《具体数学 计算机科学基础（第2版）》 P85 (4.1)
	\ExactlyDividedBy{x}{y}
	\defiff
	y > 0
	\land
...
%@see: 《具体数学 计算机科学基础（第2版）》 P85 (4.2)
	(a,b) \defeq \max\Set{ k \given (\ExactlyDividedBy{a}{k}), (\ExactlyDividedBy{b}{k}) }.
\end{equation*}
\end{definition}
\begin{definition}
...
%@see: 《具体数学 计算机科学基础（第2版）》 P86 (4.3)
	[a,b] \defeq \min\Set{ k \given k>0, (\ExactlyDividedBy{k}{a}), (\ExactlyDividedBy{k}{b}) }.
\end{equation*}
\end{definition}
...
即\begin{equation*}
	(x,y) = 1,
\end{equation*}
就称“\(x\)与\(y\) \DefineConcept{互素}（relatively prime）”.
\end{definition}
...
显然，数\(\ExactlyDividedBy{p_1 p_2 \dotsm p_k}{p_i}\ (i=1,2,\dotsc,k)\).
但数\begin{equation*}
...
\begin{definition}
%@see: 《具体数学 计算机科学基础（第2版）》 P91
把\begin{equation*}
	2^p - 1
\end{equation*}称为第\(n\)个\DefineConcept{梅森数}（Mersenne number）.
如果一个梅森数是素数，则将它称为\DefineConcept{梅森素数}.
\end{definition}
...
但\begin{align*}
	...
	&= q + \alpha q = (1+\alpha)q,
\end{align*}
即\(\ExactlyDividedBy{p(m+nq)}{q}\)，说明\(p(m+nq)\)不是一个素数.
\end{proof}
\end{theorem}
...
\begin{proof}
设从正整数\(n\)开始的连续\(r\)个正整数的乘积为\(P_n = n(n+1)(n+2)\dotsm(n+r-1)\)，
那么\begin{gather*}
...
\quad\text{或}\quad
n^p - n = n(n^{p-1}-1) = M,
\end{equation*}即\(\ExactlyDividedBy{n^{p-1}-1}{p}\).
\end{proof}
\end{theorem}
...
如果\(p\)是素数，则\(\ExactlyDividedBy{n^p-n}{p}\).
\end{corollary}
