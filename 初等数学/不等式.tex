...
我们可以按\(a\)的取值分为两种情况讨论：
\begin{itemize}
	\item 当\(a \geqslant 0\)时，\(\abs{a}=a\)，
	原式化为\(-a \leqslant a \leqslant a\)，成立.
...
对于任意实数\(a,b\)，总有\[
	...
	-\abs{b} \leqslant \pm b \leqslant \abs{b}.
\]
将上述两式相加，得\[
	-(\abs{a}+\abs{b}) \leqslant a \pm b \leqslant \abs{a}+\abs{b},
\]即\(\abs{a \pm b} \leqslant \abs{a}+\abs{b}\).
\end{proof}
\end{theorem}
...
设\(\AutoTuple{x}{n},\AutoTuple{y}{n}\geqslant0\)，\(p,q>1\)，且满足\(\frac{1}{p}+\frac{1}{q}=1\)，则
...
设\(\AutoTuple{x}{n},\AutoTuple{y}{n}\in\mathbb{R}\)，\(p\geqslant1\)，则
\def\s{\sum\limits_{i=1}^n}%
