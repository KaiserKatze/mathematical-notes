...
\begin{figure}[ht]
	...
	\begin{tikzpicture}[scale=.5]
		\begin{axis}[
			...
			\addplot[color=blue,samples=50,smooth,domain={-7*pi/2}:{-5*pi/2},variable=\x]{tan(\x r)};
			\addplot[color=blue,samples=50,smooth,domain={-5*pi/2}:{-3*pi/2},variable=\x]{tan(\x r)};
			\addplot[color=blue,samples=50,smooth,domain={-3*pi/2}:{-pi/2},variable=\x]{tan(\x r)};
			\addplot[color=blue,samples=50,smooth,domain={-pi/2}:{pi/2},variable=\x]{tan(\x r)};
			\addplot[color=blue,samples=50,smooth,domain={pi/2}:{3*pi/2},variable=\x]{tan(\x r)};
			\addplot[color=blue,samples=50,smooth,domain={3*pi/2}:{5*pi/2},variable=\x]{tan(\x r)};
			\addplot[color=blue,samples=50,smooth,domain={5*pi/2}:{7*pi/2},variable=\x]{tan(\x r)};
		\end{axis}
	\end{tikzpicture}
	\caption{正切函数的图形}
...
\begin{figure}[ht]
	...
	\begin{tikzpicture}[scale=.5]
		\begin{axis}[
			...
			\addplot[color=blue,samples=50,smooth,domain=-10:-.5]{(exp(x)+exp(-x))/(exp(x)-exp(-x))};
			\addplot[color=blue,samples=50,smooth,domain=.5:10]{(exp(x)+exp(-x))/(exp(x)-exp(-x))};
		\end{axis}
	\end{tikzpicture}
	\caption{双曲余切函数的图形}
...
\end{gather}
这里，
\(\arsinh x\)被称为\DefineConcept{反双曲正弦}，
\(\arcosh x\)被称为\DefineConcept{反双曲余弦}，
\(\artanh x\)被称为\DefineConcept{反双曲正切}，
\(\arcoth y\)被称为\DefineConcept{反双曲余切}.
\end{definition}
