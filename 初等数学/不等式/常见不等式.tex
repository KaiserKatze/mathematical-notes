...
\begin{proof}
因为\begin{align*}
	0 &\leq \sum_{i=1}^n \sum_{j=1}^n (a_i b_j - a_j b_i)^2 \\
	&= \sum_{i=1}^n \sum_{j=1}^n (a_i^2 b_j^2 - 2 a_i b_j a_j b_i + a_j^2 b_i^2) \\
	&= \sum_{i=1}^n a_i^2 \sum_{j=1}^n b_j^2
	- 2 \sum_{i=1}^n a_i b_i \sum_{j=1}^n a_j b_j
	+ \sum_{i=1}^n b_i^2 \sum_{j=1}^n a_j^2 \\
	&= 2 \sum_{i=1}^n a_i^2 \sum_{i=1}^n b_i^2
...
% 若\(\sum_{i=1}^n a_i^2 = 0\)，
% 则\(a_1=a_2=\dotsb=a_n=0\)，
% 此时(1)式成为\(\sum_{i=1}^n b_i^2 \geq 0\)，
% 必定成立.

% 否则，\(\sum_{i=1}^n a_i^2 > 0\)，
% 由于(1)式这个二次三项式非负，
% 因此它的判别式非正，
% 即\[
% 	\left(
...
% 	\right)^2
% 	- 4 \sum_{i=1}^n a_i^2 \sum_{i=1}^n b_i^2
% 	\leq 0,
% \]
% 整理得\[
