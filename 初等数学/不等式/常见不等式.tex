...
\begin{corollary}[反向柯西不等式]\label{theorem:不等式.反向柯西不等式}
	设\(a=(a_1,\cdots ,a_n),b=(b_1,\cdots ,b_n)\)为两列满足条件\(a_1^2-a_2^2-\cdots -a_n^2 \ge 0\)或\(b_1^2-b_2^2-\cdots -b_n^2 \ge 0\)的实数列.那么，不等式
	\begin{equation}
		(a_1^2-a_2^2-\cdots -a_n^2)(b_1^2-b_2^2-\cdots -b_n^2)\le (a_1b_1-a_2b_2-\cdots -a_nb_n)^2
	\end{equation}
	成立.等号成立当且仅当\(a\)和\(b\)成比例.
	\begin{proof}
		可以设\(a_1^2-a_2^2-\cdots -a_n^2=a_{n+1}^2\),\(b_1^2-b_2^2-\cdots -b_n^2=b_{n+1}^2\)\\
		于是我们要证的就变成了
		\begin{equation}
			|a_{n+1}b_{n+1}| \le |a_1b_1-a_2b_2-\cdots -a_nb_n|
		\end{equation}
		只需要证明
		\begin{equation}
			|a_{n+1}b_{n+1}| \le |a_1b_1|-|a_2b_2|-\cdots -|a_nb_n|
		...
		也就是
		\begin{equation}
			\sum_{k=2}^{n+1} |a_kb_k|\le \sqrt{\sum_{k=2}^{n+1}a_k^2\sum_{k=2}^{n+1}b_k^2} 
		\end{equation}
		这就是柯西不等式,Q.E.D.
	\end{proof}
	%@这是 Aczel 不等式.
