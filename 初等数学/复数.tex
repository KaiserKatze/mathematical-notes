...
则\begin{align*}
	z^2
	&= (x + \iu y) (x + \iu y)
	= x (x + \iu y) + \iu y (x + \iu y)
	= x^2 + \iu x y + \iu x y - y^2
	= (x^2 - y^2) + 2 \iu x y, \\
	\ComplexConjugate{z}^2
	&= (x - \iu y) (x - \iu y)
	= x (x - \iu y) - \iu y (x - \iu y)
	= x^2 - \iu x y - \iu x y - y^2
	= (x^2 - y^2) - 2 \iu x y,
\end{align*}
因此\(\ComplexConjugate{z}^2 = \ComplexConjugate{z^2}\).
\end{proof}
\end{property}
...
\begin{example}
证明：\(\ComplexLengthA{z}^2 = z^2\)当且仅当\(\Im z = 0\).
\begin{proof}
设\(z = x + \iu y\)，
则\begin{align*}
	\ComplexLengthA{z}^2
	...
	= (x^2 - y^2) + 2 \iu x y.
\end{align*}
而\(\ComplexLengthA{z}^2 - z^2 = 2 y^2 - 2 \iu x y = 0\)
当且仅当\(2 y^2 = 0\)和\(2 x y = 0\)同时成立.
因此\(\ComplexLengthA{z}^2 = z^2\)
当且仅当\(y = \Im z = 0\).
\end{proof}
\end{example}
