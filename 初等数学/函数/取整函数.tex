\section{取整函数}
%@see: https://mathworld.wolfram.com/NearestIntegerFunction.html
...
定义：\begin{itemize}
	...
	\footnote{
		有的书把不大于\(x\)的最大整数
		称为“\(x\)的\DefineConcept{整数部分}（integer part）”，记作\([x]\)；
		%@see: https://mathworld.wolfram.com/IntegerPart.html
		把\(x\)与其整数部分之差\(x - [x]\)
		称为“\(x\)的\DefineConcept{小数部分}（fractional part）”，记作\(\{x\}\).
		%@see: https://mathworld.wolfram.com/FractionalPart.html
		有的书把\begin{equation*}
			[x]
			\defeq
			\begin{cases}
			...
			\end{cases}
		\end{equation*}
		称为\(x\)的\DefineConcept{整数部分}，
		把\(\{x\} \defeq x - [x]\)称为\(x\)的\DefineConcept{小数部分}.
	}.
	我们把函数\(y=\floor{x}\)称为\DefineConcept{向下取整函数}（floor function），
	%@see: https://mathworld.wolfram.com/FloorFunction.html
	即\begin{equation}
		\floor{x}
		\defeq
	...
	即\(x\in(n-1,n]\)，
	那么记\(\ceil{x}=n\).
	我们把函数\(y=\ceil{x}\)称为\DefineConcept{向上取整函数}（ceiling function），
	%@see: https://mathworld.wolfram.com/CeilingFunction.html
	即\begin{equation}
		\ceil{x}
		\defeq
...
对于任意\(x\in\mathbb{R}\)，总有\begin{equation}
...
把序列\begin{equation*}
	...
	\dotsc,
\end{equation*}
称为“实数\(x\)的\DefineConcept{谱}（spectrum）”，
