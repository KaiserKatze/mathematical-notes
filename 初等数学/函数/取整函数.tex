...
\begin{figure}
	...
	\def\subwidth{.45\linewidth}
	\def\subscale{.9}
	%@Mathematica: Plot[Floor[x] - x, {x, -1, 4}]
	%@Mathematica: Table[{x, Floor[x] - x}, {x, -1, 4, .2}] // TableForm
	\begin{subfigure}[b]{\subwidth}%
		\centering
		\begin{tikzpicture}[scale=\subscale]
			\begin{axis}[
				...
				\foreach \i in {-1,...,3} {
					\addplot[color=orange,samples=2,domain={\i}:{\i+1}]{\i-x};
				}
