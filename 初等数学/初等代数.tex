...
以后我们用符号\(A_n^k\)表示从\(n\)个元素一次取出\(k\)个的排列数，即\[
...
\]于是\[
	C_n^k = \frac{A_n^k}{k!}
	= \frac{n(n-1)(n-2)\dotsm(n-k+2)(n-k+1)}{k!}
	= \frac{n!}{k! (n-k)!}.
\]
...
需要注意到，当\(k=n\)时，有\[
	A_n^n = \frac{n!}{0!} = n!,
\]\[
	C_n^n = \frac{n!}{n! 0!} = \frac{1}{0!};
\]
这说明，符号\(0!\)的取值应该等于\(1\).
这就是为什么我们在\hyperref[definition:数列.阶乘的定义]{阶乘的定义}中特别规定\(0!\equiv1\)的原因.
