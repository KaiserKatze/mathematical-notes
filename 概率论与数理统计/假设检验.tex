...
这里我们把\(H_0\)称为\DefineConcept{原假设}.
...
在这里，备择假设还有另外两种设置形式，它们分别是\[
	...
	H_1'' \defeq \Set{ \mu \given \mu > \mu_0 }.
\]
我们把\(H_1'\)称为“\DefineConcept{左侧检验}的备择假设”，
把\(H_1''\)称为“\DefineConcept{右侧检验}的备择假设”.
相对地，把\(H_1\)称为“\DefineConcept{双侧检验}的备择假设”.
我们常把左侧检验和右侧检验统称为\DefineConcept{单侧检验}.
下面我们暂且采用\(H_1\)作为备选假设.
...
接下来我们就会取一个很小的正数\(\alpha\)（如0.01、0.05），
并称其为\DefineConcept{显著性水平}.
然后利用\[
	P(\abs{\hat{\mu} - \mu_0} > M) = \alpha
\]解出\(M\)，称之为\DefineConcept{临界点}.
于是，如果有\[
