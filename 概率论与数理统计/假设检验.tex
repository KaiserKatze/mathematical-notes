...
这类问题是数理统计的另一个基本内容，我们将其称为\DefineConcept{假设检验}.
...
\]
其中\(H_0\)称为\DefineConcept{原假设}，
\(H_1\)称为\DefineConcept{备选假设}.
...
假设检验又叫\DefineConcept{显著性检验}，
其中\(U\)叫做\DefineConcept{检验统计量}；
\(u_{1-\frac{\alpha}{2}}\)叫做临界值；
\(W=\{\abs{U}>u_{1-\frac{\alpha}{2}}\}\)叫做检验的\DefineConcept{拒绝域}.
...
\begin{figure}[ht]
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			\def\plotNDPDF#1#2#3{\addplot[color=#3,samples=100,smooth,domain=-7:7]%
