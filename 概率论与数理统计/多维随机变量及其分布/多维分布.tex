...
\begin{definition}
设\((\AutoTuple{X}{n})\)为\(n\)维随机变量.
把\(n\)元函数\begin{equation*}
	F(\AutoTuple{x}{n})
	= P(X_1 \leq x_1,X_2 \leq x_2,\dotsc,X_n \leq x_n)
\end{equation*}
称为“随机变量\((\AutoTuple{X}{n})\)的\(n\)维\DefineConcept{分布函数}”.
\end{definition}

\begin{definition}
记\(F_i\)为\(X_i\)的边缘分布函数.
若对任意实数\(\AutoTuple{x}{n}\)，
有\begin{equation*}
