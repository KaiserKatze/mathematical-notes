...
设\(X_i\ (i=1,2,\dotsc,n)\)独立同分布于\(0-1\)分布\(B(1,p)\)，则\begin{equation*}
...
\begin{example}
设\(\AutoTuple{X}{n}\)独立同分布于\(N(\mu,\sigma^2)\)，试计算其算术平均数\begin{equation*}
	\overline{X} = \frac{1}{n} (X_1+X_2+\dotsb+X_n)
\end{equation*}的分布.
\begin{solution}
由\hyperref[theorem:正态分布与自然指数分布族.正态分布的可加性2]{正态分布的卷积公式}可知\begin{equation*}
	X_1+X_2+\dotsb+X_n \sim N(n\mu,n\sigma^2).
\end{equation*}又由\hyperref[theorem:正态分布与自然指数分布族.正态分布的线性性质]{正态分布的线性性}可知\begin{equation*}
