\section{常见统计分布}
\(\chi^2\)分布、\(t\)分布和\(F\)分布在数理统计中与正态分布一起构成四个最基本、最重要的分布.
下面逐一介绍前三个分布.

\subsection{\texorpdfstring{\(\chi^2\)}{卡方}分布}
...
\end{equation}
所服从的分布为
“自由度为\(n\)的 \DefineConcept{\(\chi^2\)分布}”
\footnote{念作“卡方分布”.}，
...
\begin{figure}[htb]
	...
	\begin{tikzpicture}
		...
		at(ChiSquareDistribution.north east){\small\begin{tabular}{cl}
			\ref{pgfplots:卡方分布.Chi^2(.5)} & \(\chi^2(0.5)\) \\
			\ref{pgfplots:卡方分布.Chi^2(3)} & \(\chi^2(3)\) \\
			\ref{pgfplots:卡方分布.Chi^2(5)} & \(\chi^2(5)\) \\
		\end{tabular}};
	\end{tikzpicture}
	\caption{\(\chi^2\)分布的密度函数}
\end{figure}
