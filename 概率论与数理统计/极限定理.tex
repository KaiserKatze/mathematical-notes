...
\begin{definition}
对于一个随机变量序列\(\{X_n\}\)，
记\(\overline{X} = \frac{1}{n} \sum_{i=1}^n{X_i}\)，若\[
\overline{X} - \frac{1}{n} \sum_{i=1}^n{E(X_i)} \overset{P}{\longrightarrow} 0,
...
\frac{1}{n} \sum_{k=1}^n{X_k} - \frac{1}{n} \sum_{k=1}^n{E(X_k)} \overset{P}{\longrightarrow} 0.
\]
\begin{proof}
\def\Yn{\frac{1}{n} \sum_{k=1}^n{X_k}}
令\(Y_n=\Yn\)，则\[
\mu_n = E(Y_n) = E\left(\Yn\right) = \frac{1}{n} \sum_{k=1}^n{E(X_k)},
\]\[
\sigma_n^2 = D(Y_n) = D\left(\Yn\right) = \frac{1}{n^2} \sum_{k=1}^n{D(X_k)} \leq \frac{C}{n},
\]
...
当\(D(X_k)=\sigma^2\)存在时，由切比雪夫不等式 \labelcref{equation:极限定理.切比雪夫不等式2} 可知，对\(\forall\epsilon>0\)，有\[
