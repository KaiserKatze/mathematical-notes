...
\begin{definition}
设随机变量序列\(\{X_n\}\)相互独立，且它们都服从相同的分布\(F\)，
则称“\(\{X_n\}\) \DefineConcept{独立同分布于} \(F\)”；
也称“\(\{X_n\}\)是\DefineConcept{独立同分布随机变量}（independently and identically
...
我们也常把\hyperref[theorem:极限定理.大数律.辛钦大数律]{辛钦大数律}称作\DefineConcept{弱大数律}.
...
设\(\{X_n\}\)是独立随机变量序列.
记\[
	Y_n \defeq \frac{1}{B_n} \sum_{i=1}^n (X_i-\mu_i).
\]
如果\begin{enumerate}
