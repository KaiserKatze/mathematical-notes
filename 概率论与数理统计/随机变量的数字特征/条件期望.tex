...
条件期望的期望就是无条件期望，
即\begin{equation}\label{equation:条件期望.全期望公式}
...
我们把\cref{equation:条件期望.全期望公式}
称为\DefineConcept{全期望公式}（law of total expectation），
\DefineConcept{亚当定律}（Adam's law）.
%@see: https://en.wikipedia.org/wiki/Law_of_total_expectation
%@see: https://zhuanlan.zhihu.com/p/1987904069281547304

%@see: https://proofwiki.org/wiki/Tower_Property_of_Conditional_Expectation
...
\begin{theorem}
设\(X,Y\)是随机变量，
则\begin{equation}\label{equation:条件期望.全方差公式}
	D(X)
	= E(D(X \vert Y))
	+ D(E(X \vert Y)).
\end{equation}
...
我们把\cref{equation:条件期望.全方差公式}
称为\DefineConcept{全方差公式}（law of total variance）
或\DefineConcept{夏娃定律}（Eve's law）.
