...
\begin{enumerate}
\item \(f(x,y) \geqslant 0, \quad (x,y) \in \mathbb{R}^2\);
...
\]存在，则称该极限为“连续型随机变量\(Y=y\)条件下\(X\)的\textbf{条件分布函数}”，记为\break\(F_{X \vert Y}(x \vert y)\)（或\(P(X \leqslant x \vert Y = y)\)）.

类似地，可以定义\(F_{Y \vert X}(y \vert x)\)（或\(P(Y \leqslant y \vert X = x)\)）.
\end{definition}
