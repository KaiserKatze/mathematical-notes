...
\begin{figure}
	...
	\begin{tikzpicture}
		...
		\begin{scope}[>=Stealth,->]
			...
			\draw(0,0)--(0,4)node[left]{\(y\)};
		\end{scope}
		\pgfmathsetmacro{\pa}{3.1}
		\pgfmathsetmacro{\pb}{\a*\pa/\z}
		\draw(0,0)--(\z,\a)--(\pa,\pb)
			node[left=.5cm]{\(\begin{array}{l}
			x=yz \\
			0<z<1
			\end{array}\)};
		\draw[dashed](\z,0)node[below]{\(z\)}--(\z,\a);
		...
		\begin{scope}[xshift=6cm]
			...
			\begin{scope}[>=Stealth,->]
				...
				\draw(0,0)--(0,4)node[left]{\(y\)};
			\end{scope}
			\pgfmathsetmacro{\pa}{3.5}
			\pgfmathsetmacro{\pb}{\pa/\z}
			\draw(0,0)--(\a,\a/\z)--(\pa,\pb)
				node[above right]{\(\begin{array}{l}
				x=yz \\
				z\geqslant1
				\end{array}\)};
