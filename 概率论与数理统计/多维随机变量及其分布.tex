...
\end{align}
\rm
\cref{equation:多维随机变量及其分布.离散型随机变量的卷积公式1,%
...
\(Z\)的值域为\(R(Z)=(0,+\infty)\).
对\(\forall z>0\)，有\begin{align*}
	F_Z(z) &= P(Z \leqslant z) = P\left(\frac{X}{Y} \leqslant z\right) \\
	&= \iint_{\frac{x}{y} \leqslant z} f(x,y) \dd{x}\dd{y}.
\end{align*}
\begin{figure}
	\centering
	\begin{tikzpicture}
...
\end{align}
\rm
\cref{equation:多维随机变量及其分布.连续型随机变量的卷积公式1,%
...
则称\((\v{X}{n})\)为\(n\)维\DefineConcept{随机变量}.
\end{definition}

\begin{definition}
设\((\v{X}{n})\)为\(n\)维\DefineConcept{随机变量}，%
称\(n\)元函数\[
F(\v{x}{n})
= P(X_1 \leqslant x_1,X_2 \leqslant x_2,\dotsc,X_n \leqslant x_n)
\]为\((\v{X}{n})\)的\(n\)维\DefineConcept{分布函数}.
\end{definition}
...
则\((\v{X}{n})\)所服从的分布称为\DefineConcept{多项分布}，%
...
\]则称\((\v{X}{n})\)是\DefineConcept{ \(n\)维连续型随机变量}，称\(f\)为\(\mat{\Xi}\)的\DefineConcept{ \(n\)维概率密度函数}.
\end{definition}
...
\end{array} \right.
\]其中\(m(G)\)为\(G\)的度量，%
称\((\v{X}{n})\)服从\(G\)上的\DefineConcept{均匀分布}.
\end{definition}

\section{分布的可加性}
\begin{definition}
当\(\v{X}{n}\)相互独立且具有同一类型分布时，若\(X_1+X_2+\dotsb+X_n\)也服从这一类型的分布，就称这种类型的分布具有\DefineConcept{可加性}.
\end{definition}
