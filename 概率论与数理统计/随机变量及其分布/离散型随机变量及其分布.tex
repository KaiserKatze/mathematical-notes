...
\end{equation}
其中\(0 < p < 1\)，\(q = 1-p\)，
则称\(X\)服从\DefineConcept{二项分布}（binomial distribution），
%@see: https://mathworld.wolfram.com/BinomialDistribution.html
记为\(X \sim B(n,p)\).

特别地，当\(n = 1\)时，二项分布\(B(1,p)\)称为~\DefineConcept{0-1分布}或\DefineConcept{伯努利分布}（Bernoulli distribution），
%@see: https://mathworld.wolfram.com/BernoulliDistribution.html
即\begin{equation*}
