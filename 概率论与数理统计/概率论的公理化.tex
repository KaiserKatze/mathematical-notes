...
如果\begin{itemize}
	...
	\item \(\mathcal{F}\)对取余集运算封闭
	（即\(A \in \mathcal{F}\)
	蕴含\(\SetComplementaryL{A} \in \mathcal{F}\)，
	其中\(\SetComplementaryL{A} \defeq \Omega - A\)）；

	\item \(\mathcal{F}\)对可列并封闭
	（即\(A_n \in \mathcal{F}\ (n \in \mathbb{N})\)
	蕴含\(\bigcup_{n=1}^\infty A_n \in \mathcal{F}\)），
\end{itemize}
