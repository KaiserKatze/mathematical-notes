...
\begin{definition}
设随机变量\(\v{X}{n}\)与总体\(X\)独立同分布，则称“\(\v{X}{n}\)为一样本容量为\(n\)的来自总体\(X\)的\DefineConcept{（简单随机）样本}”.
而\(\v{X}{n}\)的取值\(\v{x}{n}\)叫做\DefineConcept{样本观测值}.
\end{definition}
...
\begin{definition}
\def\g#1{g(\v{#1}{n})}
样本\(\v{X}{n}\)的一个连续函数\(\g{X}\)称为\DefineConcept{样本函数}.
