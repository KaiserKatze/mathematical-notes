...
假设求积公式具有\(2n+1\)次代数精度，
那么对于任意一个\(2n+1\)次多项式函数\(f\)有余项\(R[f]\)等于零，
这时\(x \mapsto f^{(n+1)}(\xi(x))\)是一个\(n\)次多项式函数.
若要求对于任意\(p \in H_n[a,b]\)，总有\begin{equation*}
	\int_a^b \rho(x) ~ p(x) ~ \omega_{n+1}(x) \dd{x} = 0,
\end{equation*}
就相当于要求\(\omega_{n+1}\)与每一个次数不超过\(n\)的多项式函数\(p\)
在区间\([a,b]\)上带权\(\rho\)正交，
也相当于要求所有以节点\(x_k\ (k=0,1,2,\dotsc,n)\)为零点的\(n+1\)次多项式函数的集合\(\{\omega_{n+1}\}\)是\([a,b]\)上带权\(\rho\)的正交多项式函数族.
\begin{theorem}
...
\(n\)次\hyperref[equation:拉格朗日插值.拉格朗日插值基函数]{插值基函数}，
...
于是高斯求积公式的求积系数\(A_0,A_1,\dotsc,A_n\)有两种计算方法：\begin{enumerate}
	\item 在已知求积节点\(x_0,x_1,\dotsc,x_n\)以后，
	再利用方程组 \labelcref{equation:高斯求积公式.高斯求积公式的待定参数方程组}
	求解求积系数；
	\item 直接利用\cref{equation:高斯求积公式.高斯求积公式的求积系数} 计算得到.
\end{enumerate}
