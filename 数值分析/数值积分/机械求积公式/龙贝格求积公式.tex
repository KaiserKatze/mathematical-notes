...
如果这时我们发现精度不够，
则将区间\([a,b]\)划分为\(2n\)等份，
在\(x_k\)与\(x_{k+1}\)之间新增分点\(
	x_{k+1/2}
	\defeq (x_k + x_{k+1}) / 2
	= a + k h + h/2
\)（\(k=0,1,2,\dotsc,n-1\)），
这时候节点个数由\(n+1\)个增加到\(2n+1\)个，
运用复合梯形公式\begin{equation*}
	T_{2n}
	= \frac{h'}{2}
	\left(
...
\end{align*}
这样我们就得到了\(T_n\)的递推公式\begin{equation}\label{equation:龙贝格求积公式.梯形值递推公式}
...
用\(T_0^{(k)}\)表示将积分区间\([a,b]\)二分\(k\)次后求得的梯形值\(T_0\left( \frac{b-a}{2^k} \right)\)，
用\(T_m^{(k)}\)表示序列\(\{T_0^{(k)}\}\)的\(m\)次加速值，
则根据上述递推公式可得\begin{equation}\label{equation:龙贝格求积公式.龙贝格求积算法}
%@see: 《数值分析（第5版）》（李庆扬、王能超、易大义） P112 (4.11)
	T_m^{(k)}
...
\begin{algorithm}[龙贝格求积算法]
\hfill
\begin{enumerate}
	\item
	欲求积分\(I \defeq \int_a^b f(x) \dd{x}\)，
	取步长\(h \defeq b - a\)，
	初始化区间均分次数\(k \defeq 0\)，
	建立T表：\begin{center}
		\begin{tblr}{c|c|*6c}
			\hline
			\(k\) & \(h\) & \(T_0^{(k)}\) & \(T_1^{(k)}\) & \(T_2^{(k)}\) & \(T_3^{(k)}\) & \(T_4^{(k)}\) & \(\cdots\) \\
			\hline
			0 & \(b-a\) & \(T_0^{(0)}\) \\
			1 & \(\frac{1}{2} (b-a)\) & \(T_0^{(1)}\) & \(T_1^{(0)}\) \\
			2 & \(\frac{1}{4} (b-a)\) & \(T_0^{(2)}\) & \(T_1^{(1)}\) & \(T_2^{(0)}\) \\
			3 & \(\frac{1}{8} (b-a)\) & \(T_0^{(3)}\) & \(T_1^{(2)}\) & \(T_2^{(1)}\) & \(T_3^{(0)}\) \\
			4 & \(\frac{1}{16} (b-a)\) & \(T_0^{(4)}\) & \(T_1^{(3)}\) & \(T_2^{(2)}\) & \(T_3^{(1)}\) & \(T_4^{(0)}\) \\
	...
	求\(
		T_0^{(0)}
		\defeq
		% = T_0\left( \frac{b-a}{2^0} \right)
		% = T_0(b-a)
		% = T_1
		\frac{h}{2} ( f(a) + f(b) )
	\)，
	把\(T_0^{(0)}\)填入T表第\(1\)行第\(3\)列；
	...
	令\(
		...
		k \defeq k+1
	\)，
	利用\cref{equation:龙贝格求积公式.梯形值递推公式}
	计算梯形值\begin{equation*}
		T_0^{(k)}
		= \frac{1}{2} T_0^{(k-1)} + \frac{h}{2} \sum_{i=0}^{n-1} f(x_{i+1/2}),
	\end{equation*}
	其中\(
		x_{i+1/2}
		\defeq a + (i + \frac12) h
		\ (i=0,1,2,\dotsc,n-1)
	\)，
	把\(T_0^{(k)}\)填入T表第\(k+1\)行第\(3\)列；

	\item
	利用\cref{equation:龙贝格求积公式.龙贝格求积算法}
	计算加速值\begin{equation*}
		T_j^{(k-j)}
		= \frac{4^j}{4^j - 1} T_{j-1}^{(k-j+1)}
		- \frac{1}{4^j - 1} T_{j-1}^{(k-j)}
		\quad(j=1,2,\dotsc,k),
	\end{equation*}
	把\(T_j^{(k-j)}\)填入T表第\(k+1\)行第\(j+3\)列；

	\item
	判断近似值是否达到预期精度\(\epsilon\)（例如\(\epsilon = 10^{-6}\)）：
	\begin{itemize}
		\item
		如果\(\abs{ T_k^{(0)} - T_{k-1}^{(0)} } < \epsilon\)，
		则终止计算，用\(T_k^{(0)}\)近似积分值\(I\)；
		\item
		否则，令\(h \defeq h/2\)，回到第3步，继续计算.
	\end{itemize}
\end{enumerate}
\end{algorithm}
