...
\begin{enumerate}
	...
	\item
	求\(T_0^{(0)} \defeq \frac{h}{2} ( f(a) + f(b) )\)，
	再令\(k \defeq k+1\)；
	...
	\end{equation*}
	计算梯形值\(T_0^{(k)} = T_0\left( \frac{b-a}{2^k} \right)\)；
	...
	\begin{equation*}
		...
		\quad(k=1,2,\dotsc),
	\end{equation*}
	计算加速值\(T_j^{(k-j)}\ (j=1,2,\dotsc,k)\)（填入T表的第\(k\)行）；
