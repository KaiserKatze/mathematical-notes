...
当数学模型不能得到精确解时，
通常需要用数值方法求它的近似解，
其近似解与精确解之间的误差称为\DefineConcept{截断误差}（truncation error）
或\DefineConcept{方法误差}.
%@see: https://en.wikipedia.org/wiki/Truncation_error
...
都会产生\DefineConcept{舍入误差}（round-off error，rounding error）.
%@see: https://en.wikipedia.org/wiki/Round-off_error
%@see: https://mathworld.wolfram.com/RoundoffError.html
...
则称\begin{equation}
	e^* \defeq x^* - x
\end{equation}
是近似值的\DefineConcept{绝对误差}（absolute error）.
%@see: https://mathworld.wolfram.com/AbsoluteError.html

通常我们不能算出准确值\(x\)，
也不能算出误差\(e^*\)的准确值，
只能根据测量工具或计算情况估计出误差的绝对值不超过某个正数\(\epsilon^*\)，
也就是误差绝对值的一个上界.
...
我们把准确值的误差\(e^*\)与准确值\(x\)的比值\begin{equation}
	...
	= \frac{x^* - x}{x}
\end{equation}
称为近似值\(x^*\)的\DefineConcept{相对误差}（relative error）.
%@see: https://mathworld.wolfram.com/RelativeError.html
...
则称“\(x^*\)有\(n\)位\DefineConcept{有效数字}（\(x^*\) has \(n\) \emph{significant digits}）”.
%@see: https://mathworld.wolfram.com/SignificantDigits.html
...
%TODO
%@see: https://mathworld.wolfram.com/ErrorPropagation.html
%@see: https://mathworld.wolfram.com/SignificanceArithmetic.html
