...
则\begin{equation}\label{equation:牛顿插值.莱布尼茨公式}
...
于是\begin{align*}
	...
	% 因为当\(i=k\)时有\(x_k - x_i = 0\)，所以要修改求和上限
	&= \sum_{i=0}^{k-1} (x_k - x_i) \cdot ( u[x_0,x_1,\dotsc,x_i] ) \cdot ( v[x_i,x_{i+1},\dotsc,x_k] ) \\
	% 因为当\(i=0\)时有\(x_i - x_0 = 0\)，所以要修改求和下限
	&\hspace{20pt}
		+ \sum_{i=1}^k (x_i - x_0) \cdot ( u[x_0,x_1,\dotsc,x_i] ) \cdot ( v[x_i,x_{i+1},\dotsc,x_k] ) \\
	&= \sum_{i=0}^{k-1} ( u[x_0,x_1,\dotsc,x_i] ) \cdot ( v[x_{i+1},\dotsc,x_k] - v[x_i,\dotsc,x_{k-1}] ) \\
	&\hspace{20pt}
		+ \sum_{i=1}^k ( u[x_1,\dotsc,x_i] - u[x_0,\dotsc,x_{i-1}] ) \cdot ( v[x_i,x_{i+1},\dotsc,x_k] ) \\
	&= \sum_{i=0}^{k-1} ( u[x_0,x_1,\dotsc,x_i] ) \cdot ( v[x_{i+1},\dotsc,x_k] ) \\
	&\hspace{20pt}
		- \sum_{i=0}^{k-1} ( u[x_0,x_1,\dotsc,x_i] ) \cdot ( v[x_i,\dotsc,x_{k-1}] ) \\
	&\hspace{20pt}
		+ \sum_{i=1}^k ( u[x_1,\dotsc,x_i] ) \cdot ( v[x_i,x_{i+1},\dotsc,x_k] ) \\
	&\hspace{20pt}
		- \sum_{i=1}^k ( u[x_0,\dotsc,x_{i-1}] ) \cdot ( v[x_i,x_{i+1},\dotsc,x_k] ) \\
	&= \sum_{i=0}^{k-1} ( u[x_0,x_1,\dotsc,x_i] ) \cdot ( v[x_{i+1},\dotsc,x_k] ) \\
	&\hspace{20pt}
		- \sum_{i=0}^{k-1} ( u[x_0,x_1,\dotsc,x_i] ) \cdot ( v[x_i,\dotsc,x_{k-1}] ) \\
	&\hspace{20pt}
		+ \sum_{i=1}^k ( u[x_1,\dotsc,x_i] ) \cdot ( v[x_i,x_{i+1},\dotsc,x_k] ) \\
	% 在第四个求和中令\(j \defeq i-1\)，那么当\(i=1\)时有\(j=0\)，当\(i=k\)时有\(j=k-1\)
	&\hspace{20pt}
		- \sum_{j=0}^{k-1} ( u[x_0,\dotsc,x_j] ) \cdot ( v[x_{j+1},x_{j+2},\dotsc,x_k] ) \\
...
\begin{example}\label{example:牛顿插值.幂函数的k阶差商就是完全齐次对称多项式}
%@credit: {gemini}
证明：幂函数\(f_m(x) \defeq x^m\ (m\in\mathbb{N}^+)\)
关于\(k+1\ (k\geq0)\)个点\(x_0,\dotsc,x_k\)的\(k\)阶差商\(f_m[x_0,\dotsc,x_k]\)
是关于点\(x_0,\dotsc,x_k\)的\(m-k\)次完全齐次对称多项式.
\begin{proof}
记\(H_d(x_0,\dotsc,x_k)\)为关于变量\(x_0,\dotsc,x_k\)的\(d\)次完全齐次对称多项式，
即\begin{equation*}
	H_d(x_0,\dotsc,x_k)
	\defeq
	\sum_{\substack{p_0 + \dotsb + p_k = d \\ p_0,\dotsc,p_k \geq 0}}
	x_0^{p_0} \dotsm x_k^{p_k}.
\end{equation*}
记\(
	a \defeq \min\{x_0,\dotsc,x_k\},
	b \defeq \max\{x_0,\dotsc,x_k\}
\)，
那么由\cref{equation:牛顿插值.差商与导数的关系} 可知，
存在\(\xi \in [a,b]\)
使得\begin{equation*}
	f_m[x_0,\dotsc,x_k]
	= \frac{f_m^{(k)}(\xi)}{k!}
	= \begin{cases}
		\frac{m!}{k!(m-k)!} \xi^{m-k},	& k \leq m, \\
		0,								& k > m.
	\end{cases}
	\eqno(1)
\end{equation*}
下面证明\begin{equation*}
	f_m[x_0,\dotsc,x_k] = H_{m-k}(x_0,\dotsc,x_k).
	\eqno(2)
\end{equation*}
...
从而有(2)式成立；
如果\(k > 0 = m\)，
则由(1)式可知\(f_m[x_0,\dotsc,x_k] = 0\)，
又因为\(d \defeq m-k < 0\)时成立\(H_d(x_0,\dotsc,x_k) = 0\)，
所以(2)式仍然成立.

假设\(m-1\)次幂函数\(f_{m-1}\)也满足(2)式.
接下来考虑\(m\)次幂函数\(f_m\).
由\hyperref[equation:牛顿插值.莱布尼茨公式]{莱布尼茨公式}有\begin{align*}
    f_m[x_0,\dotsc,x_k]
	&= (f_1 \cdot f_{m-1})[x_0,\dotsc,x_k] \\
	&= \sum_{j=0}^k ( f_1[x_0,\dotsc,x_j] ) \cdot ( f_{m-1}[x_j,\dotsc,x_k] ).
	\tag3
\end{align*}
注意到\(f_1[x_0]=x_0\)，\(f_1[x_0,x_1]=1\)，且当\(j \geq 2\)时\(f_1[x_0,\dotsc,x_j] = 0\).
因此(3)式中仅前两项非零，从而有\begin{equation*}
    f_m[x_0,\dotsc,x_k]
    = x_0 \cdot f_{m-1}[x_0,\dotsc,x_k] + 1 \cdot f_{m-1}[x_1,\dotsc,x_k].
\end{equation*}
利用归纳假设，将\(f_{m-1}\)的差商替换为完全齐次对称多项式，得\begin{align*}
    f_m[x_0,\dotsc,x_k]
	&= x_0 \cdot H_{(m-1)-k}(x_0,\dotsc,x_k)
	+ H_{(m-1)-(k-1)}(x_1,\dotsc,x_k) \\
	&= x_0 H_{m-k-1}(x_0,\dotsc,x_k) + H_{m-k}(x_1,\dotsc,x_k).
	\tag4
\end{align*}
另一方面，完全齐次对称多项式满足如下递归性质\begin{equation*}
	%TODO proof 将包含\(x_0\)的项与不包含\(x_0\)的项分离
    H_d(x_0,\dotsc,x_k) = x_0 H_{d-1}(x_0,\dotsc,x_k) + H_d(x_1,\dotsc,x_k).
	\eqno(5)
\end{equation*}
在(5)式中令\(d = m-k\)，得\begin{equation*}
    H_{m-k}(x_0,\dotsc,x_k)
	= x_0 H_{m-k-1}(x_0,\dotsc,x_k) + H_{m-k}(x_1,\dotsc,x_k).
	\eqno(6)
\end{equation*}
比较(4)式与(6)式可知，(2)式仍然成立.
\end{proof}
\end{example}

\begin{example}\label{example:牛顿插值.幂函数的插值余项的中值}
设\(f(x) \defeq x^{2n+1}\ (a \leq x \leq b)\)，
给定\(a \leq x_0 < x_1 < \dotsb \leq x_n \leq b\)，
令\begin{equation*}
	L_n(x) \defeq \sum_{k=0}^n f(x_k) ~ l_k(x),
\end{equation*}
其中\begin{equation*}
	l_k(x)
	\defeq \prod_{0 \leq i \leq n, i \neq k} \frac{(x - x_i)}{(x_k - x_i)}
	\quad(k=0,1,2,\dotsc,n).
\end{equation*}
解关于\(\xi\)的方程\begin{equation*}
	f(x) - L_n(x)
	= \frac{f^{(n+1)}(\xi)}{(n+1)!} \omega_{n+1}(x),
\end{equation*}
把\(\xi\)表示为\(x\)的函数.
\begin{solution}
由\cref{equation:牛顿插值.差商与导数的关系} 可知\begin{equation*}
	f[x_0,x_1,\dotsc,x_n,x]
	= \frac{f^{(n+1)}(\xi)}{(n+1)!}
	= \frac{(2n+1)!}{(n+1)!n!} \xi^n,
\end{equation*}
解得\begin{equation*}
	\xi^n
	= \frac{n!(n+1)!}{(2n+1)!}
...
由\cref{example:牛顿插值.幂函数的k阶差商就是完全齐次对称多项式}
可知\(f(x) = x^{2n+1}\)
关于\(n+1\ (n\geq0)\)个点\(x,x_0,\dotsc,x_n\)的\(n\)阶差商\(f[x,x_0,\dotsc,x_n]\)
是关于点\(x,x_0,\dotsc,x_n\)的\(n+1\)次完全齐次对称多项式，
即\begin{equation*}
	f[x_0,\dotsc,x_n,x]
...
%@see: 《数值分析（第5版）》（李庆扬、王能超、易大义） P31 (3.7)
	...
	f(x) - N_n(x)
		\notag \\
	&\hphantom{:}= f[x,x_0,\dotsc,x_n] (x-x_0)\dotsm(x-x_n)
