...
或\(\left[\dv{x} f(x)\right]_{x=x_0}\).

函数\(f(x)\)在点\(x_0\)处可导有时也说成%
“\(f(x)\)在点\(x_0\)具有导数”或“\(f(x)\)在点\(x_0\)导数存在”.
\end{definition}
...
“函数\(f(x)\)在点\(x_0\)处的\textbf{左导数}（left-sided derivative）”，%
记作\(f'_-(x_0)\).

称极限\[\lim\limits_{h\to0^+} \frac{f(x_0+h)-f(x_0)}{h}\]为%
和“函数\(f(x)\)在点\(x_0\)处的\textbf{右导数}（right-sided derivative）”，%
记作\(f'_+(x_0)\).
...
这样就构成一个新的函数，这个函数叫做原来函数\(y = f(x)\)的\textbf{导函数}（简称导数），%
