...
\subsection{导数的定义}
\begin{definition}
设函数\(y=f(x)\)在点\(x_0\)的某个邻域内有定义，当自变量\(x\)在\(x_0\)处取得增量\(\increment x\)（点\(x_0+\increment x\)仍在该邻域内）时，%
相应的函数取得增量\(\increment y = f(x_0 + \increment x) - f(x_0)\)；如果\(\increment y\)与\(\increment x\)之比当\(\increment x\to0\)时的极限存在，%
则称函数\(y=f(x)\)在点\(x_0\)处\textbf{可导}，并称这个极限为函数\(y=f(x)\)在点\(x_0\)处的导数，记为\(f'(x_0)\)，即\begin{equation}
f'(x_0)
...
\end{equation}也可记作\(\eval{y'}_{x=x_0}\)，\(\eval{\dv{y}{x}}_{x=x_0}\)，\(\eval{\dv{f(x)}{x}}_{x=x_0}\)或\(\left[\dv{x} f(x)\right]_{x=x_0}\).

函数\(f(x)\)在点\(x_0\)处可导有时也说成“\(f(x)\)在点\(x_0\)具有导数”或“\(f(x)\)在点\(x_0\)导数存在”.
...
\begin{definition}
如果函数\(y = f(x)\)在开区间\(I\)内的每点处都可导，就称函数\(f(x)\)在开区间\(I\)内可导.
这是，对于任一\(x \in I\)，都对应着\(f(x)\)的一个确定的导数值.这样就构成一个新的函数，这个函数叫做原来函数\(y = f(x)\)的\textbf{导函数}（简称导数），记作\(y'\)，\(f'(x)\)，\(\dv{y}{x}\)，\(\dv{f(x)}{x}\)或\(\dv{x} f(x)\).
\end{definition}

\begin{definition}\label{definition:函数族.可导函数族}
由区间\(I\)上全部的可导函数组成的集合，称作\textbf{可导函数族}，记作\(D(I)\)\footnote{%
...
\begin{definition}
极限\(\lim\limits_{h\to0^-} \frac{f(x_0+h)-f(x_0)}{h}\)和\(\lim\limits_{h\to0^+} \frac{f(x_0+h)-f(x_0)}{h}\)分别称为函数\(f(x)\)在点\(x_0\)处的\textbf{左导数}（left-sided derivative）和\textbf{右导数}（right-sided derivative），记作\(f'_-(x_0)\)和\(f'_+(x_0)\).

左导数和右导数统称\textbf{单侧导数}（one-sided derivative）.
\end{definition}

\begin{theorem}[导数存在的充要条件]
函数\(f(x)\)在点\(x_0\)处可导的充要条件是其左导数\(f'_{-}(x_0)\)和右导数\(f'_{+}(x_0)\)都存在且相等.
\end{theorem}

\begin{definition}
如果函数\(f(x)\)在开区间\((a,b)\)内可导，且\(f'_{+}(a)\)及\(f'_{-}(b)\)都存在，就说\(f(x)\)在闭区间\([a,b]\)上可导.
\end{definition}
...
\subsection{导数的几何意义}
\begin{theorem}
曲线\(y=f(x)\)在点\(M(x_0,y_0)\)处的\textbf{切线方程}为\[
y-y_0=f'(x_0)(x-x_0).
\]

过切点\(M(x_0,y_0)\)且与切线垂直的直线叫做曲线\(y=f(x)\)在点\(M\)处的\textbf{法线}.
...
\]称为\textbf{魏尔斯特拉斯函数}，它的参数\(a\)和\(b\)满足\(0<a<1, b\in\Set{ p \given p = 2q+1, q\in\mathbb{N} }, ab > 1+\frac{3}{2}\pi\).
...
上述复合函数的求导公式也称作\textbf{链式法则}（chain rule）.
...
\end{equation}
\rm
\cref{equation:导数与微分.对数导数公式1} 称作\textbf{对数导数公式}（logarithmic derivative）.
\end{theorem}

\section{高阶导数}
\subsection{函数的\texorpdfstring{\(n\)}{n}阶导数}
\begin{definition}
一般地，函数\(y=f(x)\)的导数\(y'=f'(x)\)仍然是\(x\)的函数.我们把\(y'=f'(x)\)的导数叫做函数\(y=f(x)\)的\textbf{二阶导数}，记作\(y''\)或\(\dv[2]{y}{x}\)，即\[
y''=(y')'
\quad \text{或}\quad
\dv[2]{y}{x}=\dv{x}(\dv{y}{x}).
\]

相应地，把\(y=f(x)\)的导数\(y'(x)\)叫做函数\(y=f(x)\)的\textbf{一阶导数}.

类似地，二阶导数的导数叫做\textbf{三阶导数}，记作\(y'''\)或\(\dv[3]{y}{x}\)；
三阶导数的导数叫做\textbf{四阶导数}，记作\(y^{(4)}\)或\(\dv[4]{y}{x}\)；
以此类推，\((n-1)\)阶导数的导数叫做\textbf{ \(n\)阶导数}，记作\(y^{(n)}\)或\(\dv[n]{y}{x}\).
二阶及二阶以上的导数统称\textbf{高阶导数}.
...
\end{equation}
\rm
\cref{equation:导数与微分.莱布尼茨公式} 称为\textbf{莱布尼茨公式}.
\end{theorem}

\begin{definition}\label{definition:函数族.n阶可导函数族}
由区间\(I\)上全部的\(n\)阶可导函数组成的集合，称作\(n\)阶\textbf{可导函数族}，记作\(D^n(I)\)，即\[
D^n(I) = \Set*{
	f\colon I \to \mathbb{R}
	\given
	f^{(i)} \in D(I),
...
由区间\(I\)上全部的\(n\)阶连续可导函数组成的集合，称作\(n\)阶\textbf{连续可导函数族}，记作\(C^n(I)\)，即\[
C^n(I) = \Set*{
	f\colon I \to \mathbb{R}
	\given
	f \in D^n(I)
...
称函数\(f \in C^{\infty}(I)\)为\(I\)上的\textbf{光滑函数}（smooth function）.
\end{definition}
...
\subsection{隐函数的导数}
\begin{definition}
形如\(y=f(x)\)的函数，在等号左端是因变量的符号，而右端是含有自变量的式子，当自变量取定义域内任一值时，由这个式子能确定对应的函数.用这种方式表达的函数叫做\textbf{显函数}.
相对地，形如\(g(x,y)=0\)的函数，称为\textbf{隐函数}.
把一个隐函数化为显函数，叫做\textbf{隐函数的显化}.
\end{definition}
...
\begin{solution}
由\(\frac{(x-1)(x-2)}{(x-3)(x-4)}\geqslant0\)得\[
...
\]其中\(A\)是不依赖于\(\increment x\)的常数，那么称函数\(y=f(x)\)在点\(x_0\)是\textbf{可微的}，而\(A\increment x\)叫做函数\(y=f(x)\)在点\(x_0\)相应于自变量增量\(\increment x\)的\textbf{微分}，记作\(\dd{y}\)，即\[
\dd{y}=A\increment x.
\]称\(\dd{y}\)是\(\increment y\)的\textbf{主部}.
又由于\(\dd{y}=f'(x_0)\increment x\)是\(\increment x\)的线性函数，所以在\(f'(x_0) \neq 0\)的条件下，称\(\dd{y}\)为\(\increment y\) 的\textbf{线性主部}（当\(\increment x\to0\)）.
\end{definition}
...
\]由此可见，无论\(u\)是自变量还是中间变量，微分形式\(\dd{y}=f'(u)\dd{u}\)保持不变.这一性质称为\textbf{微分形式不变性}.这性质表明，当变换自变量时，微分形式\(\dd{y}=f'(u)\dd{u}\)并不改变.
\end{theorem}
...
由于测量仪器的精度、测量的条件和测量的方法等各种因素的影响，测得的数据往往带有误差，而根据带有误差的数据计算所得的结果也会有误差，我们把它叫做\textbf{间接测量误差}.

下面就讨论怎样利用微分来估计间接测量误差.先说明什么叫绝对误差、相对误差.

如果某个量的精确值为\(A\)，它的测量值（或近似值）为\(a\)，那么\(\abs{A-a}\)叫做\(a\)的\textbf{绝对误差}，而绝对误差与\(\abs{a}\)的比值\(\frac{\abs{A-a}}{\abs{a}}\)叫做\(a\)的\textbf{相对误差}.
...
\]那么\(\delta_A\)叫做测量\(A\)的\textbf{绝对误差限}，而\(\frac{\delta_A}{\abs{a}}\)叫做测量\(A\)的\textbf{相对误差限}.
