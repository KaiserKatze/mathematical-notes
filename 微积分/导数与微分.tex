...
f'(x_0) = \lim\limits_{\increment x\to0} \frac{\increment y}{\increment x} = \lim\limits_{\increment x\to0} \frac{f(x_0+\increment x)-f(x_0)}{\increment x}
...
\begin{definition}
极限\(\lim\limits_{h\to0^-} \frac{f(x_0+h)-f(x_0)}{h}\)和\(\lim\limits_{h\to0^+} \frac{f(x_0+h)-f(x_0)}{h}\)分别称为函数\(f(x)\)在点\(x_0\)处的\textbf{左导数}和\textbf{右导数}，记作\(f'_-(x_0)\)和\(f'_+(x_0)\).
左导数和右导数统称\textbf{单侧导数}.
\end{definition}
...
函数\(y=f(x)\)具有\(n\)阶导数，也常说成函数\(f(x)\)为\(n\)阶可导，记作\(f \in C^n\).
\end{definition}
