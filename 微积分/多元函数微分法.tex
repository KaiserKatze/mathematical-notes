...
垂直于曲面上切平面的向量称为曲面的\DefineConcept{法向量}.
...
\(\vb{n}_F = \opair{F'_x(P_0),F'_y(P_0),F'_z(P_0)}\)
和\(\vb{n}_G = \opair{G'_x(P_0),G'_y(P_0),G'_z(P_0)}\)，
故交线在点\(P_0\)处的切向量为\[
\vb{\tau}
...
先解方程组\[
	...
	\end{array} \right.
\]
求得驻点为\((1,0),\opair{1,2},\opair{-3,0},\opair{-3,2}\).
