...
\]则称函数\(f(x,y)\)在点\(P_0\opair{x_0,y_0}\)\DefineConcept{连续}.
...
\]存在，则称此极限为函数\(z=f(x,y)\)在点\(\opair{x_0,y_0}\)处对\(x\)的\DefineConcept{偏导数}，记作\[
...
\]其中\(A\)、\(B\)不依赖于\(\increment x\)、\(\increment y\)而仅与\(x\)、\(y\)有关，\(\rho=\sqrt{(\increment x)^2+(\increment y)^2}\)，则称函数\(z=f(x,y)\)在点\(\opair{x,y}\)\DefineConcept{可微分}，而\(A \increment x + B \increment y\)称为函数\(z=f(x,y)\)在点\(\opair{x,y}\)的\DefineConcept{全微分}，记作\(\dd{z}\)，即\[
...
f(P) < f(P_0),
\]则称函数\(f\)在点\(P_0\)有\DefineConcept{极大值}\(f(P_0)\)，点\(P_0\)称为函数\(f\)的\DefineConcept{极大值点}；
若对于任意点\(P \in \mathring{U}(P_0)\)，都有\[
f(P) > f(P_0),
\]则称函数\(f\)在点\(P_0\)有\DefineConcept{极小值}\(f(P_0)\)，点\(P_0\)称为函数\(f\)的\DefineConcept{极小值点}.

极大值、极小值统称为\DefineConcept{极值}.使得函数取得极值的点称为\DefineConcept{极值点}.
\end{definition}
...
上面所讨论的极值问题，对于函数的自变量，除了限制在函数的定义域内以外，并无其他条件，所以有时候称为\DefineConcept{无条件极值}.
...
另有一种直接寻求条件极值的方法，可以不必先把问题化到无条件极值的问题，这就是下面要介绍的\DefineConcept{拉格朗日乘数法}.

要找函数\(z=f(x,y)\)在附加条件\(\varphi(x,y)=0\)下的可能极值点，可以先作\DefineConcept{拉格朗日函数}\[
L(x,y) = f(x,y)+\lambda \varphi(x,y),
\]其中\(\lambda\)为参数，称作\DefineConcept{拉格朗日乘子}.求其对\(x\)与\(y\)的一阶偏导数，并使之为零，然后与方程\(\varphi(x,y)=0\)联立起来：\[
\left\{ \begin{array}{l}
