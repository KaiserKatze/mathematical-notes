...
叫做\textbf{微分方程}（differential equation）.

如果未知函数是一元函数\(y = y(x)\)，
则称这个微分方程为\textbf{常微分方程}（ordinary differential equation）.

如果未知函数是多元函数\(y = y(\v{x}{m})\)，
则称这个微分方程为\textbf{偏微分方程}（partial differential equation）.

未知函数的最高阶导数（或偏导数）的阶数，
称为“微分方程的\textbf{阶}（order）”.
\end{definition}
...
F\left[x,\varphi(x),\varphi'(x),\dotsc,\varphi^{(n)}(x)\right]\equiv0,
\]
...
即\(\jacobi{\varphi,\varphi',\dotsc,\varphi^{(n-1)}}{\v{C}{n}} \neq 0\).}，%
这样的解\[
y = \varphi(x;\v{C}{n})
...
\biggr],
\]则称“函数系\(S\)在区间\(D\)上是\textbf{等度连续的}（equicontinuous）%
...
\cref{theorem:微分方程.二阶非齐次线性微分方程的解的叠加原理} 通常称为线性微分方程的解的“叠加原理”.
...
如果非齐次项\(f(x)\)可以表达为
\(e^{\lambda x} [\varphi_m(x) \cos\omega x + \psi_n(x) \sin\omega x]\)
的形式，%
则二阶常系数非齐次线性微分方程
...
\begin{solution}
所给方程是二阶常系数非齐次线性方程，%
且非齐次项\(f(x)\)属于\(e^{\lambda x} [\varphi_m(x) \cos\omega x + \psi_n(x) \sin\omega x]\)型
（其中\(\lambda=0,
\omega=2,
\varphi_m(x) = x,
\psi_n(x) = 0\)）.
