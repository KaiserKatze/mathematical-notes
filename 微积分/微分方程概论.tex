...
于是得到如下的\(n = \max\{\v{n}{k}\}\)阶\DefineConcept{常微分方程组}的一般形式：
\[
...
\]是互成共轭\footnote{如果说两个\(n\)次多项式\(\alpha_n(x) = a_0 x^n + a_1 x^{n-1} + \dotsb + a_n\)与\(\beta_n(x) = b_0 x^n + b_1 x^{n-1} + \dotsb + b_n\)\DefineConcept{互成共轭}，那么它们的对应项系数是共轭复数，即\(a_i = \overline{b_i}\ (i=0,1,\dotsc,n)\).}的\(N = \max\{m,n\}\)次多项式.
