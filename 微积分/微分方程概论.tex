...
\begin{definition}
一般地，凡表示未知函数、未知函数的导数与自变量之间的关系的方程\begin{equation}\label{equation:微分方程.微分方程的一般形式}
F\left(x,y,\dv{y}{x},\dv[2]{y}{x},\dotsc,\dv[n]{y}{x}\right)=0,
\end{equation}叫做\DefineConcept{微分方程}.微分方程中所出现的未知函数的最高阶导数的阶数，称为微分方程的\DefineConcept{阶}.
\end{definition}
如果能从方程 \labelcref{equation:微分方程.微分方程的一般形式} 中解出最高阶导数，则可得微分方程\begin{equation}\label{equation:微分方程.分离出最高阶导数}
\dv[n]{y}{x} = f\left(x,y,\dv{y}{x},\dv[2]{y}{x},\dotsc,\dv[n-1]{y}{x}\right).
\end{equation}
以后我们讨论的微分方程都是已解出最高阶导数的方程或能解出最高阶导数的方程.

由于微分方程所要求解的未知函数是单变量函数\(y = y(x)\)，所以我们称像这样的微分方程为\DefineConcept{常微分方程}.

\begin{definition}
设函数\(y=\varphi(x)\)在区间\(I\)上有\(n\)阶连续导数，如果在区间\(I\)上有\[
F[x,\varphi(x),\varphi'(x),\dotsc,\varphi^{(n)}(x)]\equiv0,
\]那么函数\(y=\varphi(x)\)就叫做微分方程 \labelcref{equation:微分方程.微分方程的一般形式} 在区间\(I\)上的\DefineConcept{解}.

如果微分方程的解中含有任意常数，且任意常数的个数与微分方程的阶数相同\footnote{这里所说的任意常数是相互独立的，就是说，它们不能合并而使得任意常数的个数减少.}，这样的解叫做微分方程的\DefineConcept{通解}.
\end{definition}
...
特别地，求一阶微分方程\(y'=f(x,y)\)满足初始条件\(\eval{y}_{x=x_0} = y_0\)的特解这样一个问题，叫做一阶微分方程的\DefineConcept{初值问题}，记作\[
\left\{ \begin{array}{l}
y' = f(x,y), \\
\eval{y}_{x=x_0} = y_0.
\end{array} \right.
...
微分方程的解的图形一条曲线，叫做微分方程的\DefineConcept{积分曲线}.上述一阶微分方程的初值问题的几何意义，就是求通过点\(\opair{x_0,y_0}\)的那条积分曲线.二阶微分方程的初值问题\[
\left\{ \begin{array}{l}
y'' = f(x,y,y'), \\
\eval{y}_{x=x_0} = y_0, \\
\eval{y'}_{x=x_0} = y'_0
