...
如果微分方程中的未知函数是一元函数\(y = y(x)\)，则称其为\DefineConcept{常微分方程}（ordinary differential equation）.

如果微分方程中的未知函数是多元函数\(y = y(\v{x}{m})\)，则称其为\DefineConcept{偏微分方程}（partial differential equation）.
...
当\(f(x) = 0\)时，我们称微分方程 \labelcref{equation:微分方程.线性常微分方程的一般形式} 为\(n\)阶\textbf{线性齐次常微分方程}（n-th order linear homogeneous ordinary differential equation）；
...
\eqno{(12)}
\]
方程(12)叫做“\cemph{对应于}线性非齐次方程(11)的线性齐次方程”.
...
如果\(y_1(x)\)与\(y_2(x)\)是方程 \labelcref{equation:微分方程.二阶齐次线性微分方程的一般形式} 的两个线性无关的特解，那么\[
...
这种方法的特点是不用积分就可求出\(\hat{y}\)来，它叫做\cemph{待定系数法}.
...
\subsection{\texorpdfstring{\(f(x) = e^{\lambda x} P_m(x)\)型}{第一类：指数函数与幂级数的乘积}}
我们知道，方程 \labelcref{equation:微分方程.二阶常系数非齐次线性微分方程的一般形式} 的特解\(\hat{y}\)是使 \labelcref{equation:微分方程.二阶常系数非齐次线性微分方程的一般形式} 称为恒等式的函数.
怎样的函数能使 \labelcref{equation:微分方程.二阶常系数非齐次线性微分方程的一般形式} 称为恒等式呢？
因为 \labelcref{equation:微分方程.二阶常系数非齐次线性微分方程的一般形式} 右端\(f(x)\)是多项式\(P_m(x)\)与指数函数\(e^{\lambda x}\)的乘积，而多项式与指数函数乘积的导数仍然是多项式与指数函数的乘积，因此，我们推测\(\hat{y} = Q(x) e^{\lambda x}\)（其中\(Q(x)\)是某个多项式）可能是方程 \labelcref{equation:微分方程.二阶常系数非齐次线性微分方程的一般形式} 的特解.
把\(\hat{y}, \hat{y}', \hat{y}''\)代入方程 \labelcref{equation:微分方程.二阶常系数非齐次线性微分方程的一般形式}，然后考虑能否选取适当的多项式\(Q(x)\)，使得\(\hat{y} = Q(x) e^{\lambda x}\)满足方程 \labelcref{equation:微分方程.二阶常系数非齐次线性微分方程的一般形式}.
为此，将\begin{align*}
...
\subsection{\texorpdfstring{\(f(x) = e^{\lambda x} [ \varphi_m(x) \cos\omega x + \psi_n(x) \sin \omega x ]\)型}{第二类：指数函数与广义三角函数的乘积}}
应用欧拉公式\[
