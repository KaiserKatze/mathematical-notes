...
那么\begin{equation}\label{equation:定积分.利用牛顿莱布尼茨公式计算无穷限的反常积分1}
	...
	= F(+\infty) - F(a),
\end{equation}
其中\(F(+\infty) = \lim\limits_{x\to+\infty} F(x)\).
...
\def\LimitFormOfImproperIntegralB{\lim_{\epsilon\to0^+} \int_{a+\epsilon}^b f(x) \dd{x}}%
设函数\(f\colon(a,b]\to\mathbb{R}\)，且\[
	\lim_{\epsilon\to a^+} f(x) = \infty;
\]
但是对于\(\forall\epsilon\in(0,b-a)\)，
函数\(f\)在\([a+\epsilon,b]\)上可积.

如果极限\[
	\LimitFormOfImproperIntegralB
\]存在且有限，
那么把这个极限称为“函数\(f(x)\)在\((a,b]\)上的\DefineConcept{广义积分}”，
记作\(\int_a^b f(x) \dd{x}\)；
我们还称“反常积分\(\int_a^b f(x) \dd{x}\) \DefineConcept{收敛}”；
另外，我们把点\(a\)称为“积分\(\int_a^b f(x) \dd{x}\)的\DefineConcept{瑕点}”.
