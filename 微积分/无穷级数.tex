...
\end{split}
\]于是\[
\lim\limits_{n\to\infty} \tau_n = \lim\limits_{n\to\infty} (s_n \pm \sigma_n) = s + \sigma.
\]
...
\def\s{\sum\limits_{n=1}^\infty }
如果级数\(\sum\limits_{n=1}^\infty u_n(x)\)的各项\(u_n(x)\)都具有连续导数\(u'_i(x)\)，且\(\sum\limits_{n=1}^\infty u_n(x)\)在区间\([a,b]\)上收敛于和\(s(x)\)，它并且级数\(\s u'_n(x)\)在\([a,b]\)上一致收敛，则级数\(\sum\limits_{n=1}^\infty u_n(x)\)在区间\([a,b]\)上也一致收敛，且可逐项求导，即\[
s'(x) = \s u'_n(x).
\]
\begin{proof}
...
= \s [u_n(x) - u_n(x_0)],
\]
而\(\sum\limits_{n=1}^\infty u_n(x) = s(x)\)，\(\sum\limits_{n=1}^\infty u_n(x_0) = s(x_0)\)，故\[
...
\s \int_{x_0}^x u'_n(x) \dd{x} = \sum\limits_{n=1}^\infty u_n(x) - \sum\limits_{n=1}^\infty u_n(x_0),
\]所以\[
\sum\limits_{n=1}^\infty u_n(x) = \s \int_{x_0}^x u'_n(x) \dd{x} + \sum\limits_{n=1}^\infty u_n(x_0).
...
\begin{solution}
先求收敛域.由\[
\lim\limits_{n\to\infty} \abs{\frac{a_{n+1}}{a_n}} = \lim\limits_{n\to\infty} \frac{n+1}{n+2} = 1,
\]得收敛半径\(R=1\).
