...
\]
我们把\(s_n\)称为级数\(\s u_i\)的\DefineConcept{部分和}（partial sum）.
...
\]
叫做“级数\(\s u_i\)的\DefineConcept{余项}”.
称这个余项的绝对值\(\abs{r_n}\)为%
“用近似值\(s_n\)代替和\(s\)所产生的\DefineConcept{误差}”.
\end{definition}
从上述定义可知，级数与数列极限有着紧密的联系.给定级数\(\s u_i\)，
就有部分和数列\(\{s_n = \sn u_i\}\)；
...
按定义，级数\(\s u_i\)与数列\(\{s_n\}\)同时收敛或同时发散，且在收敛时，有\[
	\s u_i = \l s_n,
\]
即\(\s u_i = \l \sn u_i\).
...
如果级数\(\s u_i\)收敛，则由上可知必有级数\(\s v_i\)收敛，
但已知级数\(s v_i\)发散，因此级数\(\s u_i\)不可能收敛，即级数\(\s u_i\)发散.
\end{proof}
\end{theorem}
...
而级数\(\s v_i\)收敛，则级数\(\s u_i\)收敛；
如果\(u_n\)是与\(v_n\)同阶或是比\(v_n\)低阶的无穷小，
而级数\(\s v_i\)发散，则级数\(\s u_i\)发散.
