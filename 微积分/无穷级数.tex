...
\]成立，%
则称“函数项级数\(\s u_i(x)\)在区间\(I\)上\textbf{一致收敛}于\(s(x)\)%
（\(\s u_i(x)\) is \emph{uniformly convergent} to \(s\) for the interval \(I\)）”，%
或“函数列\(\{s_n(x)\}\)在区间\(I\)上\textbf{一致收敛}于\(s(x)\)”，%
...
根据\cref{theorem:无穷级数.一致收敛级数的基本性质2}，%
...
\]
为“函数\(f(x)\)在点\(x_0\)处的\textbf{泰勒级数}（Taylor series）”.
