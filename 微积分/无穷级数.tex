...
按定义，级数\(\sum\limits_{n=1}^\infty u_n\)与数列\(\{s_n\}\)同时收敛或同时发散，且在收敛时，有\[
	\s u_i = \lim\limits_{n\to\infty} s_n,
\]
...
\]\[
	\lim\limits_{n\to\infty}\sigma_n = \lim\limits_{n\to\infty} k s_n = k \lim\limits_{n\to\infty} s_n = ks,
\]
...
\begin{proof}
因为\(u_n u_{n+1} < 0\),\(\abs{u_n} > \abs{u_{n+1}}\)，\(\lim\limits_{n\to\infty} \abs{u_n} = 0\)，那么根据\cref{theorem:无穷级数.莱布尼茨定理} 可知\(\s u_n\)收敛.
