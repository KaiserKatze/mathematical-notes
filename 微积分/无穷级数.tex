...
简称\DefineConcept{常数项级数}，记作\(\s u_i\)，即\[
\s u_i
= u_1+u_2+\dotsb+u_n+\dotsb,
\]
...
\]叫做\DefineConcept{等比级数}（又称\DefineConcept{几何级数}），其中\(a \neq 0\)，\(q\)叫做\DefineConcept{级数的公比}.试讨论上述等比级数的收敛性.
\begin{solution}
...
如果级数\(\s v_i\)收敛，且存在正整数\(N\)，使当\(n \geq N\)时有\(u_n \leq k v_n\ (k > 0)\)成立，则级数\(\s u_i\)收敛；
如果级数\(\s v_i\)发散，且存在正整数\(N\)，使当\(n \geq N\)时有\(u_n \geq k v_n\ (k > 0)\)成立，则级数\(\s u_i\)发散.
\end{corollary}
...
\begin{proof}
当\(\rho<1\).取一个适当小的正数\(\varepsilon\)，使得\(\rho+\varepsilon=r<1\)，根据极限定义，存在正整数\(m\)，当\(n \geq m\)时有不等式\[
...
\item 当\(\l n^p u_n = l\in(0,+\infty]\)，且\(p\leq1\)时，级数\(\s u_i\)发散.
\end{enumerate}
\end{theorem}
...
\begin{definition}
设级数\(\s u_i\)的各项为任意实数.
如果级数\(\s u_i\)各项的绝对值所构成的正项级数\(\s {\abs{u_i}}\)收敛，则称级数\(\s u_i\) \DefineConcept{绝对收敛}；
如果级数\(\s u_i\)收敛，而级数\(\s {\abs{u_i}}\)发散，则称级数\(\s u_i\) \DefineConcept{条件收敛}.
\end{definition}
...
定义级数\(\s u_i\)和\(\s v_i\)的\DefineConcept{柯西乘积}为\[
...
\end{equation}这就是\DefineConcept{傅里叶系数的复数形式}.
