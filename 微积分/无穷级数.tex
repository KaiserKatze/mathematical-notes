...
\begin{enumerate}
	...
	从而级数\(\sum\limits_{n=1}^\infty a_n x^n\)绝对收敛；
	...
	于是\(R=+\infty\).
		\item 如果\(\rho=+\infty\)，
	则对\(\forall x\neq0\)，
	级数\(\sum\limits_{n=1}^\infty a_n x^n\)必发散，
...
\def\s{\sum\limits_{n=0}^\infty }
如果幂级数\(\sum\limits_{n=1}^\infty a_n x^n\)的收敛半径为\(R>0\)，则此级数在\((-R,R)\)内闭一致收敛.
\end{property}
...
\def\s{\sum\limits_{n=0}^\infty }
如果幂级数\(\sum\limits_{n=1}^\infty a_n x^n\)的收敛半径为\(R>0\)，则其和函数\(s(x)\)在\((-R,R)\)内可导，且有逐项求导公式\[
s'(x)
