...
如果级数\(\sum\limits_{n=1}^\infty u_n\)、\(\s v_i\)分别收敛于\(s\)、\(\sigma\)，则级数\(\s (u_i \pm v_i)\)收敛于\(s \pm \sigma\).
\begin{proof}
设级数\(\sum\limits_{n=1}^\infty u_n\)与级数\(\s v_i\)的部分和分别为\(s_n\)与\(\sigma_n\)，则级数\(\s (u_i \pm v_i)\)的部分和\[
\begin{split}
...
设\(\sum\limits_{n=1}^\infty u_n\)和\(\s v_i\)都是正项级数，且\[
u_n \leq v_n
\quad(n=1,2,\dotsc).
\]
若级数\(\s v_i\)收敛，则级数\(\sum\limits_{n=1}^\infty u_n\)收敛；
反之，若级数\(\sum\limits_{n=1}^\infty u_n\)发散，则级数\(\s v_i\)发散.
\begin{proof}
设级数\(\s v_i\)收敛于和\(\sigma\)，则级数\(\sum\limits_{n=1}^\infty u_n\)的部分和\[
...
设\(\sum\limits_{n=1}^\infty u_n\)和\(\s v_i\)都是正项级数.
如果级数\(\s v_i\)收敛，且存在正整数\(N\)，使当\(n \geq N\)时有\(u_n \leq k v_n\ (k > 0)\)成立，则级数\(\sum\limits_{n=1}^\infty u_n\)收敛；
如果级数\(\s v_i\)发散，且存在正整数\(N\)，使当\(n \geq N\)时有\(u_n \geq k v_n\ (k > 0)\)成立，则级数\(\sum\limits_{n=1}^\infty u_n\)发散.
\end{corollary}
...
\begin{proof}
由基本不等式 \labelcref{theorem:不等式.基本不等式1} 可知\(n^3+a_n^2\geq2\sqrt{n^3 a_n^2}=2n^{3/2}a_n\)，那么\(\frac{a_n}{n^3+a_n^2}\leq\frac{a_n}{2n^{3/2}a_n}=\frac{1}{2n^{3/2}}\).
...
设\(\sum\limits_{n=1}^\infty u_n\)和\(\s v_i\)都是正项级数，记\[
l = \l \frac{u_n}{v_n}.
\]\begin{enumerate}
\item 如果\(l\in[0,+\infty)\)，且级数\(\s v_i\)收敛，则级数\(\sum\limits_{n=1}^\infty u_n\)收敛；
\item 如果\(l\in(0,+\infty]\)，且级数\(\s v_i\)发散，则级数\(\sum\limits_{n=1}^\infty u_n\)发散.
\end{enumerate}
\begin{proof}
...
如果级数\(\sum\limits_{n=1}^\infty u_n\)收敛，则由上可知必有级数\(\s v_i\)收敛，
...
而级数\(\s v_i\)收敛，则级数\(\sum\limits_{n=1}^\infty u_n\)收敛；
如果\(u_n\)是与\(v_n\)同阶或是比\(v_n\)低阶的无穷小，
而级数\(\s v_i\)发散，则级数\(\sum\limits_{n=1}^\infty u_n\)发散.
...
如果级数\(\sum\limits_{n=1}^\infty u_n\)各项的绝对值所构成的正项级数\(\s {\abs{u_i}}\)收敛，则称级数\(\sum\limits_{n=1}^\infty u_n\) \DefineConcept{绝对收敛}；
如果级数\(\sum\limits_{n=1}^\infty u_n\)收敛，而级数\(\s {\abs{u_i}}\)发散，则称级数\(\sum\limits_{n=1}^\infty u_n\) \DefineConcept{条件收敛}.
\end{definition}
...
0, & u_n \leq 0,
\end{array} \right.
\]可见级数\(\s v_i\)是把级数\(\sum\limits_{n=1}^\infty u_n\)中的负项换成得到的，它也就是级数\(\sum\limits_{n=1}^\infty u_n\)中的全体正项所构成的级数.类似可知，令\[
w_n = \frac{1}{2} (\abs{u_n} - u_n),
\]则\(\s w_i\)为级数\(\sum\limits_{n=1}^\infty u_n\)中全体负项的绝对值所构成的级数.

如果级数\(\sum\limits_{n=1}^\infty u_n\)绝对收敛，则级数\(\s v_i\)和\(\s w_i\)都收敛；如果级数\(\sum\limits_{n=1}^\infty u_n\)条件收敛，则级数\(\s v_i\)和\(\s w_i\)都发散.
...
则称“点\(x_0\)为函数项级数\(\s u_i(x)\)的\DefineConcept{收敛点}”；
反之，如果常数项级数\(\s u_i(x_0)\)发散，
就称“点\(x_0\)为函数项级数\(\s u_i(x)\)的\DefineConcept{发散点}”.
