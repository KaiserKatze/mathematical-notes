...
对\(L\)上任意一点\(M_0\)，当\(L\)上的动点\(M\opair{x,y}\)沿\(L\)趋于\(M_0\)时，有\(\abs{\vb{F}(x,y)-\vb{F}(x_0,y_0)}\to0\).%
...
\begin{example}
设一个质点在点\(M\opair{x,y}\)处受到力\(\vb{F}\)的作用，\(\vb{F}\)的大小与点\(M\)到原点\(O\)的距离成正比，\(\vb{F}\)的方向恒指向原点.
