...
\begin{theorem}
设积分曲面\(\Sigma\)由二元向量值参数方程\begin{equation*}
...
\(\Sigma\)在\(xOy\)面上的投影区域\(D_{xy}\)为圆形闭区域\begin{equation*}
...
依次记为\(\AutoTuple{\Sigma}{4}\)，于是\begin{equation*}
...
\end{equation*}
由于在\(\AutoTuple{\Sigma}{3}\)上，被积函数\(f(x,y,z)=xyz\)均为零，所以\begin{equation*}
	\oiint_{\Sigma_1} xyz \dd{S}
	= \oiint_{\Sigma_2} xyz \dd{S}
	= \oiint_{\Sigma_3} xyz \dd{S}
	= 0.
...
在\(\Sigma_4\)上，\(z=1-x-y\)，\(z'_x = z'_y = -1\)，所以\begin{equation*}
...
若\(S\)在点\(P\)处的切平面与\(xOy\)面垂直，求点\(P\)的轨迹\(C\)，并计算曲面积分\begin{equation*}
...
所以法向量\(\vb{n}\)与\(xOy\)面的法向量\(\opair{0,0,1}\)垂直，\begin{equation*}
	2x\cdot0 + (2y-z)\cdot0+(2z-y)\cdot1=0
	\quad\text{或}\quad
	2z-y=0.
\end{equation*}
因此，轨迹\(C\)的方程是\begin{equation*}
	\begin{cases}
		x^2 + y^2 + z^2 - yz = 1, \\
		2z-y=0.
	\end{cases}
\end{equation*}
消去轨迹\(C\)方程中的\(z\)，得到\(C\)在\(xOy\)面上的投影为\begin{equation*}
...
由于地球表面的重力加速度\(g\)满足\begin{equation*}
	\frac{GMm}{R^2}
	= mg,
\end{equation*}
所以\begin{equation*}
...
\end{equation*}
利用第一类曲面积分的计算公式得\begin{equation*}
...
所以一颗卫星的电波可以覆盖的地球面积占比为\begin{equation*}
	\frac{h}{2(R+h)}
	\approx \num{0.4248}.
\end{equation*}
