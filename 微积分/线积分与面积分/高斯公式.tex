...
\end{equation*}
因为\(\Sigma_3\)上任意一块曲面在\(xOy\)面上的投影为零，所以直接根据对坐标的曲面积分的定义可知\begin{equation*}
	\iint_{\Sigma_3} R(x,y,z) \dd{x}\dd{y} = 0.
\end{equation*}
把以上三个式子相加，得\begin{equation*}
...
\end{align*}
注意到积分区域\(\Omega\)关于\(xOz\)面对称，那么\begin{equation*}
...
同时，由于\(D_{xy}\)关于原点对称，\begin{equation*}
...
\end{example}
\begin{example}
%@see: 《2025年全国硕士研究生入学统一考试（数学一）》三解答题/第20题
...
\begin{solution}
%@see: https://www.bilibili.com/video/BV1bUkeY4ECf/
...
则曲面积分\begin{equation*}
	\iint_\Sigma P\dd{y}\dd{z}+Q\dd{z}\dd{x}+R\dd{x}\dd{y}
\end{equation*}
在\(G\)内与所取曲面\(\Sigma\)无关而只决定于\(\Sigma\)的边界曲线
（或沿\(G\)内任一闭曲面的曲面积分为零）的充分必要条件是\begin{equation*}
...
\end{equation*}称为“向量场\(\vb{A}\)通过曲面\(\Sigma\)向着指定侧的\DefineConcept{通量}”.
\end{definition}
由两类曲面积分的关系，通量又可表达为\begin{equation*}
	\iint_\Sigma \VectorInnerProductDot{\vb{A}}{\vb{n}} \dd{S}
...
\end{equation*}是定义在平面闭区域\(D\)内的向量场，
\(C\)是\(D\)的边界曲线的正向，
\(D\)的面积\(S = \iint_{D} \dd{\sigma}\).
对于\(D\)内一点\(M\)，把极限\begin{equation*}
	\defofdiv
\end{equation*}称为“向量场\(\vb{A}\)在点\(M\)的（二维）\DefineConcept{散度}（或\DefineConcept{通量密度}）”，
记作\(\div{\vb{A}}\)或\(\operatorname{div}\vb{A}\)，即\begin{equation*}
...
设\begin{equation*}
	\vb{A}(x,y,z)=P(x,y,z)\vb{i}+Q(x,y,z)\vb{j}+R(x,y,z)\vb{k}
\end{equation*}是定义在空间闭区域\(\Omega\)内的向量场，
\(\Sigma\)是\(\Omega\)的边界曲面的外侧，
\(\Omega\)的体积\(V = \iiint_\Omega \dd{v}\).
对于\(\Omega\)内一点\(M\)，称极限\begin{equation*}
	\defofdiv
\end{equation*}为“向量场\(\vb{A}\)在点\(M\)的（三维）\DefineConcept{散度}（或\DefineConcept{通量密度}）”，
记作\(\div{\vb{A}}\)或\(\operatorname{div}\vb{A}\)，即\begin{equation*}
	\div{\vb{A}} \defeq \defofdiv.
\end{equation*}
\end{definition}
