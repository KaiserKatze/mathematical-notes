...
我们规定\(\increment S\)在\(xOy\)面上的投影\((\increment S)_{xy}\)为\begin{equation*}
	(\increment S)_{xy}
...
如果当各小块曲面的直径的最大值\(\lambda\to0\)时，极限\begin{equation*}
	\lim_{\lambda\to0}
	\sum_{i=1}^n
	R(\xi_i,\eta_i,\zeta_i)
	(\increment S_i)_{xy}
...
一般地，可以将\begin{equation*}
	\iint_\Sigma P(x,y,z) \dd{y}\dd{z}
	+\iint_\Sigma Q(x,y,z) \dd{z}\dd{x}
	+\iint_\Sigma R(x,y,z) \dd{x}\dd{y}
\end{equation*}合并写成\begin{equation*}
...
\end{equation*}
应用\cref{equation:线积分与面积分.第二类曲面积分的计算式3} 就有\begin{equation*}
	\iint_\Sigma x^2 \dd{y}\dd{z}
...
其中\(\Sigma_1\)的方程为\begin{equation*}
	z = -\sqrt{1-x^2-y^2},
\end{equation*}
而\(\Sigma_2\)的方程为\begin{equation*}
	z = \sqrt{1-x^2-y^2}.
\end{equation*}
因此\begin{align*}
...
就是位于第一象限内的扇形\begin{equation*}
	x^2+y^2\leq1 \quad(x\geq0,y\geq0).
\end{equation*}
利用极坐标计算这个二重积分如下：\begin{equation*}
	\iint_{D_{xy}} xy \sqrt{1-x^2-y^2} \dd{x}\dd{y}
