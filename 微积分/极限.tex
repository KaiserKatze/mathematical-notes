...
\]且数列极限\(\lim_{n\to\infty}{x_n} = a\)，则称极限\(\lim_{n\to\infty}{f(x_n)}\)为函数\(f(x)\)在点\(a\)的\DefineConcept{子列极限}.

这些子列极限中的最小值称作函数\(f(x)\)在点\(a\)的\DefineConcept{下极限}，记作\(\varliminf_{x \to a}f(x)\).这些子列极限中的最大值称作函数\(f(x)\)在点\(a\)的\DefineConcept{上极限}，记作\(\varlimsup_{x \to a}f(x)\).
\end{definition}
