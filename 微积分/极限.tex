...
\subsection{数列极限的定义}
\begin{definition}
设\(\{x_n\}\)为一数列，如果存在常数\(a\)，对于\(\forall \varepsilon > 0\)，总\(\exists N \in \mathbb{N}^+\)，使当\(n > N\)时，不等式\[
\abs{x_n - a} < \varepsilon
\]都成立，那么就称数列\(\{x_n\}\)\textbf{收敛的}（Convergent），常数\(a\)是数列\(\{x_n\}\)的\textbf{极限}，或者称数列\(\{x_n\}\)\textbf{收敛于}\(a\)，记为\[
...
上述对数列极限的定义可以简化为：\[
\lim\limits_{n\to\infty}x_n = a
\iff \forall \varepsilon > 0, \exists N\in\mathbb{N}^+ (n > N \implies \abs{x_n - a} < \varepsilon).
\]

如果不存在这样的常数\(a\)，就说数列\(\{x_n\}\)没有极限，或者说数列\(\{x_n\}\)是\textbf{发散的}（Divergent），习惯上也说\(\lim\limits_{n\to\infty}x_n\)不存在.
\end{definition}

上面定义中正数\(\varepsilon\)可以任意给定是很重要的，因为只有这样，不等式\(\abs{x_n - a} < \varepsilon\)才能表达出\(x_n\)与\(a\)无限接近的意思.此外还应注意到：定义中的正整数\(N\)是与任意给定的正数\(\varepsilon\)有关的，它随着\(\varepsilon\)的给定而选定.
...
也就是说，对于\(\forall \varepsilon > 0\)，取\(N = \round*{\frac{1}{\varepsilon}}\)，则当\(n > N\)时，就有\(\abs{x_n - a} < \varepsilon\)，即\(\lim\limits_{n\to\infty}\frac{n+(-1)^{n-1}}{n}=1\).
\end{proof}
\end{example}
...
\begin{proof}
因为\(\abs{x_n - a} = \abs{\frac{(-1)^n}{(n+1)^2}-0} = \frac{1}{(n+1)^2} < \frac{1}{n+1}\)，所以对于\(\forall\varepsilon>0\)（设\(\varepsilon<1\)），只要\(\frac{1}{n+1}<\varepsilon\)或\(n>\frac{1}{\varepsilon}-1\)，不等式\(\abs{x_n-a}<\varepsilon\)必定成立.所以，取\(N=\round*{\frac{1}{\varepsilon}-1}\)，则当\(n>N\)时就有\(\abs{x_n - a}<\varepsilon\)，即\(\lim\limits_{n\to\infty}\frac{(-1)^n}{(n+1)^2}=0\).
\end{proof}
\end{example}
...
\]的极限是\(0\).
\begin{proof}
对于\(\forall\varepsilon>0\)（设\(\varepsilon<1\)），因为\(\abs{x_n-0}=\abs{q^{n-1}-0}=\abs{q}^{n-1}\)，要使\(\abs{x_n-0}<\varepsilon\)，只要\(\abs{q}^{n-1}<\varepsilon\).取自然对数得\((n-1)\ln\abs{q}<\ln\varepsilon\).因为\(\abs{q}<1\)，\(\ln\abs{q}<0\)，故\(n>1+\frac{\ln\varepsilon}{\ln\abs{q}}\).取\(N=\round*{1+\frac{\ln\varepsilon}{\ln\abs{q}}}\)，则当\(n>N\)时，就有\(\abs{q^{n-1}-0}<\varepsilon\)，即\(\lim\limits_{n\to\infty}q^{n-1}=0\).
\end{proof}
\end{example}

\begin{example}
证明：\(\lim\limits_{n\to\infty} \sqrt[n]{n^k} = 1\ (k\in\mathbb{N}^+)\).
\begin{proof}
要证\(\forall\varepsilon>0\)，\(\exists N > 0\)，使得当\(n > N\)时，\begin{gather}
...
\]也即\(\sqrt[n]{n^k} < 1 + \varepsilon\)，\(\sqrt[n]{n} < \sqrt[k]{1+\varepsilon}\)，\begin{gather}
...
\begin{proof}
用反证法.假设当\(n\to\infty\)时，同时有\(x_n \to a\)及\(x_n \to b\)，且\(a < b\).取\(\varepsilon = \frac{b-a}{2}\).因为\(\lim\limits_{n\to\infty}x_n = a\)，所以有\begin{gather}
\exists N_1 \in \mathbb{N}^+ \left(n > N_1 \implies \abs{x_n - a} < \frac{b-a}{2}\right) \tag1
\end{gather}成立.同理，因为\(\lim\limits_{n\to\infty}x_n = b\)，所以有\begin{gather}
\exists N_2 \in \mathbb{N}^+ \left(n > N_2 \implies \abs{x_n - a} < \frac{b-a}{2}\right) \tag2
\end{gather}成立.取\(N = \max\{N_1,N_2\}\)，则当\(n > N\)时，上述两个不等式应同时成立.
...
\begin{proof}
假设这级数收敛，则它具有唯有的极限\(\lim\limits_{n\to\infty}x_n=a\).
按数列极限的定义，对于\(\varepsilon=1/2\)，\(\exists N \in \mathbb{N}^+\)，当\(n > N\)时，\(\abs{x_n-a}<1/2\)或\(x_n\in\left(a-\frac{1}{2},a+\frac{1}{2}\right)\)成立.但这是不可能的，因为\(n\to\infty\)时，\(x_n\)无休止地一再重复取得\(1\)和\(-1\)这两个数，而这两个数不可能同时属于长度为\(1\)的开区间\(\left(a-\frac{1}{2},a+\frac{1}{2}\right)\)内，因此这数列发散.
\end{proof}
\end{example}
...
\abs{x_n} \leqslant M,
\]则称数列\(\{x_n\}\)是\textbf{有界的}；如果这样的正数\(M\)不存在，就说数列\(\{x_n\}\)是\textbf{无界的}.
\end{definition}
...
\begin{proof}
因为数列\(\{x_n\}\)收敛，设\(\lim\limits_{n\to\infty}x_n = a\).根据数列极限的定义，对于\(\varepsilon = 1\)，\(\exists N \in \mathbb{N}^+\)，当\(n > N\)时，不等式\(\abs{x_n - a} < 1\)都成立.于是，当\(n > N\)时，\[
\abs{x_n} = \abs{(x_n - a) + a} \leqslant \abs{x_n - a} + \abs{a} < 1 + \abs{a}.
\]取\(M = \max\{\abs{x_1},\abs{x_2},\dotsc,\abs{x_N},1+\abs{a}\}\)，那么数列\(\{x_n\}\)中的一切\(x_n\)都满足不等式\[
\abs{x_n} \leqslant M.
\]这就证明了数列\(\{x_n\}\)是有界的.
\end{proof}
\end{theorem}
根据上述定理，如果数列\(\{x_n\}\)是无界的，那么数列\(\{x_n\}\)一定发散.但是，如果数列\(\{x_n\}\)有界，却不能断定数列\(\{x_n\}\)一定收敛.
例如数列\[
1,-1,1,\dotsc,(-1)^{n+1},\dotsc,
\]有界，但它是发散的，即数列有界是数列收敛的必要不充分条件.
...
取\(N = \round*{\frac{a}{\varepsilon}}\)，则当\(n>N\)时有\(\abs{x_n-1}<\varepsilon\)，即\[
\ys = 1.
\qedhere
\]
\end{proof}
...
取\(N = \round*{\sqrt\frac{a^2}{\varepsilon}}\)，则当\(n>N\)时有\(\abs{x_n-1}<\varepsilon\)，即\[
\ys = 1.
\qedhere
\]
\end{proof}
...
取\(N=\round{-\lg\varepsilon}\)，则当\(n>N\)时有\(\abs{x_n-1}<\varepsilon\)成立，即\[
...
\subsection{函数极限的定义}
\begin{definition}
设函数\(f(x)\)在点\(x_0\)的某一去心邻域内有定义.如果存在常数\(A\)，对于任意给定的正数\(\varepsilon\)（不论它多么小），总存在正数\(\delta\)，使得当\(x\)满足不等式\(0 < \abs{x-x_0} < \delta\)时，%
对应的函数值\(f(x)\)都满足不等式\(\abs{f(x)-A}<\varepsilon\)，那么常数A就叫做函数\(f(x)\)当\(x \to x_0\)时的极限，记作\[
...
上述对函数极限的定义可以简化为：\[
\lim\limits_{x \to x_0}f(x) = A
\iff \forall \varepsilon > 0, \exists \delta > 0 ( 0 < \abs{x - x_0} < \delta \implies \abs{f(x) - A} < \varepsilon).
\]
\end{definition}
...
\begin{definition}
设函数\(f(x)\)在\(x < x_0\)时有定义.如果存在常数\(A\)，对于任意给定的正数\(\varepsilon\)，总存在正数\(\delta\)，使得当\(x\)满足不等式\(-\delta < x - x_0 < 0\)或\(x_0 - \delta < x < x_0\)时，对应的函数值\(f(x)\)都满足不等式\(\abs{f(x) - A} < \varepsilon\)，那么常数\(A\)就叫做函数\(f(x)\)当\(x \to x_0\)时的\textbf{左极限}，记作\[
\lim\limits_{x \to x_0^-}f(x) = A
\quad \text{或} \quad
f(x_0^-) = A.
\]

类似地，设函数\(f(x)\)在\(x > x_0\)时有定义.如果存在常数\(A\)，对于任意给定的正数\(\varepsilon\)，总存在正数\(\delta\)，使得当\(x\)满足不等式\(0 < x - x_0 < \delta\)或\(x_0 < x < x_0 + \delta\)时，对应的函数值\(f(x)\)都满足不等式\(\abs{f(x) - A} < \varepsilon\)，那么常数\(A\)就叫做函数\(f(x)\)当\(x \to x_0\)时的\textbf{右极限}，记作\[
\lim\limits_{x \to x_0^+}f(x) = A
\quad \text{或} \quad
f(x_0^+) = A.
\]
...
\begin{definition}
设函数\(f(x)\)当\(\abs{x}\)大于某一正数时有定义.如果存在常数\(A\)，对于任意给定的正数\(\varepsilon\)，总存在正数\(X\)，使得当\(x\)满足不等式\(\abs{x} > X\)时，对应的函数值\(f(x)\)都满足不等式\[
...
\begin{definition}
设函数\(f(x)\)在\(x > 0\)时有定义.如果存在常数\(A\)，对于任意给定的正数\(\varepsilon\)，总存在正数\(X\)，使得当\(x\)满足不等式\(x > X\)时，对应的函数值\(f(x)\)都满足不等式\[
\abs{f(x) - A} < \varepsilon,
\]那么常数\(A\)就叫做\textbf{函数\(f(x)\)当\(x \to +\infty\)时的极限}，记作\[
\lim\limits_{x \to +\infty}f(x) = A
\quad \text{或} \quad
...
\]成立.因这个不等式相当于\(\frac{1}{\abs{x}}<\varepsilon\)或\(\abs{x}>\frac{1}{\varepsilon}\).由此可知，如果取\(X=\frac{1}{\varepsilon}\)，那么当\(\abs{x}>X=\frac{1}{\varepsilon}\)时，不等式\(\abs{\frac{1}{x}-0}<\varepsilon\)成立，这就证明了\(\l \frac{1}{x} = 0\).
\end{proof}
\end{example}
...
\begin{theorem}[函数极限的局部保号性]\label{theorem:极限.函数极限的局部保号性}
...
\begin{theorem}[海涅（Heine）定理]\label{theorem:极限.海涅定理}
...
\]
\begin{proof}
必要性.因为\(\lim\limits_{x \to x_0} f(x) = A\)，所以\[
...
充分性.用反证法，假设\(\lim\limits_{x \to x_0} f(x) = A\)不成立，那么，\[
...
这里要注意定理的条件\(x_n \neq x_0\)，因为只要没有这个条件就不能保证海涅定理的结果成立，下面我们举例说明.
设\[
f(x) = \left\{ \begin{array}{cl}
2, & x\neq0, \\
1, & x=0.
\end{array} \right.
\]又设数列\(\{x_n\}\)满足\(x_n=0\ (n=1,2,\dotsc)\)，那么有\[
...
\]这就说明不总有\[
\lim\limits_{n\to\infty} f(x_n) = \lim\limits_{x\to \left(\lim\limits_{n\to\infty} x_n\right) } f(x)
\]成立.
...
\]就称数列\(\{x_n\}\)是\textbf{单调增加的}；如果数列\(\{x_n\}\)满足条件\[
x_n \geqslant x_{n+1}, \quad n=1,2,\dotsc,
\]就称数列\(\{x_n\}\)是\textbf{单调减少的}.单调增加数列和单调减少数列统称为\textbf{单调数列}.
\end{definition}
...
既然数列\(\{x_n\}\)是单调增加的，又是有界的，那么数列\(\{x_n\}\)的极限一定存在，记它的极限值为常数\(e\).再设\(n \leqslant x < n+1\)，则\[
...
上面这个例子中提到的数\(e\)是无理数，它的近似值约为2.718 281 828 459 045.
实际上，以\(e\)为底数的指数函数可以定义为\begin{equation}
...
画出函数\(y=\left(1+\frac{1}{x}\right)^x\)（\(x\in(-\infty,-1)\cup(0,+\infty)\)）的图像如\cref{figure:极限.函数(1+1/x)^x的图像} .

\begin{figure}
\centering
\begin{tikzpicture}[scale=.5]
% Plot[{E, (1 + 1/x)^x}, {x, -10, 10}, PlotRange -> {0, 10}, AspectRatio -> Automatic, PlotStyle -> {{Dashed, Thin}, Thin}]
\begin{axis}[
...
画出函数\(y=\left(1-\frac{1}{x}\right)^x\)（\(x\in(-\infty,0)\cup[1,+\infty)\)）的图像如\cref{figure:极限.函数(1-1/x)^x的图像} .
\begin{figure}
\centering
\begin{tikzpicture}[scale=.5]
% Plot[{1/E, (1 - 1/x)^x}, {x, -10, 10}, PlotRange -> {0, 1}, PlotStyle -> {{Dashed, Thin}, Thin}]
\begin{axis}[
...
数列\(\{x_n\}\)收敛的充分必要条件是：对于任意给定的正数\(\varepsilon\)，存在着这样的正整数\(N\)，使得当\(m>N\)，\(n>N\)时，就有\[
\abs{x_n-x_m}<\varepsilon.
\]
\begin{proof}
必要性.设\(\lim\limits_{n\to\infty}x_n = a\).由数列极限的定义，对于\(\forall\varepsilon>0\)，\(\exists N \in \mathbb{N}^+\)，当\(n > N\)时，有\[
...
=0,
\]那么就称\textbf{函数\(y=f(x)\)在点\(x_0\)连续}.
\end{definition}

函数在点\(x_0\)连续的定义也可叙述如下：
\begin{definition}
设函数\(y=f(x)\)在点\(x_0\)的某一邻域内有定义.如果\[
...
C(I) = \Set*{
	...
	\given
	\forall x \in I :
		\lim\limits_{\increment x\to0} [f(x+\increment x)-f(x)] = 0
}.
\]
\end{definition}
...
\subsection{函数的间断点}
设函数\(f(x)\)在点\(x_0\)的某去心邻域内有定义.在此前提下，如果函数\(f(x)\)有下列三种情形之一：\begin{enumerate}
\item 在\(x=x_0\)没有定义；
\item 虽在\(x=x_0\)有定义，但\(\lim\limits_{x \to x_0} f(x)\)不存在；
\item 虽在\(x=x_0\)有定义，且\(\lim\limits_{x \to x_0} f(x)\)存在，但\(\lim\limits_{x \to x_0} f(x) \neq f(x_0)\)，%
\end{enumerate}则函数\(f(x)\)在点\(x_0\)不连续，而点\(x_0\)称为函数\(f(x)\)的不连续点或间断点.

\begin{definition}
如果函数\(y=f(x)\)在点\(x_0\)不连续，则点\(x_0\)称为函数\(f(x)\)的\textbf{不连续点}或\textbf{间断点}.

如果\(\lim\limits_{x \to x_0}f(x) = \infty\)，则称点\(x_0\)为函数的\textbf{无穷间断点}.
如果\(f(x)\)在点\(x_0\)的某一邻域是有界的，但其左、右极限均不存在，则称点\(x_0\)为函数的\textbf{振荡间断点}.
如果\(f(x)\)在点\(x_0\)没有定义，\(\lim\limits_{x \to x_0}f(x) = A\)，则称点\(x_0\)为函数的\textbf{可去间断点}.
如果\(f(x)\)在点\(x_0\)的左、右极限存在但不相等，即\(\lim\limits_{x \to x_0^-}f(x) \neq \lim\limits_{x \to x_0^+}f(x)\)，则称点\(x_0\)为函数的\textbf{跳跃间断点}.

如果\(x_0\)是函数\(f(x)\)的间断点，但左极限\(f(x_0^-)\)及右极限\(f(x_0^+)\)都存在，那么\(x_0\)称为函数\(f(x)\)的\textbf{第一类间断点}.
不是第一类间断点的间断点，称为\textbf{第二类间断点}.
\end{definition}
...
设函数\(f(x)\)在闭区间\([a,b]\)上连续，且\(f(a)\)与\(f(b)\)异号（即\(f(a) \cdot f(b)<0\)），那么在开区间\((a,b)\)内至少有一点\(\xi\)，使\[
f(\xi) = 0.
\]
\end{theorem}
从几何上看，零点定理表示：如果连续曲线弧\(y = f(x)\)的两个端点位于\(x\)轴的不同侧，那么这段曲线弧与\(x\)轴至少有一个交点.

\begin{theorem}[介值定理]\label{theorem:极限.闭区间上连续函数的性质.介值定理}
...
\]
\end{theorem}
这定理的几何意义是：连续曲线弧\(y=f(x)\)与水平直线\(y=C\)至少相交于一点.
...
\subsection{一致连续性}
\begin{definition}
设函数\(f(x)\)在区间\(I\)上有定义.如果对于任意给定的正数\(\varepsilon\)，总存在着正数\(\delta\)，使得对于区间\(I\)上的任意两点\(x_1\)、\(x_2\)，当\(\abs{x_1 - x_2} < \delta\)时，就有\[
\abs{f(x_1) - f(x_2)} < \varepsilon,
\]那么称函数\(f(x)\)在区间\(I\)上是\textbf{一致连续的}（uniformly continuous），记作\(f \in C_U(I)\).
\end{definition}
函数的一致连续性表示：不论在区间\(I\)的任何部分，只要自变量的两个数值接近到一定程度，就可使对应的两个函数值达到所指定的接近程度.
...
需要注意的是，说“函数\(f\)是一致连续的”时一定要讲明定义域.
某些函数在区间\(I_1\)上不是一致连续的，但在另外的区间\(I_2\)上却一致连续.

\begin{example}
证明：函数\(f(x)=x\)在区间\((-\infty,+\infty)\)上是一致连续的.
\begin{proof}
因为\(\abs{f(x_1)-f(x_2)}=\abs{x_1-x_2}\)，所以\(\forall\varepsilon>0\)，总可取\(\delta=\varepsilon\)，对于\(\forall x_1,x_2\in\mathbb{R}\)，当\(0<\abs{x_1-x_2}<\delta=\varepsilon\)时，能使不等式\[
\abs{f(x_1)-f(x_2)} = \abs{x_1-x_2} < \varepsilon
\]成立，也就是说\(f(x)\)是一致连续的.
\end{proof}
\end{example}
上例说明，在某区间上无界的函数也可以是一致连续的.
...
\end{align*}而当\(\abs{x_1-x_2} \leqslant \pi\)时有\[
\abs{\sin\frac{x_1-x_2}{2}} \leqslant \abs{\frac{x_1-x_2}{2}}
\]成立，所以\(\forall\varepsilon>0\)（设\(\varepsilon<\pi\)），总可取\(\delta=\varepsilon\)，对于\(\forall x_1,x_2\in\mathbb{R}\)，当\(0<\abs{x_1-x_2}<\delta=\varepsilon\)时，能使不等式\[
...
\bigr],
\]也就是说对\(\forall x_0 \in I\)都有\(\lim\limits_{x \to x_0}f(x) = f(x_0)\)，即\(f(x)\)在区间\(I\)上连续.
\end{proof}
\end{theorem}
...
\begin{example}
设函数\(f(x)\)对于\(\forall x,y \in [a,b]\)，恒有\(\abs{f(x) - f(y)} \leqslant L \abs{x - y}\)，其中常数\(L > 0\)，且\(f(a) \cdot f(b) < 0\).证明：至少有一点\(\xi \in (a,b)\)，使得\(f(\xi) = 0\).
\begin{proof}
因为\(\forall x,y \in [a,b] \implies \abs{f(x) - f(y)} \leqslant L \abs{x - y}\)，对于\(\forall \varepsilon > 0\)，为了使当\(\abs{x - y} < \delta\)时不等式\[
\abs{f(x) - f(y)} \leqslant L \abs{x - y} < \varepsilon
\]成立，只要\(L \delta = \varepsilon\)或\(\delta = \varepsilon / L\)即可.这说明\(f(x)\)在区间\([a,b]\)上一致连续.再由零点定理可知命题成立.
\end{proof}
\end{example}

\begin{example}
试证：若\(f(x)\)在\([a,b]\)上连续，\(a < x_1 < x_2 < \dotsb < x_n < b\)（\(n \geqslant 3\)），则在\((x_1,x_n)\)内至少有一点\(\xi\)，使\[
...
\begin{theorem}\label{theorem:极限.闭区间上连续函数的性质.开区间上的连续函数一致连续的充要条件}
开区间\(I=(a,b)\)内的连续函数\(f\)一致连续的充要条件是：
极限\(f(a^+)\)和\(f(b^-)\)都存在.

特别地，若上述开区间\(I=(-\infty,+\infty)\)，那么连续函数\(f\)一致连续的充要条件是：
...
\begin{theorem}[施托尔兹（O'Stolz）公式]\label{theorem:极限.施托尔兹公式}
设数列\(\{A_n\}\)、\(\{B_n\}\).若当\(n\to\infty\)时，有\(A_n\to0\)且\(B_n\to0\)成立，或有\(B_n \to \infty\)成立，那么有\[
...
\]
\end{theorem}
施托尔兹公式实际上是离散形式的洛必达法则，它适用于\(\frac{0}{0}\)、\(\frac{\infty}{\infty}\)或\(\frac{*}{\infty}\)三种情形.
...
\begin{theorem}[斯特林（Stirling）公式]\label{theorem:极限.斯特林公式}
