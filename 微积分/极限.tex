...
依照界限的定义会有\(a_m \in B\)，而这与\(B\)的定义矛盾.
假设\(\alpha\)不是\(\{a_n\}\)的极限，
则存在正常数\(\varepsilon\)，使得对任意正整数\(n\)都有\[
	\alpha - a_n > \varepsilon
	\quad\text{或}\quad
	a_n < \alpha - \varepsilon.
\]
按照集合\(B\)的定义，由此可得\(\alpha - \varepsilon \in B\)；
但由\(\alpha - \varepsilon < \alpha\)可知，\((\alpha - \varepsilon) \in A\)；
矛盾！因此\(\alpha\)就是数列\(\{a_n\}\)的极限，即\(\lim\limits_{n\to\infty} a_n = \alpha\).
\end{proof}
\end{theorem}
