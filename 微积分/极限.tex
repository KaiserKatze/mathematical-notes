...
上面对上下极限的定义可以利用“\(\epsilon-N\)语言”更加简洁精确地重新定义为\begin{align*}
	...
	(\forall\epsilon>0)
	(\exists N\in\mathbb{N}^+)
	[n>N \implies \alpha-\epsilon < x_n]; \\
	\varlimsup\limits_{n\to\infty} x_n = \beta
	&\defiff
	(\forall\epsilon>0)
	(\exists N\in\mathbb{N}^+)
	[n>N \implies x_n < \beta+\epsilon].
\end{align*}
...
\forall \epsilon > 0, \exists \delta > 0 \bigl[
	0 < \abs{x - x_0} < \delta \implies \abs{f(x) - A} < \epsilon
\bigr].
\]
...
\bigr).
\]这样，当\(n> N\)时，有\(\abs{f(x_n) - A} < \epsilon\)，即\(\lim\limits_{n \to \infty} f(x_n) = A\).

充分性.
用反证法，假设\(\lim\limits_{x \to x_0} f(x) = A\)不成立，那么，\[
\exists \epsilon_0 > 0, \forall \delta > 0, \exists \xi\in\mathring{U}(x_0,\delta) \bigl[
...
\forall \epsilon > 0, \exists N > 0 \bigl(
	n > N \implies \abs{f(\xi_n) - A} < \epsilon
\bigr).
...
综上所述，\[
	\forall\epsilon>0,
	\exists N\in\mathbb{N}^+
	\bigl( n > N \implies \abs{x_n - a} < \epsilon \bigr);
\]
