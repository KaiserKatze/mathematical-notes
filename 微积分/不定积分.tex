...
\subsection{有理函数的积分}
\begin{definition}
两个（不含公因式的）多项式的商\(\frac{P(x)}{Q(x)}\)称为\DefineConcept{有理函数}，又称\DefineConcept{有理分式}.
当分子多项式\(P(x)\)的幂次小于分母多项式\(Q(x)\)的幂次时，称这有理函数为\DefineConcept{真分式}；否则称为\DefineConcept{假分式}.
\end{definition}
...
\]上述步骤将真分式化为\DefineConcept{部分分式}之和.
