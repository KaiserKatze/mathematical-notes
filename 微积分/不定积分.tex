...
\]那么函数\(F(x)\)就称为\(f(x)\)（或\(f(x) \dd{x}\)）在区间\(I\)上的\textbf{原函数}.
\end{definition}
...
\begin{definition}
在区间\(I\)上，函数\(f(x)\)的带有任意常数项的原函数称为\(f(x)\)（或\(f(x) \dd{x}\)）在区间\(I\)上的\textbf{不定积分}，记作\[
\int f(x) \dd{x},
\]其中记号\(\int\)称为\textbf{积分号}，\(f(x)\)称为\textbf{被积函数}，\(f(x)\dd{x}\)称为\textbf{被积表达式}，\(x\)称为\textbf{积分变量}.
\end{definition}
...
\subsection{有理函数的积分}
\begin{definition}
两个（不含公因式的）多项式的商\(\frac{P(x)}{Q(x)}\)称为\textbf{有理函数}，又称\textbf{有理分式}.
当分子多项式\(P(x)\)的幂次小于分母多项式\(Q(x)\)的幂次时，称这有理函数为\textbf{真分式}；否则称为\textbf{假分式}.
\end{definition}
