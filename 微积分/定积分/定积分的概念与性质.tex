...
\begin{example}
%@credit: {76561d2a-b7c9-48eb-ad75-aa1b4f8e85e7} 分享的题目和答案
利用定积分的定义计算\(\lim_{n\to\infty} \sum_{k=1}^n \frac{k}{n^2+k^2+1}\).
\begin{solution}
记\begin{equation*}
	I_n \defeq \sum_{k=1}^n \frac{k}{n^2+k^2+1},
	\qquad
	J_n \defeq \sum_{k=1}^n \frac{k}{n^2+k^2}.
\end{equation*}

因为\begin{align*}
	\lim_{n\to\infty} \abs{I_n - J_n}
	&= \lim_{n\to\infty} \abs{
		\sum_{k=1}^n \left( \frac{k}{n^2+k^2+1} - \frac{k}{n^2+k^2} \right)
	} \\
	&= \lim_{n\to\infty} \sum_{k=1}^n \frac{k}{(n^2+k^2)(n^2+k^2+1)} \\
	&\leq \lim_{n\to\infty} \sum_{k=1}^n \frac{k}{n^2 \cdot n^2}
	= \lim_{n\to\infty} \frac1{n^4} \sum_{k=1}^n k \\
	&= \lim_{n\to\infty} \frac1{n^4} \cdot \frac{n(n+1)}2
	= \lim_{n\to\infty} \frac{n+1}{2n^3}
	= 0,
\end{align*}
所以由\cref{example:数列极限的概念.柯西列的等价性} 有\begin{align*}
	\lim_{n\to\infty} I_n
	&= \lim_{n\to\infty} J_n
...
\end{align*}
\end{solution}
%@Mathematica: Limit[Sum[k/(n^2 + k^2 + 1), {k, 1, n}], n -> Infinity] // FullSimplify
\end{example}
