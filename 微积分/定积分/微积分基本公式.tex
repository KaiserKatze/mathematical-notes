...
%TODO
% \begin{solution}
% 设函数\(f\colon[a,b]\to\mathbb{R}\)在\([a,c)\cup(c,b]\)上连续，
% 点\(c\in(a,b)\)是\(f\)的可去间断点，
% 函数\[
...
% 	\end{array} \right.
% \]
% 显然\(f\)在\([a,b]\)上可积，\(g\)在\([a,b]\)上连续，且\[
% 	\int_a^x f(t) \dd{t} = \int_a^x g(t) \dd{t}
% 	\quad(a \leq x \leq b).
% \]
% 由\cref{theorem:定积分.变限积分定理}
% 可知积分上限函数\(x \mapsto \int_a^x g(t) \dd{t}\)在\([a,b]\)上可导，
% 并且它的导函数是\[
% 	\dv{x} \int_a^x g(t) \dd{t}
% 	= g(x).
% \]
% \end{solution}
\end{example}
