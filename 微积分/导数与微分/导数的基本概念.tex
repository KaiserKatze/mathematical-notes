...
当\(\increment x\to0\)时的极限存在，
则称“函数\(y=f(x)\)在点\(x_0\)~\DefineConcept{可导}%
（\(f\) is \emph{differentiable} at \(x_0\)）”，
或“\(f(x)\)在点\(x_0\)具有导数”，
或“\(f(x)\)在点\(x_0\)导数存在”.
并把这个极限称为“函数\(f\)在点\(x_0\)处的\DefineConcept{导数}%
...
\]称为“函数\(f\)在点\(x_0\)处的\DefineConcept{左导数}（left-sided derivative）”，
记作\(f'_-(x_0)\).

把极限\[
	\lim_{h\to0^+} \frac{f(x_0+h)-f(x_0)}{h}
\]称为“函数\(f\)在点\(x_0\)处的\DefineConcept{右导数}（right-sided derivative）”，
记作\(f'_+(x_0)\).
