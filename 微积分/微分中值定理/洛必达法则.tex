...
\begin{example}%野题
设函数\(f\)在点\(x=0\)二阶可导，
且\(\lim_{x\to0} \frac{f(x)}{x^3} = 1\).
证明极限\(\lim_{x\to0} \frac{f'(x)}{x}\)存在，
并说明为什么不能用洛必达法则计算该极限.
\begin{proof}
由\cref{theorem:高阶导数.在一点高阶可导是在该点某邻域内低阶可导的充分条件}
可知\(f\)在点\(x=0\)的某个邻域内一阶连续可导，
从而有\(f(0) = 0\)，
于是\begin{equation*}
	f'(0) = \lim_{x\to0} \frac{f(x)}{x}
	= \lim_{x\to0} \frac{f(x)}{x^3} \cdot \frac{x^3}{x}
	= \lim_{x\to0} \frac{f(x)}{x^3} \cdot \lim_{x\to0} \frac{x^3}{x}
	= 1 \cdot 0
	= 0.
\end{equation*}
因为\(f\)在点\(x=0\)二阶可导，
...
%@credit: {ce603838-a24d-4616-9395-d7b223e8cb72} 指出\(f'\)在点\(x=0\)不连续，我才发现要用洛必达法则，必须要满足分子是无穷小
但是，由于我们不知道是否成立\(\lim_{x\to0} f'(x) = 0\)，
无法满足洛必达法则所要求的条件，
所以不能用洛必达法则计算\(\lim_{x\to0} \frac{f'(x)}{x}\).
\end{proof}
\end{example}
