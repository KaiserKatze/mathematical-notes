...
假设\(a_n,b_n\neq0\)，
求有理分式函数\(F\)当\(x\to\infty\)时的极限\(\lim_{x\to\infty} F(x)\)时，
则\begin{equation}\label{equation:函数极限.重要极限5}
	\lim_{x\to\infty} F(x)
	= \left\{ \begin{array}{cl}
		a_n/b_n, & m=n, \\
		0, & n<m, \\
...
直接计算得\begin{align*}
	...
	= \frac12 \lim_{x\to0} \frac{\sin^2\frac{x}{2}}{\left(\frac{x}{2}\right)^2}
	= \frac12 \left(\lim_{x\to0} \frac{\sin \frac{x}{2}}{\frac{x}{2}}\right)^2
	= \frac12.
\end{align*}
%\label{equation:函数极限.重要极限4}
\end{solution}
\end{example}
