...
\begin{itemize}
	...
	\end{equation*}
	称为“函数\(f\)在点\(x_0\)的\DefineConcept{上极限}（upper limit）”，
	...
	\end{equation*}
	称为“函数\(f\)在点\(x_0\)的\DefineConcept{下极限}（lower limit）”，
...
设\(f\colon \mathbb{R} \to \overline{\mathbb{R}}\)，
则\begin{gather*}
	\varlimsup_{x \to x_0} f(x)
	= \inf_{\delta > 0} \left( \sup_{0<\abs{x-x_0}<\delta} f(x) \right), \\
	\varliminf_{x \to x_0} f(x)
	= \sup_{\delta > 0} \left( \inf_{0<\abs{x-x_0}<\delta} f(x) \right).
\end{gather*}
\end{theorem}
