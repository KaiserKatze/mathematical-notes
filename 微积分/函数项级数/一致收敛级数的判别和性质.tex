...
设函数项级数\(\sum_{n=1}^\infty u_n(x)\)在\([a,b]\)上一致收敛于函数\(S\)，
...
设函数列\(\{S_n\}\)满足\begin{itemize}
	...
	\item 函数列\(\{S_n\}\)在\([a,b]\)上点态收敛于函数\(S\)，
	\item 导函数列\(\{S_n'\}\)在\([a,b]\)上一致收敛于函数\(\sigma\)，
\end{itemize}
则函数\(S\)在区间\([a,b]\)上可导，
且\[
	\dv{x} S(x) = \sigma(x).
...
设函数项级数\(\sum_{n=1}^\infty u_n(x)\)满足\begin{itemize}
	...
	\item 函数项级数\(\sum_{n=1}^\infty u'_n(x)\)在区间\([a,b]\)上一致收敛于\(\sigma\)，
\end{itemize}
则函数\(S\)在区间\([a,b]\)上可导，
且求导运算与无限求和运算可以交换次序，
即\[
