...
可以将级数\(\sum\limits_{n=0}^\infty b_n x^n\)
与\(\sum\limits_{n=0}^\infty c_n x^n\)相乘，
...
相除后所得的幂级数\(\sum\limits_{n=0}^\infty c_n x^n\)的收敛区间可能比原来的两级数的收敛区间小得多.
\end{definition}
...
\]在整个数轴上收敛，
但是这两个级数的商\(\sum\limits_{n=0}^\infty c_n x^n
= \sum\limits_{n=0}^\infty x^n\)仅在区间\((-1,1)\)内收敛.
...
幂级数\(\sum\limits_{n=0}^\infty a_n x^n\)的和函数\(s(x)\)在其收敛域\(I\)上可积，
并有逐项积分公式\begin{equation}
	\int_0^x s(x) \dd{x}
	=\int_0^x \left(\sum\limits_{n=0}^\infty a_n x^n\right) \dd{x}
	=\sum\limits_{n=0}^\infty \int_0^x a_n x^n \dd{x}
...
且有逐项求导公式\begin{equation}
	s'(x)
	= \dv{x}( \sum\limits_{n=1}^\infty a_n x^n )
	= \sum\limits_{n=0}^\infty (a_n x^n)'
