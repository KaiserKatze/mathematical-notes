...
由于这个函数是通过逐点定义的方式得到的，
因此我们称“级数\(\sum_{n=1}^\infty u_n(x)\)在\(D\)上\DefineConcept{点态收敛}于\(S\)”.
...
\begin{example}\label{example:函数项级数.一致收敛的函数列}
...
\end{proof}
\end{theorem}
回过头来，对于\cref{example:函数项级数.一致收敛的函数列} 中的
...
对于\cref{example:函数项级数.不一致收敛的函数列} 中的
\(S_n(x) = x^n\ (0 \leq x < 1)\)，
由于\[
