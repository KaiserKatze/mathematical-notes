\section{二重积分}
\subsection{二重积分的概念}
设\(f(x,y)\)是有界闭区域\(D\)上的有界函数.将闭区域\(D\)任意分成\(n\)个小闭区域\begin{equation*}
...
\begin{example}
%@see: https://www.bilibili.com/video/BV1SriiYEEFa/
...
\begin{solution}
%@Mathematica: Integrate[Abs[x y - 1], {x, 0, 2}, {y, 0, 2}]
首先有\begin{align*}
...
\begin{example}
设\(f(x,y) = f_1(x) \cdot f_2(y)\)，证明：\begin{equation*}
...
\begin{example}
%@see: https://www.bilibili.com/video/BV1JZ5i6gEAk
% 原题题干给出的条件是“二元函数\(f\)在点\((0,0)\)连续”，但是这是错误的
% 因为它不能保证“二元函数\(f\)在某个闭区域上连续”，从而无法直接使用现有的二重积分的中值定理
设\(f\)在点\((0,0)\)的某个邻域内连续，且\(f(0,0) = 2\).
计算极限：\begin{equation*}
	\lim_{t \to 0} \frac{1}{t^2} \iint_{D_t} f(x,y) \dd{x} \dd{y},
\end{equation*}
其中\(D_t \defeq \Set{(x,y) \given x^2 + y^2 \leq t^2}\).
\begin{solution}
由中值定理可知，存在一点\((\xi,\eta)\)使得\begin{equation*}
	\iint_{D_t} f(x,y) \dd{x} \dd{y}
	= \sigma f(\xi,\eta),
\end{equation*}
其中\(\sigma = \iint_{D_t} \dd{x} \dd{y} = \pi t^2\).
因此\begin{equation*}
	\lim_{t \to 0} \frac{1}{t^2} \iint_{D_t} f(x,y) \dd{x} \dd{y}
	= \lim_{t \to 0} \frac{1}{t^2} \pi t^2 f(\xi,\eta)
	= \pi \lim_{t \to 0} f(\xi,\eta)
	= \pi f(0,0)
	= 2\pi.
\end{equation*}
\end{solution}
...
这个先对\(y\)、后对\(x\)的二次积分也常记作\begin{equation*}
...
类似地，设积分区域\(D\)可以用不等式\begin{equation*}
...
如果积分区域\(D\)既是X型的，可用不等式\begin{equation*}
	a \leq x \leq b, \qquad
	\phi_1(x) \leq y \leq \phi_2(x)
\end{equation*}表示，
又是Y型的，可用不等式\begin{equation*}
	\psi_1(y) \leq x \leq \psi_2(y), \qquad
	c \leq y \leq d
\end{equation*}表示，
则有\begin{equation*}
...
\end{equation*}
上式表明，这两个不同次序的二次积分相等，因为它们都等于同一个二重积分\begin{equation*}
	\iint_D f(x,y) \dd{\sigma}.
\end{equation*}
...
\begin{example}
%@see: 《数学分析（第二版 下册）》（陈纪修） P254 习题 17.
利用重积分的性质和计算方法证明：
若\(f \in C[a,b]\)，则\begin{equation*}
...
下面我们按二重积分的定义\begin{equation*}
	\iint_D f(x,y) \dd{\sigma}
	= \lim_{\lambda\to0} \sum_{i=1}^n f(\xi_i,\eta_i) \increment\sigma_i
\end{equation*}来研究一下这个和的极限在极坐标系中的形式.
...
\end{gather*}来表示，其中函数\(\phi_1(\theta)\)、\(\phi_2(\theta)\)在区间\([\alpha,\beta]\)上连续，那么利用以下关系\begin{equation*}
\left\{ \begin{array}{l}
x = \rho\cos\theta \\
y = \rho\sin\theta \\
\end{array} \right.
...
\tan\theta = y/x \quad (x \neq 0) \\
\end{array} \right.,
\end{equation*}替换二重积分\(\iint_{D}{f(x,y)\dd{x}\dd{y}}\)中的积分变量，即有
\begin{equation}
\iint_D f(\rho\cos\theta,\rho\sin\theta) \rho \dd{\rho} \dd{\theta}
...
\begin{proof}
以圆心为原点建立极坐标系，得圆内的方程为\begin{equation*}
	D: \rho \leq r.
\end{equation*}
那么圆内的面积为\begin{equation*}
...
由于\(e^{-x^2-y^2} > 0\)，
从而在这些闭区域上的二重积分之间有有不等式\begin{equation*}
...
变换\begin{equation*}
	T\colon \left\{ \begin{array}{l}
		...
		y = y(u,v),
	\end{array} \right.
\end{equation*}将\(uOv\)平面上的闭区域\(G\)变为\(xOy\)平面上的\(D\)，
且满足：\begin{enumerate}
...
任意一点\((\rho,\theta)\)的雅克比式为\begin{equation*}
	J(u,v)
	= \jacobi{x,y}{\rho,\theta}
...
\end{equation*}
它仅在\(\rho = 0\)处为零，
故不论闭区域\(G\)是否含有极点，换元公式仍成立，即有\begin{equation*}
	\iint_D f(x,y) \dd{x} \dd{y}
...
\subsection{二重积分被积函数的对称性}
\begingroup
%@see: https://www.bilibili.com/video/BV1Zf421U72F/
% 二重积分的奇偶对称性
...
%@see: https://math.libretexts.org/Bookshelves/Calculus/CLP-3_Multivariable_Calculus_(Feldman_Rechnitzer_and_Yeager)/03%3A_Multiple_Integrals/3.01%3A_Double_Integrals
设积分区域\(D\)关于原点对称.
当被积函数\(f\)在区域\(D\)上是奇函数，
即\begin{equation*}
	f(-x,-y) = -f(x,y)
...
%@Mathematica: Manipulate[Plot[y[x] /. Solve[\[Lambda] x + \[Mu] y[x] == 0, y[x]], {x, -5, 5}, PlotRange -> {-5, 5}, AspectRatio -> 1], {\[Lambda], -5, 5}, {\[Mu], -5, 5}]
%@Mathematica: Manipulate[RegionPlot[\[Lambda] x + \[Mu] y >= 0, {x, -5, 5}, {y, -5, 5}], {\[Lambda], -5, 5}, {\[Mu], -5, 5}]
...
\end{align*}
\end{solution}
%@Mathematica: Integrate[(x^8 y + x^2 y^2) Boole[Abs[x] + Abs[y] <= 1], {x, -1, 1}, {y, -1, 1}]
%@Mathematica: 4 Integrate[x^2 Integrate[y^2, {y, 0, 1 - x}], {x, 0, 1}]
\end{example}

%@see: https://www.bilibili.com/video/BV1G1421z7Ew/
% 二重积分的轮换对称性
...
\end{equation*}
\end{solution}
%@Mathematica: Integrate[Max[x^2, y^2], {x, 0, 1}, {y, 0, 1}]
\end{example}

若\(D\)关于\(y=x\)对称，则\begin{equation*}
...
\def\op{\underset{(>)}{=}}
若将\(D\)中的\(x,y\)对换后，\(D\)不变，则有\begin{equation*}
	\iint_D f(x,y) \dd{x}\dd{y} = \iint_D f(y,x) \dd{x}\dd{y} = I.
\end{equation*}
若进一步有\(f(x,y)+f(y,x) \op a\)，则\begin{equation*}
...
%@see: https://www.bilibili.com/video/BV1fM4m1m7Nq/
\begin{example}
...
直接计算得\begin{align*}
	...
	&= \iint_D (x^2+4xy+4y^2) \dd{\sigma} \\ % 函数\(f_2(x,y) = xy\)在\(D\)上的二重积分等于0
...
\begin{example}
%@see: https://www.bilibili.com/video/BV1Ju4y1g752/
...
\end{equation*}
因为\(D\)关于直线\(x=1\)对称，所以\begin{equation*}
	\iint_D (x-1) \dd{\sigma}
	= 0.
\end{equation*}
因为\(D\)关于直线\(y=1\)对称，所以\begin{equation*}
	\iint_D (y-1) \dd{\sigma}
	= 0.
\end{equation*}
于是\begin{equation*}
...
\begin{example}
%@see: 《2021年全国硕士研究生入学统一考试（数学二）》三解答题/第21题
设曲线\((x^2+y^2)^2=x^2-y^2\ (x\geq0,y\geq0)\)与\(x\)轴围成的区域\(D\).
求\(\iint_D x y \dd{x}\dd{y}\).
\begin{solution}
%@see: https://www.bilibili.com/video/BV16XUkY7E3J/
%@see: https://www.bilibili.com/video/BV1EBqrYUEUc/
...
\end{equation*}
\end{solution}
%@Mathematica: RegionPlot[(x^2 + y^2)^2 <= x^2 - y^2, {x, 0, 1}, {y, 0, 1}]
%@Mathematica: Integrate[x y Boole[(x^2 + y^2)^2 <= x^2 - y^2], {x, 0, 1}, {y, 0, 1}]
\end{example}

\begin{example}
计算二重积分\(\iint_D x^2 \dd{x}\dd{y}\)，
其中\(D: x^2+xy+y^2\leq2\).
%TODO
%@Mathematica: Integrate[x^2 Boole[x^2 + x y + y^2 <= 2], {x, -2, 2}, {y, -2, 2}]
%@Mathematica: RegionPlot[x^2 + x y + y^2 <= 2, {x, -2, 2}, {y, -2, 2}]
\end{example}
