...
如果函数\(f\colon X \times Y \to \mathbb{R}\)在\(X \times Y\)上可积，则\(f\)的重积分与累次积分同时存在且彼此相等，即\begin{equation*}
...
如果区间\(I \subseteq \mathbb{R}^n\)是闭区域\(I_i = [a_i,b_i]\ (i=1,2,\dotsc,n)\)的直积，则\begin{equation*}
	\int_I f(x) \dd{x}
...
并且在闭区域\(\Omega'\)内雅克比行列式\begin{equation*}
	J = \det\vb{J}
