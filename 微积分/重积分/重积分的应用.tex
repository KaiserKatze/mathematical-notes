...
\begin{theorem}
设曲面\(S\)由方程\begin{equation*}
	z=f(x,y)
\end{equation*}给出，\(D_{xy}\)为曲面\(S\)在\(xOy\)面上的投影区域，
函数\(f(x,y)\)在\(D_{xy}\)上具有连续偏导数\(f'_x(x,y)\)和\(f'_y(x,y)\).
那么曲面\(S\)面积元素\(\dd{A}\)为\begin{equation*}
...
\begin{example}
%@see: 《2017年全国硕士研究生入学统一考试（数学一）》三解答题/第19题
...
于是\begin{align*}
	...
	%\cref{equation:定积分.点火公式1}
	= 64.
\end{align*}
\end{solution}
%@Mathematica: Plot3D[{Sqrt[x^2 + y^2], Sqrt[2 x]}, {x, -2, 2}, {y, -2, 2}, PlotLegends -> "Expressions", AxesLabel -> Automatic]
\end{example}
...
质量分别为\(\AutoTuple{m}{n}\).
根据定义，该质点系的质心的坐标为\begin{equation*}
...
\end{equation*}分别是该质点系对\(y\)轴和\(x\)轴的\emph{静矩}.
...
这部分质量可近似看作集中在点\((x,y)\)上，于是可以写出对应的静矩元素\(\dd{M_y}\)和\(\dd{M_x}\)：\begin{equation*}
	\dd{M_y} = x \mu(x,y) \dd{\sigma},
	\qquad
	\dd{M_x} = y \mu(x,y) \dd{\sigma}.
\end{equation*}
以这些元素为被积表达式，在闭区域\(D\)上积分，便得\begin{equation*}
	M_y = \iint_D x \mu(x,y) \dd{\sigma},
	\qquad
	M_x = \iint_D y \mu(x,y) \dd{\sigma}.
\end{equation*}
我们知道，薄片的质量为\begin{equation*}
	M = \iint_D \mu(x,y) \dd{\sigma}.
\end{equation*}
所以薄片的质心的坐标为\begin{equation*}
	\overline{x} = \frac{M_y}{M}
...
即\(\overline{x} = 0\).
再按\cref{equation:重积分.均匀平面薄片的形心坐标} 可得\begin{equation*}
	\overline{y} = \frac1A \iint_D y \dd{\sigma},
\end{equation*}
