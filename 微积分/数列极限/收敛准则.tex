...
\begin{example}\label{example:收敛准则.迭代法计算开平方}
...
\end{example}
\begin{remark}
上述递推公式\(x_{n+1} = \frac12\left(x_n+\frac{a}{x_n}\right)\ (n=1,2,\dotsc)\)
是求正数\(a\)的平方根的最简迭代算法.
类似地，我们还可以给出计算立方根\(\sqrt[3]{a}\)的递推公式：\begin{equation*}
%@see: https://mathworld.wolfram.com/CubeRoot.html
	x_{n+1} = \frac13 \left( 2 x_n + \frac{a}{x_n^2} \right).
\end{equation*}
%TODO 上面的迭代法计算开立方的公式未经验证
\end{remark}
...
\begin{theorem}\label{theorem:极限.上下极限的等价定义2}
%@see: 《Real Analysis Modern Techniques and Their Applications Second Edition》（Gerald B. Folland） P11
数列\(\{x_n\}\)的上下极限满足：\begin{gather*}
	\varliminf_{n\to\infty} x_n
	= \sup_{k \geq 1} \left( \inf_{n \geq k} x_n \right), \\
	\varlimsup_{n\to\infty} x_n
	= \inf_{k \geq 1} \left( \sup_{n \geq k} x_n \right).
\end{gather*}
\end{theorem}

\cref{theorem:极限.上下极限的等价定义1,theorem:极限.上下极限的等价定义2}
都是数列的上下极限的等价定义.
