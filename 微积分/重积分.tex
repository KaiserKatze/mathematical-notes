...
如果\(f(x,y,z)\)表示某物体在点\((x,y,z)\)处的密度，\(\Omega\)是该物体所占有的空间闭区域，\(f(x,y,z)\)在\(\Omega\)上连续，则\(\sum_{i=1}^n f(\xi_i,\eta_i,\zeta_i) \increment v_i\)是该物体的质量\(m\)的近似值，这个和当\(\lambda\to0\)时的极限就是该物体的质量\(m\)，所以\[
...
\]其中，\(\dd{v} = r^2 \sin\phi \dd{r} \dd{\phi} \dd{\theta}\)称为\DefineConcept{球面坐标系中的体积元素}.
...
，我们不能直接应用曲面面积公式，
所以先取区域\[
	D_1 = \Set{(x,y) \given x^2+y^2 \leq r^2}
	\quad(0<r<R)
\]为积分区域，
算出相应于\(D_1\)上的球面面积\[
	A_1 = \iint_{D_1} \frac{R}{\z} \dd{x}\dd{y}
\]后，
令\(r \to R\)取\(A_1\)的极限就得半球面的面积.
