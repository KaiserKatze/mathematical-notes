...
继而公式 \labelcref{equation:微分中值定理.泰勒公式1} 称为“\(f(x)\)按\((x-x_0)\)的幂展开的\cemph{带有拉格朗日型余项}的\(n\)阶\DefineConcept{泰勒公式}（Taylor's formula with the Lagrange form of the remainder）”.
...
因此可以令\(\xi = \theta x\ (0 < \theta < 1)\)，从而泰勒公式变成较简单的形式，即所谓“带有拉格朗日型余项的\DefineConcept{麦克劳林公式}”（Maclaurin's formula with the Lagrange form of the remainder）：
...
= M(a^2 + x_0^2).
\]因为\(x_0\in(-a,a)\)，则\(\abs{f(a) - f(-a)} \leqslant M(a^2+x_0^2) \leqslant 2 M a^2\)，%
则\(M \geqslant \frac{1}{2 a^2} \abs{f(a) - f(-a)}\)，即存在\(\eta\in\{\gamma_1,\gamma_2\}\subseteq(-a,a)\)，使得\(\abs{f''(\eta)} \geqslant \frac{1}{2 a^2} \abs{f(a) - f(-a)}\).
命题2得证.
\end{proof}
\end{example}
