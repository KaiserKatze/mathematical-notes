...
\item 在开区间\((a,b)\)内可导；
\end{enumerate}
那么\(\exists \xi \in (a,b)\)使得\(f(b)-f(a)=f'(\xi)(b-a)\)成立.
\end{theorem}
...
\end{proof}
实际上该极限被称为\textbf{欧拉-马歇罗尼常数}，记作\(\gamma\)，其在数值上近似等于0.577 216.
\end{example}

利用拉格朗日中值定理公式可以对函数增量\(\increment y\)作出更精确的估计.
设\(x\)为区间\([a,b]\)内一点，\(x + \increment x\)为这区间内另一点（\(\increment \gtrless 0\)），则拉格朗日中值定理公式在区间\([x,x + \increment x]\)（当\(\increment x > 0\)时）或在区间\([x + \increment x,x]\)（当\(\increment x < 0\)时）上就成为\[
f(x + \increment x) - f(x) = f'(x + \theta \increment x) \cdot \increment x,
\quad 0 < \theta < 1.
\]如果记\(y=f(x)\)，那么上式又可写成\[
\increment y = f'(x + \theta \increment x) \cdot \increment x,
\quad 0 < \theta < 1.
\]我们知道，函数的微分\(\dd{y} = f'(x) \cdot \increment x\)是函数的增量\(\increment y\)的近似表达式，一般说来，以\(\dd{y}\)近似代替\(\increment y\)时所产生的误差只有当\(\increment x\to0\)时才趋于零；而上式却给出了自变量取得有限增量\(\increment x\)（\(\abs{\increment x}\)不一定很小）时，函数增量\(\increment y\)的准确表达式.因此，这个定理也叫作\textbf{有限增量定理}，上式称为\textbf{有限增量公式}.
...
\section{洛必达法则}
如果当\(x \to a\)（或\(x \to \infty\)）时，两个函数\(f(x)\)与\(F(x)\)都趋于零或都趋于无穷大，那么极限\(\lim\frac{f(x)}{F(x)}\)可能存在，也可能不存在.通常把这种极限叫做\textbf{未定式}，并简记为\(\frac{0}{0}\)或\(\frac{\infty}{\infty}\).
...
\]
\begin{proof}
因为求\(\frac{f(x)}{F(x)}\)当\(x \to a\)时的极限与\(f(a)\)及\(F(a)\)无关，所以可以假定\(f(a)=F(a)=0\)，于是由条件1、2可知，\(f(x)\)及\(F(x)\)在点\(a\)的某一邻域内是连续的.
设\(x\)是这邻域内的一点，那么在以\(x\)及\(a\)为端点的区间上，\hyperref[theorem:微分中值定理.柯西中值定理]{柯西中值定理}的条件均满足，因此有\[
\frac{f(x)}{F(x)}
= \frac{f(x)-f(a)}{F(x)-F(a)}
= \frac{f'(\xi)}{F'(\xi)}
\quad(\xi \text{在} x \text{与} a \text{之间}).
\]令\(x \to a\)，这时\(\xi \to a\)，再根据条件3便得要证的结论.
\end{proof}
\end{theorem}
像这样，在一定条件下，通过分子分母分别求导再求极限来确定未定式的值的方法称为\textbf{洛必达法则}（L'Hospital's rule）.

如果\(\frac{f'(x)}{F'(x)}\)当\(x \to a\)时仍属\(\frac{0}{0}\)型，且这时\(f'(x)\)和\(F'(x)\)能满足定理中\(f(x)\)和\(F(x)\)所要满足的条件，那么可以继续施用洛必达法则先确定\(\lim\limits_{x \to a} \frac{f'(x)}{F'(x)}\)，从而确定\(\lim\limits_{x \to a} \frac{f(x)}{F(x)}\)，即\[
...
洛必达法则不总是有效的.
作为反例，对于极限\(\a\)，虽然\(\l\frac{(x+\sin x)'}{x'} = \l(1+\cos x)\)不存在，但原极限存在，且\(\a = \l1+\l\frac{\sin x}{x} = 1 + 0 = 1\).
...
在无视这个错误的情况下继续求解居然算得\(L = \l\frac{2\cos x}{e^{\sin x}(2\cos x + x + \sin x \cos x)} = 0\).
这显然是错误的.
\end{example}
...
如果函数\(f(x)\)在含有\(x_0\)的某个开区间\((a,b)\)内具有直到\((n+1)\)阶的导数，则对任一\(x \in (a,b)\)，有\begin{equation}\label{equation:微分中值定理.泰勒公式1}
f(x) = p_n(x) + R_n(x),
\end{equation}其中\begin{gather}
p_n(x) = \sum_{k=0}^n \left[ \frac{f^{(k)}(x_0)}{k!} (x-x_0)^k \right], \label{equation:微分中值定理.泰勒公式.多项式1} \\
R_n(x) = \frac{f^{(n+1)}(\xi)}{(n+1)!} (x-x_0)^{n+1}. \label{equation:微分中值定理.泰勒公式.余项1}
\end{gather}
这里\(\xi\)是\(x_0\)与\(x\)之间的某个值，即\(\xi\in(\min\{x_0,x\},\max\{x_0,x\})\).
\end{theorem}

多项式\labelcref{equation:微分中值定理.泰勒公式.多项式1} 称为函数\(f(x)\)按\((x-x_0)\)的幂展开的\(n\)次\DefineConcept{泰勒多项式}，公式\labelcref{equation:微分中值定理.泰勒公式1} 称为\(f(x)\)按\((x-x_0)\)的幂展开的\cemph{带有拉格朗日型余项}的\(n\)阶\DefineConcept{泰勒公式}，而\(R_n(x)\)的表达式\labelcref{equation:微分中值定理.泰勒公式.余项1} 称为\DefineConcept{拉格朗日型余项}.
...
\subsection{佩亚诺型余项}
在不需要余项的精确表达式时，\(n\)阶泰勒公式也可以写成\begin{equation}\label{equation:微分中值定理.泰勒公式2}
f(x) = p_n(x) + o[(x - x_0)^n].
\end{equation}\(R_n(x)\)的表达式\labelcref{equation:微分中值定理.泰勒公式.余项2} 称为\DefineConcept{佩亚诺型余项}.
\cref{equation:微分中值定理.泰勒公式2} 称为\(f(x)\)按\((x-x_0)\)的幂展开的带有佩亚诺型余项的\(n\)阶泰勒公式.
该公式虽然是在“\(f^{(n+1)}(x)\)在区间\((a,b)\)内有界”的条件下推得的，但其实只要在“\(f(x)\)在含有\(x_0\)的开区间\((a,b)\)内具有直到\(n\)阶的导数，且\(f^{(n)}(x)\)在\((a,b)\)内连续”的条件下就成立.

\subsection{麦克劳林公式}
在泰勒公式\labelcref{equation:微分中值定理.泰勒公式1} 中，如果取\(x_0 = 0\)，则\(\xi\)在\(0\)与\(x\)之间.
因此可以令\(\xi = \theta x\ (0 < \theta < 1)\)，从而泰勒公式变成较简单的形式，即所谓带有拉格朗日型余项的\DefineConcept{麦克劳林（Maclaurin）公式}
\begin{equation}\label{equation:微分中值定理.泰勒公式3}
f(x)=\sum_{k=0}^n \left[ \frac{f^{(k)}(0)}{k!} x^k \right]
	+ \frac{f^{(n+1)}(\theta x)}{(n+1)!} x^{n+1},
\quad 0 < \theta < 1.
\end{equation}

在泰勒公式\labelcref{equation:微分中值定理.泰勒公式2} 中，如果取\(x_0 = 0\)，则有带有佩亚诺型余项的麦克劳林公式\begin{equation}\label{equation:微分中值定理.泰勒公式4}
f(x)=\sum_{k=0}^n \left[ \frac{f^{(k)}(0)}{k!} x^k \right] + o(x^n).
\end{equation}

误差估计式\labelcref{equation:微分中值定理.泰勒公式.误差1} 相应地变成\begin{equation}\label{equation:微分中值定理.泰勒公式.误差2}
...
除了上述带有拉格朗日型余项的泰勒公式以外，还有带有\textbf{柯西余项}的泰勒公式和带有\textbf{积分余项}的泰勒公式.
...
\subsection{柯西余项}
柯西余项的表达式为\begin{equation}\label{equation:微分中值定理.泰勒公式.余项4}
R_n = \frac{f^{(n)}(\zeta)}{n!} (x-\zeta)^k (x-x_0),
\end{equation}
其中\(\zeta = x_0 + \theta' (x-x_0)\ \theta'\in(0,1)\).
...
设\(f(x)\)在区间\(I\)上连续，对\(I\)上任意不同两点\(x_1\)、\(x_2\)，如果恒有\[f\left(\frac{x_1+x_2}{2}\right) < \frac{f(x_1)+f(x_2)}{2},\]那么称\(f(x)\)在\(I\)上的图形是（向上）凹的（Convex），或称\(f(x)\)是\(I\)上的\textbf{凹函数}；
如果恒有\[f\left(\frac{x_1+x_2}{2}\right) > \frac{f(x_1)+f(x_2)}{2},\]那么称\(f(x)\)在\(I\)上的图形是（向上）凸的（Concave），或称\(f(x)\)是\(I\)上的\textbf{凸函数}.
\end{definition}
更一般地，对于\(\forall x_1,x_2 \in I\)，\(\forall \lambda \in (0,1)\)，若\[
f[\lambda x_1 + (1-\lambda) x_2] < \lambda f(x_1) + (1 - \lambda) f(x_2)
\]成立，则称\(f(x)\)是\((x_1,x_2)\)上的凹函数；若\[
f[\lambda x_1 + (1-\lambda) x_2] > \lambda f(x_1) + (1-\lambda) f(x_2)
...
如果曲线\(y=f(x)\)在经过点\(\opair{x_0,f(x_0)}\)时，曲线的凹凸性改变了，那么就称点\(\opair{x_0,f(x_0)}\)为这曲线的\textbf{拐点}.
\end{definition}
...
\begin{definition}[驻点]
设函数\(f(x)\)在区间\([a,b]\)上连续，在\((a,b)\)内具有一阶导数.如果\(f'(x_0)=0\)，则称\(\opair{x_0,f(x_0)}\)为\textbf{驻点}.
\end{definition}

\begin{definition}[极值点]
设函数\(f(x)\)在点\(x_0\)的某邻域\(U(x_0)\)内有定义，如果对于去心邻域\(\mathring{U}(x_0)\)内的任一\(x\)，有\[
f(x)<f(x_0) \quad \text{（或}f(x)>f(x_0)\text{）},
\]那么称\(f(x_0)\)是函数\(f(x)\)的一个\textbf{极大值}（或\textbf{极小值}）.

函数的极大值与极小值统称为函数的\textbf{极值}.使函数取得极值的点称为\textbf{极值点}.
\end{definition}
...
\begin{enumerate}
\item 求出\(f(x)\)在\((a,b)\)内的驻点\(\v{x}{m}\)和不可导点\(x'_1,x'_2,\dotsc,x'_n\)；
...
我们用比值\(\frac{\abs{\increment\alpha}}{\abs{\increment s}}\)，即单位弧段上切线转过的角度的大小来表达弧段\(\arc{MM'}\)的平均弯曲程度，把这比值叫做弧段\(\arc{MM'}\)的\textbf{平均曲率}，并记作\(\overline{K}\)，即\[
\overline{K} = \abs{\frac{\increment\alpha}{\increment s}}.
\]

类似于从平均速度引进瞬时速度的方法，当\(\increment s\to0\)（即\(M' \to M\)）时，上述平均曲率的极限叫做曲线\(C\)在点\(M\)处的\textbf{曲率}，记作\(K\)，即\[
...
当点\(\opair{x,f(x)}\)沿曲线\(C\)移动时，相应的曲率中心\(D\)的轨迹曲线\(G\)称为曲线\(C\)的\textbf{渐屈线}，相对地，曲线\(C\)称为曲线\(G\)的\textbf{渐伸线}.
