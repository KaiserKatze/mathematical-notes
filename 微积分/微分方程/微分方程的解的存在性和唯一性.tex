...
%@see: 《数学分析八讲》（辛钦） P157 引理（阿尔泽拉-阿斯科拉引理）
...
%@see: https://math.berkeley.edu/~mgu/MA128ASpring2017/MA128ALectureWeek9.pdf
\begin{definition}
设区域\(D \subseteq \mathbb{R}^2\)，
函数\(f\colon D \to \mathbb{R}\).
如果存在实数\(L > 0\)，
使得对于任意两点\((x,y_1),(x,y_2) \in D\)，
成立\begin{equation*}
	\abs{ f(x,y_1) - f(x,y_2) }
	\leq
	L \abs{ y_1 - y_2 },
\end{equation*}
则称“函数\(f\)在区域\(D\)上关于\(y\)满足\DefineConcept{利普希茨条件}%
（function \(f\) satisfies a \emph{Lipschitz condition} in the variable \(y\) on \(D\)）”，
称“\(L\)是函数\(f\)的\DefineConcept{利普希茨常数}（Lipschitz constant）”.
\end{definition}

\begin{example}
函数\(f(x,y) \defeq x y^2\)在区域\(D \defeq \Set{ (x,y) \in \mathbb{R}^2 \given 0 \leq x \leq X }\)上不满足利普希茨条件.
\end{example}
\begin{example}
函数\(f(x,y) \defeq x y^2\)在区域\(D \defeq \Set{ (x,y) \in \mathbb{R}^2 \given 0 \leq x \leq X, -Y \leq y \leq Y }\)上满足利普希茨条件.
\end{example}
