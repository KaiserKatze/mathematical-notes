...
\end{equation}
像微分方程 \labelcref{equation:微分方程.分离出最高阶导数} 这样的微分方程称为
...
函数\(f\)称为“微分方程 \labelcref{equation:微分方程.线性常微分方程的一般形式}
的\DefineConcept{非齐次项}（non-homogeneous term）”.
当\(f(x) \equiv 0\)时，我们称微分方程 \labelcref{equation:微分方程.线性常微分方程的一般形式} 为
...
带初速度的自由落体运动问题
--- 已知\begin{equation*}
%@see: 《常微分方程教程（第二版）》（丁同仁、李承治） P6 (1.9)
	\begin{cases}
		x''(t) = -g, \\
		x(0) = x_0, \\
		x'(0) = v_0,
...
% 初值问题又称为\DefineConcept{柯西问题}.

\begin{example}
%@see: 《常微分方程教程（第二版）》（丁同仁、李承治） P128
悬链线问题
--- 已知\begin{equation*}
	\begin{cases}
		y''(x) = a \sqrt{1 + (y'(x))^2}
...
% 演化规律不显含时间变量的系统，称为\DefineConcept{自治系统}.
