...
由闭区间\([a,b]\)上全部的黎曼可积函数组成的集合，称作\DefineConcept{黎曼可积函数族}\footnote{%
...
\begin{proof}
因为\(f(x) \geqslant 0\)，所以在区间\([a,b]\)上任取\(n\)个分点\(\AutoTuple{x}{n}\)使之满足\[
...
那么\(\exists\mu\in\left[\inf\limits_{a \leqslant x \leqslant b} f(x),\sup\limits_{a \leqslant x \leqslant b} f(x)\right]\)，使得\[
...
\]在上式中令\(x=b\)，就得到所要证明的公式，并称之为\DefineConcept{牛顿-莱布尼茨公式}（或\DefineConcept{微积分基本公式}）.
\end{proof}
\end{theorem}
...
现在要计算它的面积.
这里，\(\varphi(\theta)\)在\([\alpha,\beta]\)上连续，且\(\varphi(\theta)\geqslant0\).
...
\leqslant \sqrt{ \int_a^b [f(x)]^2 \dd{x} }
