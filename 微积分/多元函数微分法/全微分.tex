...
于是\begin{gather*}
	...
	= \lim_{x\to0} \frac{g(x,0)}{x} % 令\(x = \rho \cos\theta\)
	= \lim_{x\to0} \frac{g(x,0)}{\rho} \frac{\rho}{\rho \cos\theta}
	% \(\cos\theta\)是有界量，\(g(x,0)\)是比\(\rho\)高阶的无穷小，
	% 利用\cref{theorem:函数极限.无穷小.有界函数与无穷小的乘积是无穷小}
	% 可知\(g'_x(0,0) = 0\)，\(g'_y(0,0)\)同理
	= 0, \\
	g'_y(0,0) = \lim_{y\to0} \frac{g(0,y) - g(0,0)}{y - 0}
	= \lim_{y\to0} \frac{g(0,y)}{y} % 令\(y = \rho \sin\theta\)
