...
最小值是\(\sqrt{9-5\sqrt3}\).
\end{solution}
%@Mathematica: MinValue[{Sqrt[x^2 + y^2 + z^2], x^2 + y^2 - z == 0, x + y + z == 1}, {x, y, z}, Reals]
%@Mathematica: MaxValue[{Sqrt[x^2 + y^2 + z^2], x^2 + y^2 - z == 0, x + y + z == 1}, {x, y, z}, Reals]
\end{example}
...
于是函数\(f\)在给定约束条件下的最小值为\begin{equation}
	f(13/3,52/5) = 26.
\end{equation}
%@Mathematica: Minimize[{m + n + Sqrt[m^2 + n^2], 1/m + 8/n == 1}, {m, n}, PositiveReals]
\end{solution}
\end{example}
