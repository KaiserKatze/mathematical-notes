...
% 分别用\(tx\)和\(ty\)代\(x\)和\(y\)，得\[
% 	tx f'_1(tx,ty) + ty f'_2(tx,ty) = n f(tx,ty),
% \]
% 即\[
% 	f(tx,ty)
% 	= \frac{t}{n} x f'_1(tx,ty)
% 	+ \frac{t}{n} y f'_2(tx,ty).
% \]
% 要证\(f(tx,ty) = t^n f(x,y)\)，
% 只需证\(tx f'_1(tx,ty) + ty f'_2(tx,ty) = nt^n f(x,y)\).
% \end{proof}
\end{example}
