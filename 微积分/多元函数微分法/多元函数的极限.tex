...
在向量空间\(\mathbb{R}^n\)上引入\DefineConcept{内积}运算\begin{equation*}
...
设\(\vb{a}=(\AutoTuple{a}{n})\in\mathbb{R}^n\).
对于任意正数\(\delta\)，
把点集\begin{equation*}
	\Set{ \vb{x}\in\mathbb{R}^n \given \norm{\vb{x}-\vb{a}}<\delta }
\end{equation*}
称为“点\(\vb{a}\)的 \DefineConcept{\(\delta\)邻域}”，
记作\(U(\vb{a},\delta)\)或\(U_\delta(\vb{a})\).
...
把点集\begin{equation*}
	U(\vb{a},\delta) - \vb{a}
\end{equation*}
称为“点\(\vb{a}\)的 \DefineConcept{去心\(\delta\)邻域}”，
记作\(\mathring{U}(\vb{a},\delta)\)或\(\mathring{U}_\delta(\vb{a})\).
把\(\vb{a}\)称为“去心邻域\(\mathring{U}(\vb{a},\delta)\)的\DefineConcept{中心}”.
把\(\delta\)称为“去心邻域\(\mathring{U}(\vb{a},\delta)\)的\DefineConcept{半径}”.
\end{definition}
\begin{definition}
...
%@see: 《数学分析（第二版 下册）》（陈纪修） P115 定理11.1.6（闭矩形套定理）
...
\end{theorem}
\begin{theorem}
%@see: 《数学分析（第二版 下册）》（陈纪修） P116 定理11.1.6'（Cantor闭区域套定理）
...
\begin{theorem}
%@see: 《数学分析（第二版 下册）》（陈纪修） P117 定理11.1.7（Bolzano-Weierstrass定理）
\(\mathbb{R}^n\)上的有界点列\(\{\vb{x}_n\}\)中必有收敛子列.
\end{theorem}
\begin{corollary}
%@see: 《数学分析（第二版 下册）》（陈纪修） P117 推论11.1.1
...
则称“\(\{\vb{x}_k\}\)是\DefineConcept{基本点列}或\DefineConcept{柯西点列}”.
\end{definition}
\begin{theorem}
%@see: 《数学分析（第二版 下册）》（陈纪修） P117 定理11.1.8（Cauchy收敛原理）
...
\begin{proof}
%\cref{theorem:不等式.基本不等式2}
%\cref{theorem:不等式.不等式与非零数相乘}
由于\(x^2+y^2 \geq 2xy\)，
所以当\(xy>0\)时，
...
\end{proof}
%@Mathematica: Limit[(x y)/Sqrt[x^2 + y^2], {x, y} -> {0, 0}]
\end{example}
...
\end{equation*}成立，
则称常数\(A\)为“函数\(f(\vb{x})\)当\(\vb{x}\to\vb{a}\)时的\DefineConcept{极限}”，记作\begin{equation*}
...
\end{equation*}\begin{itemize}%武忠祥把这种解法称为“先代后求”
