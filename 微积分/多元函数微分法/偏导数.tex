...
\end{equation*}称为“函数\(f\)对\(x\)的\DefineConcept{偏导函数}”，
记作\begin{equation*}
	\pdv{f}{x},
	\quad\text{或}\quad
	f'_x(x,y).
...
但是它们的混合偏导数不相等.
\begin{solution}
%@Mathematica: f[x_, y_] := Piecewise[{{(x y (x^2 - y^2))/(x^2 + y^2), {x, y} != {0, 0}}, {0, {x, y} == {0, 0}}}]
取\begin{equation*}
	f(x,y) = \left\{ \begin{array}{cl}
	...
	\end{array} \right.
\end{equation*}
%@Mathematica: D[f[x, y], x] // Factor
%@Mathematica: D[f[x, y], y] // Factor
因为\begin{align*}
	...
	\end{array} \right.
\end{align*}
%@Mathematica: Limit[(D[f[x, y], x] /. x -> 0)/y, y -> 0]
%@Mathematica: Limit[(D[f[x, y], y] /. y -> 0)/x, x -> 0]
所以\begin{align*}
...
% 	\pdv[2]{f(x,y)}{y} = \left\{ \begin{array}{rl}
% 		\phi_1''(y), & y>0, \\
% 		0, & y=0, \\
% 		\phi_2''(y), & y<0.
% 	\end{array} \right.
...
% \end{equation*}
% \end{solution}
%TODO {8b6edada-f2fd-4ae5-9020-eb533149a54c} 于2024年12月19日说存在反例，但没有具体说反例是怎样构造的
\end{example}
