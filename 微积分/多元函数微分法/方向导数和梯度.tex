...
对于曲线\(L^*\)上的一切点，
已给函数的函数值都是\(c\)，
所以我们称平面曲线\(L^*\)为函数\(z = f(x,y)\)的\DefineConcept{等值线}.
...
\end{equation*}
这向量称为函数\(f(x,y,z)\)在点\(P_0(x_0,y_0,z_0)\)的\DefineConcept{梯度}，
...
称为（三维的）\DefineConcept{向量微分算子}或\DefineConcept{Nabla算子}.
记\begin{equation*}
	\grad f
	\defeq
...
f(x,y,z) = c
\end{equation*}为函数\(f(x,y,z)\)的\DefineConcept{等值面}的概念，
...
如果对于空间区域\(G\)内的任一点\(M\)，
都有一个确定的数量值\(f(M)\)，
则称在这空间区域\(G\)内确定了一个\DefineConcept{数量场}.
一个数量场可用一个数量值函数\(f(M)\)来确定.

如果与点\(M\)相对应的是一个向量值函数\(\vb{F}(M)\)，
则称在这空间区域\(G\)内确定了一个\DefineConcept{向量场}.
一个向量场可用一个向量值函数\(\vb{F}(M)\)来确定.

若向量场\(\vb{F}(M)\)是某个数量值函数\(f(M)\)的梯度，
则称\(f(M)\)是向量场\(\vb{F}(M)\)的一个\DefineConcept{势函数}，
并称向量场\(\vb{F}(M)\)为\DefineConcept{势场}.
\end{definition}
由此可知，
