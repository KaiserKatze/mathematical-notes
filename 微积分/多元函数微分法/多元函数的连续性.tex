...
“\(\vb{x}_0\)是函数\(f\)的\DefineConcept{不连续点}”
...
\begin{example}
举例说明：即便二元函数在一点连续，但是它在该点的任意邻域内都不连续.
\begin{solution}
设\begin{equation*}
	f(x,y) = \begin{cases}
		x^2+y^2, & (x,y) \in \mathbb{Q}^2, \\
		0, & \text{其他}.
	\end{cases}
\end{equation*}
显然\(f(0,0) = 0\).
...
\begin{itemize}
	\item 假设\((x,y) \in \mathbb{Q}^2\).
	对于任意给定\(\epsilon > 0\)，
	只要取\(\delta = \sqrt{\epsilon}\)，
	当\(\sqrt{x^2+y^2} < \delta\)时，
	就有\begin{align*}
		\abs{f(x,y) - f(0,0)}
		= \abs{(x^2+y^2) - 0}
		= x^2+y^2
...
对于任意的\(\epsilon > 0\)，
总存在\(\delta > 0\)，
使得当\(\sqrt{x^2+y^2} < \delta\)时，
恒有\(\abs{f(x,y) - f(0,0)} < \epsilon\)；
这就说明\(f\)在点\((0,0)\)连续.

点\((0,0)\)的任意\(\delta\)邻域\(U_\delta(0,0)\)内均包含无数个坐标不全为有理数的点，
同时也包含无数个坐标全为有理数的点.
从\((0,0)\)的去心\(\delta\)邻域内取一点\((x_0,y_0) \in \mathring{U}_\delta(0,0)\).
\begin{itemize}
	\item 若\((x_0,y_0) \in \mathbb{Q}^2\)，
	则\(f(x_0,y_0) = x_0^2+y_0^2 > 0\).

	由于无理数在实数集中的稠密性，
	可以在\((x_0,y_0)\)的任意小邻域内找到收敛于\((x_0,y_0)\)、坐标不全为有理数的点列\(\{(x_n,y_n)\}\)，
	使得\begin{align*}
	...
	由于有理数在实数集中的稠密性，
	可以在\((x_0,y_0)\)的任意小邻域内找到收敛于\((x_0,y_0)\)、坐标全为有理数的点列\(\{(u_n,v_n)\}\)，
	使得\begin{align*}
		\lim_{n\to\infty} f(u_n,v_n)
		= \lim_{n\to\infty} (u_n^2+v_n^2)
		= x_0^2+y_0^2
		\neq 0
		= f(x_0,y_0).
	\end{align*}
\end{itemize}
综上所述，
\(f\)在\((0,0)\)的任意邻域内除原点外处处不连续.
\end{solution}
\end{example}

\subsection{多元连续函数的和差积商以及复合函数的连续性}
