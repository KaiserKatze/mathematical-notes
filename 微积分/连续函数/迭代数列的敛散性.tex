...
设\(a\)是函数\(f\)的不动点，
\(f\)在点\(a\)连续，
在点\(a\)的邻域\((a-r,a+r)\)上严格单调增加，
并且在区间\((a-r,a)\)上有\(f(x)>x\)，
而在区间\((a,a+r)\)上有\(f(x)<x\)，
...
\]
则\begin{itemize}
	\item 数列\(\{x_n\}_{n\geq1}\)以后不会越出区间\((a-r,a+r)\)，
	即\(x_n \in (a-r,a+r)\ (n=1,2,\dotsc)\)，
...
用数学归纳法可以证明数列\(\{x_n\}_{n\geq1}\)完全落在区间\((a-r,a)\)内，且严格单调增加.
%\hyperref[theorem:极限.数列的单调有界定理]{单调有界定理}
由于它以\(a\)为上界，因此收敛.
它的极限应当在区间\([a_1,a]\subseteq(a-r,a]\)内.
由于\(a\)是唯一的不动点，因此极限就是\(a\).
