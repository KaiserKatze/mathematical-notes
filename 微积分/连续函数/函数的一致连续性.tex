...
\end{equation*}
其中\(\alpha = \min f(x)\)，\(\beta = \max f(x)\)；
那么\begin{gather*}
	n \alpha \leq f(x_1) + f(x_2) + \dotsb + f(x_n) \leq n \beta, \\
	\alpha \leq \frac{1}{n} \left[f(x_1) + f(x_2) + \dotsb + f(x_n)\right] \leq \beta.
\end{gather*}
根据介值定理，在\([a,b]\)上必存在一点\(\xi\)使得\begin{equation*}
	f(\xi) = \frac{1}{n} \left[ f(x_1) + f(x_2) + \dotsb + f(x_n) \right]
\end{equation*}
成立.
\end{proof}
\end{example}
