...
\begin{proposition}
对于\(\sum\limits_{n=1}^\infty u_n\)和\(\sum\limits_{n=1}^\infty v_n\)这两个级数，
不论它们是都收敛，还是都发散，还是一个收敛一个发散，
将它们的通项对应相乘所得的级数\(\sum\limits_{n=1}^\infty u_n v_n\)既可能收敛，也可能发散.
\end{proposition}
...
则级数\(\sum\limits_{n=1}^\infty u_n v_n\)绝对收敛.
\begin{proof}
因为\(\sum\limits_{n=1}^\infty v_n\)收敛，
根据\hyperref[theorem:无穷级数.收敛级数性质5]{级数收敛的必要条件}，
必有\(\lim\limits_{n\to\infty} v_n = 0\).
...
则级数\(\sum\limits_{n=1}^\infty u_n v_n\)绝对收敛.
\begin{proof}
因为级数\(\sum\limits_{n=1}^\infty u_n\)绝对收敛，
所以级数\(\sum\limits_{n=1}^\infty \abs{u_n}\)收敛.
又因为级数\(\sum\limits_{n=1}^\infty v_n\)条件收敛，
所以根据\hyperref[theorem:无穷级数.收敛级数性质5]{级数收敛的必要条件}
有\(\lim_{n\to\infty} v_n = 0\).
因此，根据\cref{theorem:绝对收敛.命题3}，
级数\(\sum\limits_{n=1}^\infty \abs{u_n} v_n\)绝对收敛，
也就是说，级数\[
