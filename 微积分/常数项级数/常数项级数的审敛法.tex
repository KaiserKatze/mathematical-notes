...
%因为\[
%	\frac{a_n}{s_n^k}
%	= \frac{s_n-s_{n-1}}{s_n^k}
%	< \frac{s_n-s_{n-1}}{s_{n-1} s_n^{k-1}}
%	= \frac{1}{s_{n-1} s_n^{k-2}} - \frac{1}{s_n^{k-1}}
%	< \frac{1}{s_{n-1}^{k-1}} - \frac{1}{s_n^{k-1}},
%\]
%所以\[
%	\sum\limits_{i=2}^n \frac{a_i}{s_i^k}
%	< \frac{1}{s_1^{k-1}} - \frac{1}{s_n^{k-1}}
%	< \frac{1}{a_1^{k-1}},
%\]
%这就是说部分和\(\sum\limits_{i=1}^n \frac{a_i}{s_i^k}\)有界，
%因此，正项级数\(\sum\limits_{n=1}^\infty a_n\)收敛.
%\end{proof}
\end{example}
