\section{计数原理}
\DefineConcept{计数原理}，
是指\DefineConcept{加法原理}（addition principle）
和\DefineConcept{乘法原理}（multiplication principle）：\begin{itemize}
	\item {\rm\bf 加法原理}

	若事件\(A_1\)有\(m_1\)种不同选取方式，
	事件\(A_2\)有\(m_2\)种不同选取方式，……，
	事件\(A_k\)有\(m_k\)种不同选取方式，
	且这\(k\)个事件之间没有共同的选取方式，
	则“这\(k\)个事件有1个发生”有\(m_1+m_2+\dotsb+m_k\)种不同选取方式.
	...
	事件\(A_2\)有\(m_2\)种不同选取方式，……，
	事件\(A_k\)有\(m_k\)种不同选取方式，
	则“这\(k\)个事件依次发生”有\(m_1 m_2 \dotsm m_k\)种不同选取方式.
\end{itemize}
\begin{remark}
...
例如，从\(a,b,c,d\)四个字母里一次取出两个的排列数有12个，即\begin{equation*}
	\begin{gathered}
	ab, \quad
	ac, \quad
	ad, \quad
