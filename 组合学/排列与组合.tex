...
以后我们用符号\(A_n^k\)表示从\(n\)个元素一次取出\(k\)个的排列数，
即\begin{equation}
...
因此\(C_n^k \times k!\)便等于\(n\)个元素一次取\(k\)个的排列数，
即\begin{equation*}
...
\begin{example}[错排问题]
将\(n\)个不同的小球，放入\(n\)个不同的袋子中，
要求袋子序号与其中小球的序号不同，求可能的放法种数.
\begin{solution}
假设所求放法种数有\(D(n)\)种.
当\(n=1\)时，显然不可能将小球放入序号不同的袋子，于是\(D(1) = 0\).
当\(n=2\)时，要使袋子序号与其中小球的序号不同，只能将1号球放入2号袋，再将2号球放入1号袋，于是\(D(2) = 1\).
当\(n=3\)时，如果不考虑上述限制，那么小球的排列为\begin{equation*}
	123,
	132,
	213,
	231,
...
就得到\(231,312\)，
于是\(D(3) = 2\).
让我们观察上述计算结果，便可归纳如下规律：
\begin{enumerate}
	\item 要使1号球不放入1号袋，一共有\(n-1\)种放法；
	\item 假设1号球在\(x\ (x\neq1)\)号袋中，那么\(x\)号球有两类放法
		--- 要么\(x\)号球在1号袋中，要么\(x\)号球不在1号袋中.

		\begin{itemize}
			\item 如果\(x\)号球在1号袋中，
			于是剩余\(n-2\)个小球的放法种数是\(D(n-2)\).

			\item 如果\(x\)号球不在1号袋中，
			那么我们只能从除了1号袋、\(x\)号袋以外的其余\(n-2\)个袋子中选出1个袋子，将\(x\)号球放入其中.
			此时\(x\)号球就好似1号球的“替身”，虽然1号袋空着，但是\(x\)号球不可放入1号袋.
			换句话说，这种情况相当于从体系内排除了1号球、\(x\)号袋，只考虑剩余\(n-1\)个小球和袋子的放法种数，
			于是它们的放法种数是\(D(n-1)\).
		\end{itemize}

		因此，根据加法原理，
		在放好1号球以后，剩余小球的放法种数为\(D(n-2) + D(n-1)\).
\end{enumerate}
根据乘法原理，结合上述讨论可以建立递归方程\begin{equation*}
	\begin{cases}
		D(0) = 0, \\
		D(1) = 1, \\
		D(n) = (n-1) (D(n-1) + D(n-2)),
...
\end{equation*}
于是\begin{equation*}
%@Mathematica: RSolve[{a[n] == (n - 1) (a[n - 1] + a[n - 2]), a[1] == 0, a[2] == 1}, a[n], n]
	D(n) = n! \sum_{k=0}^n \frac{(-1)^k}{k!}.
% 	D(n) = \frac1e \Gamma(1+n,-1).
\end{equation*}
\begin{table}[hbt]
%@Mathematica: Table[{n, Subfactorial[n]}, {n, 1, 10}] // TeXForm
%@Mathematica: Table[{n, Subfactorial[n]}, {n, 11, 20}] // TeXForm
%@Mathematica: Table[Subfactorial[n], {n, 1, 20}]
	\centering
	\begin{tblr}{rr}
		\hline
		小球个数 & 放法种数 \\
	...
	\begin{tblr}{rr}
		...
		17 & \num{130850092279664} \\
		18 & \num{2355301661033953} \\
		19 & \num{44750731559645106} \\
		20 & \num{895014631192902121} \\
		\hline
	\end{tblr}
	\caption{错排问题}
\end{table}
\end{solution}
%@see: https://blog.csdn.net/shulianghan/article/details/109438773
%@see: https://oeis.org/A000166
%@see: https://mathworld.wolfram.com/Subfactorial.html
%@see: https://mathworld.wolfram.com/Derangement.html
\end{example}
...
\(\sum_{k=0}^{\floor{n/2}} C_n^{2k} = 2^{n-1}\ (n=1,2,\dotsc)\).
%@Mathematica: Sum[Binomial[n, 2 k], {k, 0, Floor[n/2]}]
\end{property}
\begin{property}\label{theorem:组合数性质6}
\(\sum_{k=0}^{\floor{(n-1)/2}} C_n^{2k+1} = 2^{n-1}\ (n=1,2,\dotsc)\).
...
%TODO Mathematica 计算结果\(\frac{(-1)^k (k-m-n-1)!}{k!(-m-n-1)!}\)与公式不同
\begin{proof}
由二项式定理可知，
一方面\begin{equation*}
	(1+x)^{n+m}
...
%@credit: {gemini}
考虑多项式\((1+x)^{2n}\)的二项式展开.
一方面，根据二项式定理，我们有\begin{align*}
	(1+x)^{2n} &= \sum_{m=0}^{2n} C_{2n}^m x^m.
\end{align*}
在此展开式中，\(x^n\)的系数明显是\(C_{2n}^n\).

另一方面，我们可以将\((1+x)^{2n}\)分解为两个多项式的乘积，
即\begin{align*}
	(1+x)^{2n}
	= (1+x)^n \cdot (1+x)^n
	= \left( \sum_{k=0}^n C_n^k x^k \right) \left( \sum_{j=0}^n C_n^j x^j \right).
\end{align*}
我们需要计算上述乘积中\(x^n\)项的系数.
根据多项式乘法规则，
该系数由所有满足指数之和为\(n\)的项的系数乘积之和组成.
具体来说，对于第一个因式中的每一项\(C_n^k x^k\)，
必须与第二个因式中的\(C_n^{n-k} x^{n-k}\)相乘，
才能得到\(x^n\)项.
这里\(k\)的取值范围是从\(0\)到\(n\).
因此，\(x^n\)的系数可以表示为\begin{align*}
	\sum_{k=0}^n \left( C_n^k \cdot C_n^{n-k} \right)
\end{align*}
利用组合数的对称性质，
即\(C_n^{n-k} = C_n^k\).
我们可以将上式重写为\begin{align*}
	\sum_{k=0}^n \left( C_n^k \cdot C_n^k \right) &= \sum_{k=0}^n (C_n^k)^2.
\end{align*}
因为两种展开方式对应同一个多项式，
所以\(x^n\)的系数必须相等，
于是\begin{align*}
