...
非零实数\(a\)、\(b\)、\(c\)依次叫做
“平面\(\alpha\)在\(x\)、\(y\)、\(z\)轴上的\DefineConcept{截距}（intercept）”.
...
\subsection{共点直线束}
平面上，经过同一个点\(P\)的所有直线组成的集合，
称为\DefineConcept{共点直线束}，
简称\DefineConcept{直线束}（pencil of lines）.
%@see: https://mathworld.wolfram.com/Pencil.html

\subsection{有轴平面束}
几何空间中，经过同一条直线\(l\)的所有平面组成的集合，
称为\DefineConcept{有轴平面束}，
简称\DefineConcept{平面束}（sheaf of planes, pencil of planes）；
直线\(l\)称为“平面束的\DefineConcept{轴}（axis）”.
%@see: https://mathworld.wolfram.com/SheafofPlanes.html
%@see: https://mathworld.wolfram.com/PencilofPlanes.html
