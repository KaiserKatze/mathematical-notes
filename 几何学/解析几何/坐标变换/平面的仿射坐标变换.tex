...
化为\begin{equation}\label{equation:解析几何.平面坐标系的点的仿射坐标变换公式I到II.矩阵形式1}
	...
	\end{bmatrix},
\end{equation}
或\begin{equation}%\label{equation:解析几何.平面坐标系的点的仿射坐标变换公式I到II.矩阵形式2}
	\begin{bmatrix}
		x \\ y \\ 1
	\end{bmatrix}
	= \begin{bmatrix}
...
\end{equation*}
\begin{proof}
假设\cref{equation:解析几何.平面坐标系的点的仿射坐标变换公式I到II} 中的系数行列式等于零，
那么由\hyperref[theorem:解析几何.两向量共线的充分必要条件2]{两向量共线的充分必要条件}可知，
\(\vb{d}_1'\)与\(\vb{d}_2'\)共线，矛盾！
因此结论成立.
\end{proof}
\end{theorem}
...
\begin{figure}[hbt]
	...
	\begin{tikzpicture}
		...
		\coordinate[label=left:$\vb{d}_2$](d2)at(-.1,1);
		\coordinate[label=right:$\vb{d}_1$](d1)at(1.2,-.3);
		\coordinate[label=above:$M_1$](M1)at(0.6,1.6);
		\coordinate[label=above:$M_2$](M2)at(2.3,1.8);
		\coordinate[label=right:$O'$](O')at(3.2,.9);
		\coordinate[label=right:$\vb{d}_1'$](d1')at(3.8,.1);
		\coordinate[label=right:$\vb{d}_2'$](d2')at(3.5,2.1);
		\begin{scope}[-{Latex[length=3mm,width=0pt 10]}]
			...
			\draw(O)--(M2);
			\draw(O')--(M2);
			\draw(M1)--(M2)node[midway,above]{$\vb{m}$};
			\draw(O')--(d1');
			\draw(O')--(d2');
		\end{scope}
	\end{tikzpicture}
...
化为\begin{equation}\label{equation:解析几何.平面坐标系的向量的仿射坐标变换公式I到II.矩阵形式1}
	...
	\end{bmatrix},
\end{equation}
或\begin{equation}%\label{equation:解析几何.平面坐标系的向量的仿射坐标变换公式I到II.矩阵形式2}
	\begin{bmatrix}
		u \\ v \\ 0
	\end{bmatrix}
	= \begin{bmatrix}
