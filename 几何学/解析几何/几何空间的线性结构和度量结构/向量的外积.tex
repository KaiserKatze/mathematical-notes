...
\begin{example}\label{example:向量的外积.向量的外积不满足结合律}
举例说明：向量的外积不满足结合律.
\begin{solution}
任取两个单位向量\(\vb\alpha,\vb\beta\)，
使之满足\(\vb\alpha \perp \vb\beta\)，
记\(\vb\gamma \defeq \VectorOuterProduct{\vb\alpha}{\vb\beta}\)，
则有\begin{align*}
	\VectorOuterProduct
		{(\VectorOuterProduct{\vb\alpha}{\vb\alpha})}
		{\vb\beta}
	&= \VectorOuterProduct{\vb0}{\vb\beta}
	= \vb0, \\
	\VectorOuterProduct
		{\vb\alpha}
		{(\VectorOuterProduct{\vb\alpha}{\vb\beta})}
	&= \VectorOuterProduct{\vb\alpha}{\vb\gamma}
	= -\vb\beta.
\end{align*}
显然两者不相等，
因此一般情况下向量的外积不满足结合律.
...
设\(\vb{a},\vb{b}\)在右手直角坐标系\([O;\vb{e}_1,\vb{e}_2,\vb{e}_3]\)中的坐标分别为\begin{equation*}
	(a_1,a_2,a_3)^T, \qquad
	(b_1,b_2,b_3)^T,
\end{equation*}
则
...
\end{equation*}
记\begin{equation*}
%@see: 《机器人学中的状态估计》（Timothy D. Barfoot著；高翔、谢晓佳译） P152 (6.2)
	\vb{a}^\times
	\defeq
	\begin{bmatrix}
		0 & -a_3 & a_2 \\
	...
	\end{bmatrix}
%@Mathematica:
%		CrossProductMatrix[{a1_, a2_, a3_}] := {
%			{   0, -a3,  a2 },
%			{  a3,   0, -a1 },
%			{ -a2,  a1,   0 }
%		}
\end{equation*}
那么我们还可以把\(\vb{a}\)与\(\vb{b}\)的外积\(\VectorOuterProduct{\vb{a}}{\vb{b}}\)的坐标写成矩阵与向量相乘的形式：\begin{equation}
	\VectorOuterProduct{\vb{a}}{\vb{b}}
	= \vb{a}^\times
	\vb{b}
	= (\vb{b}^\times)^T
...
\end{equation}
我们把\(\vb{a}^\times\)称为
“向量\(\vb{a}\)的\DefineConcept{外积矩阵}”.
% 又称{叉乘矩阵},{向量积矩阵}

容易看出，对于任意一个非零向量\(\vb{a}\)，
\(\vb{a}^\times\)是反对称矩阵，
且\(\rank\vb{a}^\times = 2\).

\begin{property}
%@see: 《机器人学中的状态估计》（Timothy D. Barfoot著；高翔、谢晓佳译） P152
设\(
	\vb{a} \defeq (a_1,a_2,a_3)^T,
	\vb{b} \defeq (b_1,b_2,b_3)^T,
	k \in \mathbb{R}
\)，
则\begin{equation*}
	(k \vb{a})^\times
...
\end{property}
\begin{remark}
\(\vb{a}^\times \vb{a} = \vb0\)和\(\rank\vb{a} = 2\)说明
\(\Ker\vb{a}^\times = \Span\{\vb{a}\}\).
\end{remark}

\begin{property}
对于任意向量\(\vb{a},\vb{b}\)，
有\((\vb{a} + \vb{b})^\times = \vb{a}^\times + \vb{b}^\times\).
\begin{proof}
由右分配律\((\vb{a} + \vb{b}) \times \vb{c} = \vb{a} \times \vb{c} + \vb{b} \times \vb{c}\)立即可得.
\end{proof}
\end{property}

\begin{example}
设向量\(\vb{a} = (a_1,a_2,a_3)^T\)，
则\begin{align*}
	(\vb{a}^\times)^2
	&= \begin{bmatrix}
	...
	(\vb{a}^\times)^3
	&= \begin{bmatrix}
		0 & a_3 (a_1^2+a_2^2+a_3^2) & -a_2 (a_1^2+a_2^2+a_3^2) \\
		-a_3 (a_1^2+a_2^2+a_3^2) & 0 & a_1 (a_1^2+a_2^2+a_3^2) \\
		a_2 (a_1^2+a_2^2+a_3^2) & -a_1 (a_1^2+a_2^2+a_3^2) & 0 \\
	\end{bmatrix}
	= -\norm{\vb{a}}^2 \vb{a}^\times.
%@Mathematica: VecA = {Subscript[a, 1], Subscript[a, 2], Subscript[a, 3]};
%@Mathematica: MatA = CrossProductMatrix[VecA];
%@Mathematica: Simplify[MatrixPower[MatA, 3] + Norm[VecA]^2 MatA,
%						Assumptions -> {Subscript[a, 1] \[Element] Reals,
%						Subscript[a, 2] \[Element] Reals,
%						Subscript[a, 3] \[Element] Reals}]
%@Mathematica: Transpose[{VecA}].{VecA} - Simplify[Norm[VecA]^2,
% 					 	 Assumptions -> {Subscript[a, 1] \[Element] Reals,
% 					 	 Subscript[a, 2] \[Element] Reals,
% 					 	 Subscript[a, 3] \[Element] Reals}] IdentityMatrix[3]
\end{align*}
\end{example}
...
\begin{remark}
当\(\vb{A}\)是旋转矩阵时，
由\(\vb{A}^{-1} = \vb{A}^T\)和\(\DeterminantA{\vb{A}} = 1\)
可得\begin{equation*}
	(\vb{A} \vb\alpha)^\times
	= \vb{A}
	\vb\alpha^\times
...
\subsection{二重外积}
通过\cref{example:向量的外积.向量的外积不满足结合律}，
我们已经知道向量的外积不满足结合律.
接下来让我们深入探索\(\VectorOuterProduct{\vb{a}}{(\VectorOuterProduct{\vb{b}}{\vb{c}})}\)的计算结果.
