...
如\cref{figure:解析几何.向量加法结合律}，
作\(\vec{OA}\)表示\(\vb{a}\)，
作\(\vec{AB}\)表示\(\vb{b}\)，
作\(\vec{BC}\)表示\(\vb{c}\)，
则\begin{gather*}
	(\vb{a} + \vb{b}) + \vb{c}
	= (\vec{OA} + \vec{AB}) + \vec{BC}
	= \vec{OB} + \vec{BC}
	= \vec{OC}, \\
	\vb{a} + (\vb{b} + \vb{c})
	= \vec{OA} + (\vec{AB} + \vec{BC})
...
作\(\vec{AB}\)表示\(\vb{a}\)，\(\vb0\)可以用\(\vec{BB}\)表示，
于是\begin{equation*}
	\vb{a} + \vb0
	= \vec{AB} + \vb{BB}
	= \vec{AB}
...
%@see: 《解析几何》（丘维声） P3 图1.6
	...
	\begin{tikzpicture}
		...
		\begin{scope}[->,>=Stealth,ultra thick]
			...
			\draw(0,\ymin)--(0,\ymax)node[above]{\(y\)};
		\end{scope}
		\coordinate[label=left:$O$](O)at(1,2);
		\coordinate[label=below:$A$](A)at(2,1);
		\coordinate[label=right:$B$](B)at(5,1.5);
		\coordinate[label=above:$C$](C)at(3,3);
		\begin{scope}[-{Latex[length=3mm,width=0pt 10]}]
			\begin{scope}[blue]
				\draw(O)--(A)node[midway,below,black]{$\vb{a}$};
				\draw(A)--(B)node[midway,below,black]{$\vb{b}$};
				\draw(B)--(C)node[midway,above right,black]{$\vb{c}$};
			\end{scope}
			\begin{scope}[red]
				\draw(O)--(B);
				\draw(A)--(C);
...
对任意向量\(\vb{a},\vb{b}\)，都有\begin{equation*}
	\VectorLengthA{\vb{a}+\vb{b}} \leq \VectorLengthA{\vb{a}} + \VectorLengthA{\vb{b}}.
\end{equation*}
\end{theorem}
...
对于任意向量\(\vb{a}\)，
由于\(\abs{0 \vb{a}} = 0 \VectorLengthA{\vb{a}} = 0\)，
所以\(0 \vb{a} = \vb{0}\).
同理，对一切实数\(\lambda\)，都有\(\lambda \vb{0} = \vb{0}\).

对于任意非零向量\(\vb{a}\)，
因为向量\(\VectorLengthA{\vb{a}}^{-1} \vb{a}\)与\(\vb{a}\)同向，
且\begin{equation*}
	\VectorLengthA{\frac{1}{\VectorLengthA{\vb{a}}} \vb{a}}
	= \frac{1}{\VectorLengthA{\vb{a}}} \VectorLengthA{\vb{a}} = 1,
\end{equation*}
所以\(\vb{a}^0 = \VectorLengthA{\vb{a}}^{-1} \vb{a}\).
...
\begin{theorem}
%@see: 《解析几何》（丘维声） P6
若向量\(\vb{a},\vb{b}\)满足\(\vb{a} = \lambda \vb{b}\)或\(\vb{b} = \mu \vb{a}\)，
则\(\vb{a}\)与\(\vb{b}\)共线.
\end{theorem}
...
从而有\begin{equation*}
	\vb{b}
	= \VectorLengthA{\vb{b}} \vb{b}^0
	= \VectorLengthA{\vb{b}} \vb{a}^0
	= \VectorLengthA{\vb{b}} (\VectorLengthA{\vb{a}}^{-1} \vb{a})
	= (\VectorLengthA{\vb{b}} \VectorLengthA{\vb{a}}^{-1}) \vb{a}.
\end{equation*}
取\(\lambda \defeq \VectorLengthA{\vb{b}} \VectorLengthA{\vb{a}}^{-1}\)，
便得\(\vb{b} = \lambda \vb{a}\).
若\(\vb{a}\)与\(\vb{b}\)反向，可以类似讨论.

唯一性.
设\(\vb{b} = \lambda \vb{a} = \mu \vb{a}\)，
则\((\lambda - \mu) \vb{a} = 0\).
因为\(\vb{a} \neq \vb0\)，
所以\(\lambda - \mu = 0\)，
即\(\lambda = \mu\).
...
则存在实数\(\lambda\)，使得\(\vb{b}=\lambda\vb{a}\)，
从而有\(\lambda\vb{a}+(-1)\vb{b}=\vb{0}\).
...
对于任意一点\(P\)，
存在非负实数\(\lambda,\mu\)，使得\(\lambda+\mu=1\)，且\begin{equation*}
	\vec{PM} = \lambda \vec{PA} + \mu \vec{PB}.
\end{equation*}
\begin{proof}
必要性.
设\(M\)在线段\(AB\)上，
则\(\vec{AM}\)与\(\vec{AB}\)同向，
并且\(0 \leq \VectorLengthA{\vec{AM}} \leq \VectorLengthA{\vec{AB}}\)，
所以\begin{equation*}
	\vec{AM} = k \vec{AB}
	\quad(0 \leq k \leq 1).
\end{equation*}
任取一点\(P\)，由上式得\(\vec{PM} - \vec{PA} = k (\vec{PB} - \vec{PA})\)，
即\begin{equation*}
	\vec{PM} = (1 - k) \vec{PA} + k \vec{PB}.
\end{equation*}
取\(
	\lambda \defeq 1 - k,
	\mu \defeq k
...
若对某一点\(P\)，
存在非负实数\(\lambda,\mu\)，
使得\(\vec{PM} = \lambda \vec{PA} + \mu \vec{PB}\)
且\(\lambda + \mu = 1\)，
则\begin{equation*}
	\vec{AM} = \vec{PM} - \vec{PA}
	= (\lambda \vec{PA} + \mu \vec{PB})
	- (\lambda + \mu) \vec{PA}
	= \mu (\vec{PB} - \vec{PA})
	= \mu \vec{AB}.
\end{equation*}
于是\(\vec{AM}\)与\(\vec{AB}\)共线，
所以\(M\)在直线\(AB\)上.
由于\(0 \leq \mu \leq 1\)，
所以\(M\)在线段\(AB\)上.
\end{proof}
