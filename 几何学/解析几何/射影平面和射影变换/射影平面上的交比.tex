...
\begin{figure}[hbt]
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			\addplot[color=blue,samples=50,smooth,domain=-5:.95]{x/(1-x)};
			\addplot[color=blue,samples=50,smooth,domain=1.05:5]{x/(1-x)};
			\draw[dashed] (1,-4)--(1,2) (-5,-1)--(5,-1);
			\fill(0,0)circle(2pt)node[above left]{$A$}
				(1,0)circle(2pt)node[above right]{$B$};
		\end{axis}
	\end{tikzpicture}
