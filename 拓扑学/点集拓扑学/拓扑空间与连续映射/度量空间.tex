...
如果对于\(\forall x,y,z \in X\)，总有\begin{enumerate}
	\item {\rm\bf 半正定性}，即\begin{equation*}
		\rho(x,y)=0 \iff x=y;
	\end{equation*}

	\item {\rm\bf 对称性}，即\begin{equation*}
		\rho(x,y) = \rho(y,x);
	\end{equation*}

	\item {\rm\bf 三角不等式}，即\begin{equation*}
...
如果总有\begin{equation*}
	...
	(\forall y \in X - \{x\})
	[\rho(x,y) > \delta_x]
\end{equation*}成立，则称\((X,\rho)\)是离散的.

例如，设\(X\)是任意一个集合，映射\(\rho\colon X \times X\to\mathbb{R}\)满足\begin{equation*}
\rho(x,y) = \left\{ \begin{array}{ll}
0, & x=y, \\
1, & x\neq y.
\end{array} \right.
\end{equation*}容易验证\(\rho\)是\(X\)的一个离散的度量，或者说度量空间\((X,\rho)\)是离散的.
\end{example}
...
对于\(\forall\epsilon>0\)，集合\begin{equation*}
...
若\begin{equation*}
	...
	[\rho(x,y) > M],
\end{equation*}
则称“\(A\)是\DefineConcept{无界的}（unbounded）”，
记作\(\diam A = \infty\).
\end{definition}
