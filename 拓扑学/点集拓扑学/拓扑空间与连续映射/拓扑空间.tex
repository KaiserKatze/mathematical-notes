...
因为\(\emptyset\)是\(X\)的一个有限子集，
所以\(X = X - \emptyset \in \T\).
...
任取\(\T_1 \subseteq \T\)，令\(\T_2 = \T_1 - \{ \emptyset \}\).
显然有\begin{equation*}
	\bigcup_{A \in \T_1} A
	= \bigcup_{A \in \T_2} A.
\end{equation*}
...
\begin{example}
%@see: 《点集拓扑讲义（第四版）》（熊金城） P62 习题 4.
试讨论：什么情况下，有限补空间是可度量化的？
%TODO
\begin{solution}
设\((X,\T)\)是一个有限补空间，\(\rho\)是\(X\)的一个度量，
\(\T_\rho\)是\(X\)的由度量\(\rho\)诱导出来的拓扑.
要使\(X\)成为一个可度量化空间，必有\(\T = \T_\rho\)，
即\begin{equation*}
	\Set{
		X-U
		\given
...
\end{equation*}
换言之，只要\(A\)是度量空间\((X,\rho)\)中的一个非空的开集，
那么\(A\)就是\(X\)的某个有限子集\(U\)的补集.
\end{solution}
\end{example}

\begin{example}
%@see: 《点集拓扑讲义（第四版）》（熊金城） P62 习题 4.
试讨论：什么情况下，可数补空间是可度量化的？
%TODO
\end{example}
