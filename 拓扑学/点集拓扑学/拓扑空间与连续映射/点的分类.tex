...
\begin{itemize}
	...
	即\(
		f\ImageOfSetUnderRelation{\TopoClosureL{A}}
		\subseteq
		\TopoClosureL{f\ImageOfSetUnderRelation{A}}
	\)；
	\item 对于\(Y\)中的任何一个子集\(B\)，\(B\)的闭包的原像包含\(B\)的原像的闭包，
	即\(
		f^{-1}\ImageOfSetUnderRelation{\TopoClosureL{B}}
		\supseteq
		\TopoClosureL{f^{-1}\ImageOfSetUnderRelation{B}}
	\).
\end{itemize}
\end{theorem}
