...
或“\(A,B\)是\(X\)的\DefineConcept{隔离子集}”.
\end{definition}
...
\subsection{连通性}
下面讨论\DefineConcept{连通性}（connectedness）.
...
\subsection{拓扑空间的各类性质}
%@see: 《点集拓扑讲义（第四版）》（熊金城） P126
我们知道，所谓拓扑不变性质，就是同胚类中全体拓扑空间所共有的性质.
事实上，对于拓扑空间的任意一个性质，
只要这个性质是借助于开集表达的，
或者借助于经由开集定义的其他概念表达的，
则该性质必定是拓扑不变性质.
%TODO 上面这句话能否依靠谓词逻辑建立严格的表述呢？
...
% 拓扑空间的某种性质，
% 如果为一个拓扑空间所具有，
% 也必然为它在任何一个连续映射下的像所具有，
% 则称这个性质是一个\DefineConcept{在连续映射下保持不变的性质}.
设\(P(x)\)是一个谓词公式，
\(X\)是一个拓扑空间.
如果
	只要\(X\)满足\(P(X)\)，
	就有对于任意连续映射\(f\colon X \to Y\)，
	%TODO 我对“也必然为它在任何一个连续映射下的像所具有”这句话的理解有两种：
	% 一种是固定陪域\(Y\)，一种是任取陪域\(Y\)，不能确定是哪一种，留待以后确定
	\(X\)在\(f\)下的像\(f(X)\)也满足\(P(f(X))\)，
则称“\(P\)是一个\DefineConcept{在连续映射下保持不变的性质}”.
\end{definition}
...
\begin{theorem}\label{theorem:连通空间.连续映射的值域是其陪域的连通子集}
%@see: 《点集拓扑讲义（第四版）》（熊金城） P126 定理4.1.8
...
%@see: 《点集拓扑讲义（第四版）》（熊金城） P127 定理4.1.9
