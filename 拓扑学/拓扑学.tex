...
\]
\end{enumerate}
那么称“映射\(\rho\)是集合\(X\)的一个\textbf{度量}（metric）”；
称实数\(\rho(x,y)\)为“从点\(x\)到点\(y\)的\textbf{距离}（distance）”.
\end{definition}
...
那么称“集合\(X\)是一个对于度量\(\rho\)而言的\textbf{度量空间}（metric space）”，
记为\((X,\rho)\).
...
综上所述，\(\T\)是\(X\)的一个拓扑，称其为\(X\)的\textbf{有限补拓扑}；
...
则称\(U\)是“点\(x\)的一个\textbf{邻域}”.

点\(x\)的所有邻域构成的\(X\)的子集族称为“点\(x\)的\textbf{邻域系}”.
\end{definition}

易见，如果\(U\)是包含着点\(x\)的一个开集，
那么它一定是\(x\)的一个邻域，
于是我们称\(U\)为“点\(x\)的一个\textbf{开邻域}”.
...
即\(U \cap (A - \{x\}) \neq \emptyset\)，
则称“点\(x\)是集合\(A\)的\textbf{聚点}”.
集合\(A\)的所有聚点构成的集合称为\(A\)的\textbf{导集}，记作\(D(A)\).

如果\(x \in A\)不是\(A\)的聚点，
即存在\(x\)的一个邻域\(U\)使得\(U \cap (A - \{x\}) = \emptyset\)，
则称\(x\)为“\(A\)的一个\textbf{孤立点}”.
\end{definition}
...
设\(X\)是一个拓扑空间，\(A \subseteq X\).
集合\(A\)及其导集\(D(A)\)的并\(A \cup D(A)\)称为“集合\(A\)的\textbf{闭包}”，记作\(\overline{A}\).
\end{definition}
...
称实数\(\d\)
为“点\(x\)到\(A\)的\textbf{距离}”，记作\(\rho(x,A)\)，即\[
\rho(x,A) \defeq \d.
\]
\end{definition}
...
设\(X\)是一个拓扑空间，\(A \subseteq X\).
如果\(A\)是点\(x \in A\)的一个邻域，则称“点\(x\)是集合\(A\)的一个\textbf{内点}”.
集合\(A\)的所有内点构成的集合称为“\(A\)的\textbf{内部}”，记作\(I(A)\).
\end{definition}
